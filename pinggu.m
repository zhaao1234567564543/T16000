...
imagesc(diff);
colormap(subplot(3,4,3), getRdBuColormap());
colorbar;
title('差异图 - XY切面');
% XZ切片
subplot(3,4,5);
...
imagesc(diff);
colormap(subplot(3,4,7), getRdBuColormap());
colorbar;
title('差异图 - XZ切面');
% YZ切片
subplot(3,4,9);
...
imagesc(diff);
colormap(subplot(3,4,11), getRdBuColormap());
colorbar;
title('差异图 - YZ切面');
% 3D投影
subplot(3,4,[4,8,12]);
...
function cmap = getRdBuColormap(m)
% 创建与ColorBrewer相似的RdBu颜色映射
...
xi = linspace(0, 1, size(baseColors, 1));
xq = linspace(0, 1, m);

cmap = interp1(xi, baseColors, xq, 'linear');
