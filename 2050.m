...
fprintf('2. 跳过，由系统根据最优空间特征自动生成\n');
clusterParamChoice = input('请选择 (1 或 2): ');
autoClusterTargets = (clusterParamChoice == 2);

if autoClusterTargets
    fprintf('已选择自动生成孔隙簇参数，系统将在初始模型阶段根据空间与形态特征随机设定。\n');
    target_max = [];
    target_min = [];
...
optParams = configureClusterTargetStrategy(optParams);
if optParams.autoClusterTargets && isfield(optParams, 'dynamicClusterTargets') && ~isempty(optParams.dynamicClusterTargets)
    dynTargets = optParams.dynamicClusterTargets;
    fprintf('自动生成的孔隙簇参数: 最小体素数 %d, 最大体素数 %d, 目标簇数 %d。\n', ...
        dynTargets.targetMin, dynTargets.targetMax, dynTargets.targetCount);
end
% 生成快速查找表
fprintf('正在生成快速查找表...\n');
...
if optParams.autoClusterTargets
    weights.cluster = weights.cluster * 1.35;
    weights.morphology = weights.morphology * 1.25;
    weights.spatial = weights.spatial * 1.25;
    weights.porosity = weights.porosity * 1.1;
    weights.connectivity = weights.connectivity * 0.9;
    weights.multiScale = weights.multiScale * 0.9;
    optParams.energyPriority = 'clusterSpatialMorphology';
else
    optParams.energyPriority = 'comprehensive';
end
% 初始化MCMC状态
...
function optParams = configureClusterTargetStrategy(optParams)
    % 根据用户选择配置孔隙簇目标策略
    if ~isfield(optParams, 'autoClusterTargets') || isempty(optParams.autoClusterTargets)
        optParams.autoClusterTargets = false;
    end

    if ~optParams.autoClusterTargets
        optParams.dynamicClusterTargets = [];
        optParams.clusterTargetPriority = 'manual';
        return;
    end

    targets = deriveAutoClusterTargets(optParams);
    optParams.dynamicClusterTargets = targets;
    optParams.targetMin = targets.targetMin;
    optParams.targetMax = targets.targetMax;
    optParams.targetClusterCount = targets.targetCount;
    optParams.clusterTargetPriority = 'auto';
end
function targets = deriveAutoClusterTargets(optParams)
    % 从原始模型特征与空间指标中自动生成孔隙簇目标
    targets = struct();
    sizes = [];
    if isfield(optParams, 'originalFeatures') && isfield(optParams.originalFeatures, 'sizes')
        sizes = double(optParams.originalFeatures.sizes);
    end

    baseMin = 8;
    baseMax = 64;
    if ~isempty(sizes)
        baseMin = max(5, round(prctile(sizes, 20)));
        baseMax = max(baseMin + 1, round(prctile(sizes, 85)));
    else
        approxVolume = prod(optParams.modelSize) * max(optParams.targetPorosity, 1e-3);
        approxCount = [];
        if isfield(optParams, 'originalFeatures') && isfield(optParams.originalFeatures, 'numClusters')
            approxCount = optParams.originalFeatures.numClusters;
        end
        if isempty(approxCount) || approxCount <= 0
            approxCount = max(4, round((prod(optParams.modelSize)^(1/3))));
        end
        meanSize = max(10, approxVolume / max(approxCount, 1));
        baseMin = max(5, round(meanSize * 0.6));
        baseMax = max(baseMin + 1, round(meanSize * 1.8));
    end

    anisotropyFactor = 1;
    if isfield(optParams, 'spatialFeatures') && isfield(optParams.spatialFeatures, 'anisotropy')
        anisotropyFactor = 1 + min(0.25, abs(optParams.spatialFeatures.anisotropy) * 0.35);
    end

    connectivityBias = 1;
    if isfield(optParams, 'spatialFeatures') && isfield(optParams.spatialFeatures, 'connectivity')
        conn = optParams.spatialFeatures.connectivity;
        if isfield(conn, 'largestComponentRatio')
            connectivityBias = 1 + min(0.2, max(0, 0.5 - conn.largestComponentRatio));
        end
    end

    randomScale = @(scale) (1 + scale * (rand() - 0.5));
    targetMin = max(5, round(baseMin * randomScale(0.25) / anisotropyFactor));
    targetMax = max(targetMin + 1, round(baseMax * randomScale(0.25) * anisotropyFactor));

    baseCount = [];
    if isfield(optParams, 'originalFeatures') && isfield(optParams.originalFeatures, 'numClusters')
        baseCount = optParams.originalFeatures.numClusters;
    end
    if isempty(baseCount) || baseCount <= 0
        meanSize = max(1, (targetMin + targetMax) / 2);
        baseCount = max(1, round(prod(optParams.modelSize) * max(optParams.targetPorosity, 1e-3) / meanSize));
    end
    targetCount = max(1, round(baseCount * randomScale(0.2) * connectivityBias));

    targets.targetMin = targetMin;
    targets.targetMax = targetMax;
    targets.targetCount = targetCount;
    targets.countRange = [max(1, round(targetCount * 0.9)), max(1, round(targetCount * 1.1))];
    targets.anisotropyFactor = anisotropyFactor;
    targets.connectivityBias = connectivityBias;
    targets.randomSignature = rand();
end
...
function model = enforceTargetClusterCount(model, optParams)
    ...
    targetCount = max(1, round(optParams.targetClusterCount));
    CC = bwconncomp(model, 26);
    if isfield(optParams, 'autoClusterTargets') && optParams.autoClusterTargets && ...
            isfield(optParams, 'dynamicClusterTargets') && ~isempty(optParams.dynamicClusterTargets)
        range = optParams.dynamicClusterTargets.countRange;
        if numel(range) == 2 && CC.NumObjects >= range(1) && CC.NumObjects <= range(2)
            % 已经落在允许范围内，保持随机匹配结果
            return;
        end
        targetCount = max(1, round(optParams.dynamicClusterTargets.targetCount));
    end
    % 如果当前没有簇，尝试播种新的孔隙簇
    if CC.NumObjects == 0
...
function finalModel = comprehensivePostProcessWithSmallPores(model, optParams)
    ...
    if isfield(optParams, 'autoClusterTargets') && optParams.autoClusterTargets
        fprintf('  6. 优先匹配自动簇空间与形态特征...\n');
        model = enforceClusterMorphologyPriority(model, optParams);
    end

    % 6/7. 最终孔隙率调整
    fprintf('  %d. 恢复目标孔隙率...\n', 6 + (optParams.autoClusterTargets ~= 0));
    model = smartPorosityAdjustment(model, originalPorosity, optParams);

    % 最终质量检查和微调
    fprintf('  %d. 最终质量检查...\n', 7 + (optParams.autoClusterTargets ~= 0));
...
function finalModel = comprehensivePostProcess(model, optParams)
    ...
    if isfield(optParams, 'autoClusterTargets') && optParams.autoClusterTargets
        fprintf('  5. 优先匹配自动簇空间与形态特征...\n');
        model = enforceClusterMorphologyPriority(model, optParams);
    end

    % 最终孔隙率调整
    fprintf('  %d. 恢复目标孔隙率...\n', 5 + (optParams.autoClusterTargets ~= 0));
...
function model = enforceClusterMorphologyPriority(model, optParams)
    % 在自动簇策略下优先匹配空间与形态特征
    if ~isfield(optParams, 'autoClusterTargets') || ~optParams.autoClusterTargets
        return;
    end
    ...
    model = adjustClusterSizeDistribution(model, optParams);
    model = matchPoreSizeDistribution(model, optParams.originalFeatures, optParams);
    model = enforceTargetClusterCount(model, optParams);
    model = adjustPorosity(model, optParams.targetPorosity);
end
