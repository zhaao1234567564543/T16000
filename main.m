...
function mcmcModel = generateComprehensiveInitialModel(dims, targetPorosity, ...
    ...
    model = generateClusterBasedInitialModel(dims, targetPorosity, clusterTarget, anisotropyProfile, optParams);
...
function E_cluster = calculateClusterEnergy(features, optParams)
    ...
    if isempty(clusterTarget) || ~isfield(clusterTarget, 'numClusters')
        ...
        clusterTarget = struct('numClusters', targetCount, 'histEdges', targetHistEdges, ...
            'histogram', targetHist, 'maxSizeTarget', max(sizes));
    end
    Ndiff = (features.numClusters - clusterTarget.numClusters).^2 / max(clusterTarget.numClusters^2, 1);
    ...
    if numel(currentHist) ~= numel(clusterTarget.histogram)
        currentHist = interp1(linspace(0,1,numel(currentHist)), currentHist, linspace(0,1,numel(clusterTarget.histogram)), 'linear', 'extrap');
        currentHist(isnan(currentHist)) = 0;
    end
    histDiff = sum((currentHist - clusterTarget.histogram).^2);
    largestClusterSize = max(sizes);
    if isfield(clusterTarget, 'maxSizeTarget') && clusterTarget.maxSizeTarget > 0
        exceedRatio = max(0, largestClusterSize / clusterTarget.maxSizeTarget - 1);
    else
        exceedRatio = 0;
    end
    maxPenalty = exceedRatio.^2 * 10;
    sizeMeanDiff = (mean(sizes) - clusterTarget.meanSize).^2 / max(clusterTarget.meanSize^2, 1);
    E_cluster = 0.45 * Ndiff + 0.25 * histDiff + 0.2 * sizeMeanDiff + 0.1 * maxPenalty;
end
function metrics = computeIslandMetrics(model)
    ...
    poreIslands = model & neighborPore <= 1;
    solidIslands = (~model) & neighborSolid <= 1;
    metrics = struct();
    metrics.numPoreIslands = nnz(poreIslands);
    metrics.numSolidIslands = nnz(solidIslands);
    metrics.poreIslandVolume = metrics.numPoreIslands;
    metrics.solidIslandVolume = metrics.numSolidIslands;
end
function E_island = calculateIslandEnergy(model)
    metrics = computeIslandMetrics(model);
    islandCount = metrics.numPoreIslands + metrics.numSolidIslands;
    volume = numel(model);
    E_island = islandCount / max(volume, 1) * 5 + ...
        0.5 * (metrics.poreIslandVolume + metrics.solidIslandVolume) / max(volume, 1);
end
...
function [moves, moveTypes] = generateComprehensiveBatchMoves(model, strategy, ...
    ...
    boundaryContext = computeBoundaryMasks(model);
    ...
    if parallelEnabled
        parfor i = 1:batchSize
            [moves{i}, moveTypes{i}] = dispatchComprehensiveMove(model, strategy, ...
                optParams, lookupTables, boundaryContext);
        end
    else
        for i = 1:batchSize
            [moves{i}, moveTypes{i}] = dispatchComprehensiveMove(model, strategy, ...
...
function [move, moveType] = dispatchComprehensiveMove(model, strategy, optParams, lookupTables, boundaryContext)
    % 按策略分派单个移动生成器
    switch strategy
        case 'boundary_exchange'
            move = generateBoundaryExchangeMove(model, boundaryContext, optParams);
            moveType = 'boundary_exchange';
        case 'boundary_birthdeath'
            move = generateBoundaryBirthDeathMove(model, boundaryContext, optParams);
...
function move = generateBoundaryExchangeMove(model, boundaryContext, optParams)
    ...
    candidates = boundaryContext.solidIdx(dist <= radius);
...
function move = generateBoundaryBirthDeathMove(model, boundaryContext, optParams)
    move = struct('linearIdx', [], 'oldValues', [], 'newValues', []);
    targetPorosity = optParams.targetPorosity;
    currentPorosity = mean(model(:));
    ...
    nFlip = max(1, round(0.005 * numel(model)));
    nFlip = min(nFlip, numel(sourceSet));
