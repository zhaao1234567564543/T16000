...
fprintf('梯度分析完成，可用区域样本数：%d\n', size(spatialGradientModel.dataset, 1));
%% 1. 二值化并保存原始模型
...
if isempty(target_porosity) || ~isfinite(target_porosity)
    warning('未提供有效的目标孔隙率，默认使用原始孔隙率。');
    target_porosity = raw_porosity;
end
target_porosity = max(0, min(1, target_porosity));
% 基于梯度模型推断目标空间特征
new_target_spatial_features = predictSpatialFeaturesFromPorosity(spatialGradientModel, target_porosity);
% 根据原始簇分布生成自适应指导参数
clusterGuidance = deriveAdaptiveClusterGuidance(originalFeatures);
% 预估目标簇统计用于软约束（不再由用户提供）
adaptiveTargetMin = clusterGuidance.recommendedMin;
adaptiveTargetMax = clusterGuidance.recommendedMax;
adaptiveTargetCount = clusterGuidance.recommendedCount;
% 询问是否保留小孔隙特征
fprintf('\n小孔隙保留选项：\n');
...
optParams.clusterGuidance = clusterGuidance;
optParams.targetMin = adaptiveTargetMin;
optParams.targetMax = adaptiveTargetMax;
optParams.targetClusterCount = adaptiveTargetCount;
...
function gradientModel = computeSpatialParameterGradients(binaryModel, referenceSpatialFeatures, referencePorosity)
    % 计算孔隙率与空间特征之间的梯度/映射关系
    if nargin < 3 || isempty(referencePorosity)
        referencePorosity = mean(binaryModel(:));
    end

    dims = size(binaryModel);
    defaultWindow = max(round(dims .* [0.25, 0.25, 0.3]), [24, 24, 8]);
    windowSize = min(dims, defaultWindow);
    stepSize = max(floor(windowSize ./ 2), [8, 8, 3]);

    startsX = unique([1:stepSize(1):dims(1), max(1, dims(1) - windowSize(1) + 1)]);
    startsY = unique([1:stepSize(2):dims(2), max(1, dims(2) - windowSize(2) + 1)]);
    startsZ = unique([1:stepSize(3):dims(3), max(1, dims(3) - windowSize(3) + 1)]);

    [refVector, metadata] = flattenSpatialFeatures(referenceSpatialFeatures);
    nFeatures = numel(refVector);
    dataset = [];

    validRange = [max(0, referencePorosity - 0.2), min(1, referencePorosity + 0.2)];
    featureMins = inf(1, nFeatures);
    featureMaxs = -inf(1, nFeatures);

    for ix = 1:numel(startsX)
        xs = startsX(ix);
        xe = min(xs + windowSize(1) - 1, dims(1));
        for iy = 1:numel(startsY)
            ys = startsY(iy);
            ye = min(ys + windowSize(2) - 1, dims(2));
            for iz = 1:numel(startsZ)
                zs = startsZ(iz);
                ze = min(zs + windowSize(3) - 1, dims(3));

                subVolume = binaryModel(xs:xe, ys:ye, zs:ze);
                localPorosity = mean(subVolume(:));
                if localPorosity < validRange(1) - 0.05 || localPorosity > validRange(2) + 0.05
                    continue;
                end

                try
                    localFeatures = computeEnhancedSpatialFeatures(subVolume);
                    localVector = flattenSpatialFeatures(localFeatures, metadata);
                catch
                    continue;
                end

                if any(~isfinite(localVector))
                    continue;
                end

                featureMins = min(featureMins, localVector');
                featureMaxs = max(featureMaxs, localVector');
                dataset = [dataset; localPorosity, localVector']; %#ok<AGROW>
            end
        end
    end

    if isempty(dataset)
        dataset = [referencePorosity, refVector'];
        featureMins = refVector';
        featureMaxs = refVector';
    end

    porosityVals = dataset(:, 1);
    featureVals = dataset(:, 2:end);
    nSamples = size(featureVals, 1);
    fits = cell(nFeatures, 1);

    for i = 1:nFeatures
        y = featureVals(:, i);
        if all(abs(y - y(1)) < 1e-8)
            fits{i} = struct('type', 'constant', 'value', y(1));
            continue;
        end

        uniquePorosity = numel(unique(porosityVals));
        degree = min(2, max(1, uniquePorosity - 1));
        if nSamples <= degree
            degree = 1;
        end

        coeffs = polyfit(porosityVals, y, degree);
        yPred = polyval(coeffs, porosityVals);
        rmse = sqrt(mean((y - yPred).^2));
        fits{i} = struct('type', 'polynomial', 'coeffs', coeffs, ...
            'degree', degree, 'rmse', rmse);
    end

    gradientModel = struct();
    gradientModel.dataset = dataset;
    gradientModel.metadata = metadata;
    gradientModel.fits = fits;
    gradientModel.windowSize = windowSize;
    gradientModel.stepSize = stepSize;
    gradientModel.validRange = validRange;
    gradientModel.referenceVector = refVector;
    gradientModel.referenceFeatures = referenceSpatialFeatures;
    gradientModel.referencePorosity = referencePorosity;
    gradientModel.featureMin = featureMins;
    gradientModel.featureMax = featureMaxs;
end

function predictedFeatures = predictSpatialFeaturesFromPorosity(gradientModel, targetPorosity)
    % 根据梯度模型预测指定孔隙率下的空间特征
    if isempty(gradientModel) || ~isfield(gradientModel, 'metadata')
        error('梯度模型不可用。');
    end

    targetPorosity = max(0, min(1, targetPorosity));
    predictedVector = zeros(size(gradientModel.referenceVector));

    for i = 1:numel(predictedVector)
        fit = gradientModel.fits{i};
        switch fit.type
            case 'constant'
                value = fit.value;
            case 'polynomial'
                value = polyval(fit.coeffs, targetPorosity);
            otherwise
                value = gradientModel.referenceVector(i);
        end
        minVal = gradientModel.featureMin(i);
        maxVal = gradientModel.featureMax(i);
        if isfinite(minVal) && isfinite(maxVal) && minVal <= maxVal
            value = min(maxVal + 0.1 * abs(maxVal), max(minVal - 0.1 * abs(minVal), value));
        end
        predictedVector(i) = value;
    end

    predictedFeatures = reconstructSpatialFeatures(gradientModel.metadata, predictedVector, ...
        gradientModel.referenceFeatures);
end

function guidance = deriveAdaptiveClusterGuidance(originalFeatures)
    % 基于原始簇统计构建自适应的指导参数
    guidance = struct();
    ...
    if isempty(sizes)
        ...
        guidance.minSize = 0;
        guidance.maxSize = 0;
        guidance.recommendedMin = 1;
        guidance.recommendedMax = 1;
        guidance.recommendedCount = 1;
        guidance.histogramEdges = 0:1;
        guidance.histogram = 1;
        return;
    end

    meanSize = mean(sizes);
    stdSize = std(sizes);
    guidance.meanSize = meanSize;
    guidance.stdSize = stdSize;
    guidance.minSize = min(sizes);
    guidance.maxSize = max(sizes);
    guidance.recommendedMin = max(1, round(meanSize - 1.5 * stdSize));
    guidance.recommendedMax = max(guidance.recommendedMin + 1, round(meanSize + 2.5 * stdSize));
    guidance.recommendedCount = max(1, round(originalFeatures.numClusters));

    nBins = min(20, max(5, round(sqrt(numel(sizes)))));
    edges = linspace(guidance.minSize, guidance.maxSize, nBins + 1);
    if numel(unique(edges)) < 2
        edges = [guidance.minSize, guidance.maxSize + 1];
    end
    histogramCounts = histcounts(sizes, edges, 'Normalization', 'probability');
    guidance.histogramEdges = edges;
    guidance.histogram = histogramCounts;
end

function [values, metadata] = flattenSpatialFeatures(features, metadataTemplate)
    % 将空间特征结构展平为向量，并记录元信息
    if nargin < 2 || isempty(metadataTemplate)
        [values, metadata] = flattenSpatialFeaturesRecursive(features, {});
        values = values(:);
    else
        metadata = metadataTemplate;
        values = zeros(numel(metadataTemplate), 1);
        for i = 1:numel(metadataTemplate)
            values(i) = getValueAtPath(features, metadataTemplate{i});
        end
    end
end

function reconstructed = reconstructSpatialFeatures(metadata, values, baseStruct)
    % 根据向量和元信息还原空间特征结构
    if nargin < 3 || isempty(baseStruct)
        reconstructed = struct();
    else
        reconstructed = baseStruct;
    end
    for i = 1:numel(metadata)
        reconstructed = setValueAtPath(reconstructed, metadata{i}, values(i)); %#ok<AGROW>
    end
end

function val = getValueAtPath(structure, meta)
    current = structure;
    for i = 1:numel(meta.path)
        fieldName = meta.path{i};
    ...
    arr = double(current);
    if numel(arr) < meta.linearIndex
        val = 0;
    else
        val = arr(meta.linearIndex);
    end
end

function structure = setValueAtPath(structure, meta, val)
    if isempty(meta.path)
        return;
    end
    fieldName = meta.path{1};
    if numel(meta.path) == 1
        if ~isfield(structure, fieldName) || ~isnumeric(structure.(fieldName))
            structure.(fieldName) = zeros(meta.size);
        end
        arr = structure.(fieldName);
        if isempty(arr) || ~isequal(size(arr), meta.size)
            arr = zeros(meta.size);
        end
        arr(meta.linearIndex) = val;
        structure.(fieldName) = arr;
        return;
    end
    if ~isfield(structure, fieldName) || ~isstruct(structure.(fieldName))
        structure.(fieldName) = struct();
    end
    subMeta = meta;
    subMeta.path = meta.path(2:end);
    structure.(fieldName) = setValueAtPath(structure.(fieldName), subMeta, val);
end

function [values, metadata] = flattenSpatialFeaturesRecursive(value, prefix)
    values = [];
    metadata = {};
    if isnumeric(value)
        flatVal = reshape(double(value), [], 1);
        for idx = 1:numel(flatVal)
            values(end+1, 1) = flatVal(idx); %#ok<AGROW>
            pathCells = prefix;
            metadata{end+1} = struct('path', {{pathCells}}, ...
                'size', size(value), 'linearIndex', idx); %#ok<AGROW>
        end
    elseif isstruct(value)
        fields = fieldnames(value);
        for i = 1:numel(fields)
            fieldName = fields{i};
            [subValues, subMeta] = flattenSpatialFeaturesRecursive(value.(fieldName), [prefix, {fieldName}]);
            values = [values; subValues]; %#ok<AGROW>
            metadata = [metadata, subMeta]; %#ok<AGROW>
        end
    end
end
...
function model = embedRepresentativeClusters(model, densityMap, optParams, morphologyFeatures)
    ...
    targetCount = [];
    if isfield(optParams, 'clusterGuidance') && ~isempty(optParams.clusterGuidance)
        targetCount = max(0, round(optParams.clusterGuidance.recommendedCount));
...
function model = enforcePorosityClusterTargets(model, optParams, densityMap)
    ...
    if isfield(optParams, 'clusterGuidance') && ~isempty(optParams.clusterGuidance)
        ...
        if CC.NumObjects > 0
            ...
            guidance = optParams.clusterGuidance;
            maxTolerance = max(guidance.recommendedMax * 1.1, guidance.maxSize * 1.15);
            ...
            minTolerance = max(3, floor(min(guidance.recommendedMin * 0.9, guidance.minSize * 1.1)));
            ...
            for i = 1:numel(undersized)
                if isfield(optParams, 'preserveSmallPores') && optParams.preserveSmallPores
                    grownIdx = growSmallClusterIndices(CC.PixelIdxList{undersized(i)}, ...
                        size(model), max(guidance.recommendedMin, minTolerance));
...
function E_cluster = calculateClusterEnergy(features, optParams)
    ...
    if ~isfield(optParams, 'originalFeatures') || ...
            ~isfield(optParams.originalFeatures, 'sizes') || ...
            isempty(optParams.originalFeatures.sizes)
        E_cluster = 0.5;
        return;
    end

    original = optParams.originalFeatures;
    guidance = optParams.clusterGuidance;
    currentSizes = double(features.sizes(:));
    origSizes = double(original.sizes(:));

    origMean = mean(origSizes);
    origStd = std(origSizes) + eps;
    currentMean = mean(currentSizes);
    currentStd = std(currentSizes) + eps;

    meanDiff = abs(currentMean - origMean) / (origMean + eps);
    stdDiff = abs(currentStd - origStd) / (origStd + eps);
    minDiff = abs(min(currentSizes) - guidance.minSize) / (guidance.minSize + 1);
    maxDiff = abs(max(currentSizes) - guidance.maxSize) / (guidance.maxSize + 1);

    % 分布差异（使用概率直方图）
    if isfield(guidance, 'histogramEdges') && numel(guidance.histogramEdges) > 2
        edges = guidance.histogramEdges;
        currentHist = histcounts(currentSizes, edges, 'Normalization', 'probability');
        % 对齐维度
        targetHist = guidance.histogram;
        minLength = min(numel(currentHist), numel(targetHist));
        distributionDiff = 0.5 * sum(abs(currentHist(1:minLength) - targetHist(1:minLength)));
    else
        distributionDiff = 0.5 * abs(std(currentSizes) - std(origSizes)) / (origStd + eps);
    end

    % 簇数量偏差（软约束）
    if guidance.recommendedCount > 0
        ratio = (features.numClusters + 1) / (guidance.recommendedCount + 1);
        countPenalty = abs(log(ratio));
    else
        countPenalty = 0;
    end
    ...
    if features.numClusters > 4 * max(1, guidance.recommendedCount)
        fragmentationPenalty = fragmentationPenalty + 1.5;
    end
    if features.sizeStats(2) > max(guidance.maxSize * 3, guidance.maxSize + 5 * origStd)
        fragmentationPenalty = fragmentationPenalty + 1.0;
    end

    E_cluster = 1.6 * meanDiff + 1.2 * stdDiff + 0.8 * distributionDiff + ...
        0.6 * minDiff + 0.6 * maxDiff + 0.9 * countPenalty + fragmentationPenalty;
end
...
function model = enforceClusterSizeConstraints(model, optParams)
    ...
    guidance = optParams.clusterGuidance;
    sizes = double(cellfun(@numel, CC.PixelIdxList));
    meanSize = max(guidance.meanSize, mean(sizes));
    stdSize = max(guidance.stdSize, std(sizes) + eps);

    largeThreshold = max(guidance.recommendedMax, meanSize + 3 * stdSize);
    removeThreshold = max(1, min(guidance.recommendedMin, meanSize - 2 * stdSize));
    growThreshold = max(removeThreshold + 1, round(0.5 * (meanSize + removeThreshold)));

    modified = false;

    % 缩减极大的簇，仅对远超阈值者操作
    largeClusters = find(sizes > largeThreshold);
    for idx = largeClusters(:)'
        if sizes(idx) > largeThreshold
    ...
    for i = 1:max(0, numel(smallClusters)-1)
        for j = i+1:numel(smallClusters)
...
function model = enforceTargetClusterCount(model, optParams)
    ...
    targetCount = max(1, round(optParams.clusterGuidance.recommendedCount));
    tolerance = max(3, round(0.4 * targetCount));
    ...
    if CC.NumObjects > targetCount + tolerance
        maxOperations = min(10, CC.NumObjects - targetCount - tolerance);
        for i = 1:maxOperations
    ...
    elseif CC.NumObjects < max(1, targetCount - tolerance)
        maxOperations = min(10, targetCount - CC.NumObjects);
        for i = 1:maxOperations
...
function model = structureSimplification(model, optParams)
    ...
    sizes = cellfun(@numel, CC.PixelIdxList);
    if isfield(optParams, 'clusterGuidance') && ~isempty(optParams.clusterGuidance)
        minKeep = max(1, optParams.clusterGuidance.recommendedMin);
    else
        minKeep = max(1, optParams.targetMin);
    end
    keepIdx = find(sizes >= minKeep);
...
function reportFinalStatistics(model, optParams)
    ...
    if isfield(optParams, 'clusterGuidance') && ~isempty(optParams.clusterGuidance)
        guidance = optParams.clusterGuidance;
        fprintf('  参考目标簇数: %d (允许 ±%d)\n', ...
            round(guidance.recommendedCount), ...
            max(3, round(0.4 * guidance.recommendedCount)));
        minThreshold = max(1, round(guidance.recommendedMin));
        maxThreshold = max(minThreshold + 1, round(guidance.recommendedMax));
    else
        minThreshold = max(1, optParams.targetMin);
        maxThreshold = max(minThreshold + 1, optParams.targetMax);
    end
    fprintf('  孔隙率: %.4f\n', mean(model(:)));
    fprintf('  最小孔隙: %d 体素\n', min(sizes));
    fprintf('  最大孔隙: %d 体素\n', max(sizes));
    ...
    % 分布统计
    smallCount = sum(sizes < minThreshold);
    mediumCount = sum(sizes >= minThreshold & sizes <= maxThreshold);
    largeCount = sum(sizes > maxThreshold);

    fprintf('\n孔隙大小分布：\n');
    fprintf('  小孔隙 (<%d): %d (%.1f%%)\n', minThreshold, smallCount, 100*smallCount/CC.NumObjects);
    fprintf('  中孔隙 (%d-%d): %d (%.1f%%)\n', minThreshold, maxThreshold, mediumCount, 100*mediumCount/CC.NumObjects);
    fprintf('  大孔隙 (>%d): %d (%.1f%%)\n', maxThreshold, largeCount, 100*largeCount/CC.NumObjects);
...
function checkComprehensiveModelQuality(finalModel, originalModel, optParams)
    ...
    if CC.NumObjects > 0
        ...
        if isfield(optParams, 'clusterGuidance') && ~isempty(optParams.clusterGuidance)
            guidance = optParams.clusterGuidance;
            tolerance = max(3, round(0.4 * guidance.recommendedCount));
            fprintf('  目标簇数量: %d ± %d\n', round(guidance.recommendedCount), tolerance);
            if abs(CC.NumObjects - guidance.recommendedCount) <= tolerance
                fprintf('  ✅ 簇数量处于允许范围内\n');
            else
                fprintf('  ⚠️ 簇数量与参考范围存在偏差\n');
            end
            minThreshold = max(1, round(guidance.recommendedMin));
            maxThreshold = max(minThreshold + 1, round(guidance.recommendedMax));
        else
        ...
        outOfRange = sum(sizes < minThreshold) + sum(sizes > maxThreshold);
...
function displayComprehensiveFinalResults(originalModel, finalModel, ...
    ...
    fprintf('  自适应目标簇数范围: [%d, %d]\n', ...
        max(1, round(optParams.clusterGuidance.recommendedCount - ...
        max(3, round(0.4 * optParams.clusterGuidance.recommendedCount)))), ...
        round(optParams.clusterGuidance.recommendedCount + ...
        max(3, round(0.4 * optParams.clusterGuidance.recommendedCount))));

    % 2. 梯度推断的目标空间特征
    fprintf('\n2. 目标空间特征（孔隙率梯度推断）：\n');
    targetSpatial = optParams.spatialFeatures;
    fprintf('  目标各向异性: %.4f\n', targetSpatial.anisotropy);
    if isfield(targetSpatial, 'surfaceToVolumeRatio')
        fprintf('  目标表面积/体积比: %.4f\n', targetSpatial.surfaceToVolumeRatio);
    end
    if isfield(targetSpatial, 'connectivity') && ...
            isfield(targetSpatial.connectivity, 'largestComponentRatio')
        fprintf('  目标最大连通簇占比: %.4f\n', ...
            targetSpatial.connectivity.largestComponentRatio);
    end
    if isfield(optParams, 'spatialGradientModel')
        fprintf('  梯度样本数量: %d | 孔隙率有效区间: [%.3f, %.3f]\n', ...
            size(optParams.spatialGradientModel.dataset, 1), ...
            optParams.spatialGradientModel.validRange(1), ...
            optParams.spatialGradientModel.validRange(2));
    end
    ...
    % 5. 簇分布与自适应评估
    fprintf('\n5. 簇分布与自适应评估：\n');
    fprintf('  最终簇大小范围: [%d, %d] | 均值=%.1f | 标准差=%.1f\n', ...
        finalFeatures.sizeStats(1), finalFeatures.sizeStats(2), ...
        finalFeatures.sizeStats(3), finalFeatures.sizeStats(4));
    fprintf('  原始簇大小范围: [%d, %d] | 均值=%.1f | 标准差=%.1f\n', ...
        originalFeatures.sizeStats(1), originalFeatures.sizeStats(2), ...
        originalFeatures.sizeStats(3), originalFeatures.sizeStats(4));
    ...
    % 7. 综合评分
    morphMatchOriginal = calculateMorphologyMatch(finalMorph, optParams.morphologyFeatures);
    spatialMatchTarget = calculateSpatialMatch(finalSpatial, optParams.spatialFeatures);
    overallScore = 0.5 * morphMatchOriginal + 0.5 * spatialMatchTarget;

    fprintf('\n7. 综合评分：\n');
    fprintf('  对原始形态特征的匹配度: %.3f\n', morphMatchOriginal);
    fprintf('  对新目标空间特征的匹配度: %.3f\n', spatialMatchTarget);
