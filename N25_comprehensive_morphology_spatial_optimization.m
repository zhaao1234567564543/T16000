...
function model = connectClusters(model, cluster1, cluster2)
    % 使用地质距离规划连接路径，避免穿墙
    if nargin < 3 || isempty(cluster2)
        if iscell(cluster1)
            if numel(cluster1) < 2
                error('connectClusters:InvalidInput', 'Cluster cell array must contain at least two elements.');
            end
            cluster2 = cluster1{2};
            cluster1 = cluster1{1};
        elseif isstruct(cluster1)
            if all(isfield(cluster1, {'clusterA', 'clusterB'}))
                cluster2 = cluster1.clusterB;
                cluster1 = cluster1.clusterA;
            else
                fields = fieldnames(cluster1);
                if numel(fields) >= 2
                    cluster2 = cluster1.(fields{2});
                    cluster1 = cluster1.(fields{1});
                else
                    error('connectClusters:InvalidInput', 'Structure input must contain at least two fields with cluster indices.');
                end
            end
        else
            error('connectClusters:InvalidInput', 'Provide either two explicit clusters or a cell/struct containing them.');
        end
    end
    volumeSize = size(model);
