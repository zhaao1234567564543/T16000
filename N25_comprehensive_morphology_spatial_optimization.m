function 
N25_comprehensive_morphology_spatial_optimization
% N25_comprehensive_morphology_spatial_optimization - 
综合形态和空间特征优化
% 
结合了形态一致性和空间匹配度的双重优化
...
saveRawModel(binVol, 
'originalDataModel.raw'
);
%% 2. 
用户设定目标参数
target_porosity = input(
'
请输入目标孔隙率（
0-1
之间）
...
target_cluster_count = input(
'
请输入目标孔隙簇数量
: '
);
if 
isempty(target_cluster_count) || ~isfinite(target_cluster_count)
    warning(
'
未提供有效的孔隙簇数量，默认使用原始模型的簇数量。
'
);
...
optParams.originalBinaryModel = binVol; 
% 
添加原始二值模型供后处理使用
optParams.preserveSmallPores = preserve_small_pores; 
% 
添加小孔隙保留标志
...
weights.structureCoherence = 6.0; 
% 
新增：结构一致性权重
% 
初始化
...
% 
形态保持阶段：增强形态权重
        currentWeights = adjustWeightsForPhase(weights, 
'morphology'
);
    
elseif 
iter <= spatial_phase_end
        
if 
strcmp(optimization_phase, 
'morphology'
)
            fprintf(
'\n
...
if 
idx <= length(performanceMonitor.acceptanceRatio)
...
保存结果
outFileName = sprintf(
'newModel_comprehensive_optimized_%dx%dx%d.raw'
, dims(1), dims(2), dims(3));
...
新增的模型调整函数
 ==========
function 
resizedModel = resizeModel(originalModel, originalDims, targetDims)
...
函数
 ==========
function 
vol = readRawModel(fileName, dims)
...
特征提取函数
 ==========
function 
features = extractEfficientClusterFeatures(binaryModel)
...
function 
spatialFeatures = computeEnhancedSpatialFeatures(binaryModel)
...
% 8.
弦长分布（Chord Length Distribution, CLD）
    spatialFeatures.chordLengthDistribution = computeChordLengthDistribution(binaryModel);


% 9.
孔隙大小分布（Pore Size Distribution, PSD）
    spatialFeatures.poreSizeDistribution = computePoreSizeDistribution(binaryModel);


% 10.
Minkowski 函数：积分平均曲率
    spatialFeatures.integralMeanCurvature = computeIntegralMeanCurvature(binaryModel);


% 11.
线性路径函数（Lineal Path Function）
    linealDistances = [1, 2, 3, 5, 7, 10, 15, 20];
    spatialFeatures.linealPath = computeLinealPathFunction(binaryModel, linealDistances);
end
function 
morphologyFeatures = computeDetailedMorphologyFeatures(binaryModel)
...
创建空的形态学特征结构
    ...
    morphologyFeatures.textureFeatures = struct(
'entropy'
, 0, 
'energy'
, 0, 
...
'homogeneity'
, 0);
    morphologyFeatures.skeletonFeatures = struct(
'density'
, 0, 
'numBranchPoints'
, 0, 
...
function 
multiScaleFeatures = computeMultiScaleSpatialFeatures(binaryModel)
...
特征计算辅助函数
 ==========
function 
tpc = computeFastTwoPointCorrelation(binaryModel, keyDistances)
...
function 
aniso = computeQuickAnisotropy(binaryModel)
...
function 
grad = computePorosityGradient(binaryModel)
...
function 
connectivity = computeConnectivityMetrics(binaryModel)
...
function 
svRatio = computeSurfaceToVolumeRatio(binaryModel)
...
function 
tortuosity = estimateTortuosity(binaryModel)
...
if 
binaryModel(startX, startY, 1) && binaryModel(startX, startY, nz)
...
function
autocorr = computeSpatialAutocorrelation(binaryModel)
...
function
cld = computeChordLengthDistribution(binaryModel)

%
计算弦长分布（Chord Length Distribution, CLD），分别沿
X/Y/Z
方向统计孔隙弦长
    directions = {'x', 'y', 'z'};
    maxDim = max(size(binaryModel));
    maxChord = max(1, min(100, round(maxDim)));
    edges = 0:maxChord;
    histograms = zeros(length(directions), length(edges)-1);
    meanLengths = zeros(1, length(directions));


for
d = 1:length(directions)
        lengths = computeChordLengthsAlongAxis(binaryModel, directions{d});
...
isempty(lengths)
        ...
        counts = histcounts(lengths, edges);


if
sum(counts) > 0
            histograms(d, :) = counts / sum(counts);

end
        meanLengths(d) = mean(lengths);

end
    cld = struct();
    cld.directions = directions;
    cld.edges = edges(2:end);
    cld.histograms = histograms;
    cld.meanChordLengths = meanLengths;
end
function lengths = computeChordLengthsAlongAxis(binaryModel, direction)
...
for z = 1:sz(3)
                line = squeeze(binaryModel(:, y, z));
                [values, runs] = runLengthEncode(line(:)');
                lengths = [lengths; runs(values == 1)']; %#ok<AGROW>
...
function
psd = computePoreSizeDistribution(binaryModel)

%
通过最大球法近似计算孔隙大小分布
    distanceMap = bwdist(~binaryModel);
    %
局部极大值代表最大内切球中心
    se = strel('sphere', 1);
    localMax = imdilate(distanceMap, se) == distanceMap & binaryModel;
    radii = distanceMap(localMax);


if
isempty(radii)
        radii = distanceMap(binaryModel);
...
maxRadius == 0
        ...
        edges = linspace(0, maxRadius, 15);

end

    counts = histcounts(radii, edges);
...
sum(counts) > 0
    ...
    psd = struct();
    psd.edges = edges(2:end);
    psd.histogram = histogram;
    psd.meanRadius = mean(radii);
    psd.medianRadius = median(radii);
    psd.maxRadius = maxRadius;
end
function
imc = computeIntegralMeanCurvature(binaryModel)

%
使用带符号距离场的散度近似积分平均曲率
    insideDist = bwdist(~binaryModel);
    outsideDist = bwdist(binaryModel);
    signedDist = insideDist - outsideDist;

    [gx, gy, gz] = gradient(signedDist);
    gradMag = sqrt(gx.^2 + gy.^2 + gz.^2) + eps;
    nx = gx ./ gradMag;
    ny = gy ./ gradMag;
    nz = gz ./ gradMag;
    ...
    [~, dny_dy, ~] = gradient(ny);
    [~, ~, dnz_dz] = gradient(nz);
    divergenceField = dnx_dx + dny_dy + dnz_dz;

    boundary = bwperim(binaryModel, 26);
    imc = 0.5 * sum(divergenceField(boundary));
end
function
lineal = computeLinealPathFunction(binaryModel, distances)
...
X/Y/Z
方向求平均
    validDistances = distances(distances >= 1 & distances <= max(size(binaryModel)));
...
isempty(validDistances)
    ...
    dirNames = {'x', 'y', 'z'};
    directionalValues = zeros(length(dirNames), length(validDistances));


for
d = 1:length(dirNames)
        directionalValues(d, :) = computeLinealPathAlongAxis(binaryModel, dirNames{d}, validDistances);

end

    lineal = struct();
    lineal.distances = validDistances;
    lineal.directionalValues = directionalValues;
    lineal.meanValues = mean(directionalValues, 1);
end
function
values = computeLinealPathAlongAxis(binaryModel, direction, distances)

%
沿指定轴向计算线性路径概率
    sz = size(binaryModel);
    values = zeros(1, length(distances));
...
'x'
        numLines = sz(2) * sz(3);
        iterator = @(idx) squeeze(binaryModel(:, idx(1), idx(2)));
        idxList = arrayfun(@(y, z) [y, z], repelem((1:sz(2))', sz(3), 1), repmat((1:sz(3))', sz(2), 1), 'UniformOutput', false);

case
'y'
        numLines = sz(1) * sz(3);
        iterator = @(idx) squeeze(binaryModel(idx(1), :, idx(2)));
        idxList = arrayfun(@(x, z) [x, z], repelem((1:sz(1))', sz(3), 1), repmat((1:sz(3))', sz(1), 1), 'UniformOutput', false);

case
'z'
        numLines = sz(1) * sz(2);
        iterator = @(idx) squeeze(binaryModel(idx(1), idx(2), :));
        idxList = arrayfun(@(x, y) [x, y], repelem((1:sz(1))', sz(2), 1), repmat((1:sz(2))', sz(1), 1), 'UniformOutput', false);
...
;
            ...
            totalSegments = totalSegments + (lineLength - r + 1);
            [valuesSeq, runLengths] = runLengthEncode(line(:)');
            poreRuns = runLengths(valuesSeq == 1);


if
~isempty(poreRuns)
                validSegments = validSegments + sum(max(poreRuns - r + 1, 0));
...
if
totalSegments > 0
            values(d) = validSegments / totalSegments;
...
function
[values, lengths] = runLengthEncode(sequence)
...
;
    ...
    diffSeq = [true, diff(sequence) ~= 0];
    idx = find(diffSeq);
    values = sequence(idx);
    idx = [idx, numel(sequence)+1];
    lengths = diff(idx);
end
function
density = computePoreNetworkDensity(model)
...
function 
avgCoordination = computeAverageCoordinationNumber(model)
...
function 
lacunarity = computeLacunarity(model)
...
function 
textureFeatures = computeTextureFeatures(model)
...
function 
skeletonFeatures = computeSkeletonFeatures(model)
...
if 
skeletonFeatures.numBranchPoints > 0
...
function 
distribution = computeClusterDistribution(model)
...
function 
spectrum = computeSpatialSpectrum(model)
...
function 
invariantFeatures = computeScaleInvariantFeatures(model)
...
for 
x = 1:boxSize:size(model, 1)
            
for 
y = 1:boxSize:size(model, 2)
                
for 
z = 1:boxSize:size(model, 3)
                    box = model(x:min(x+boxSize-1, end), 
...
                        y:min(y+boxSize-1, end), 
...
...
sum(counts > 0) > 1
        ...
        invariantFeatures.fractalDimension = 2.5; 
% 
默认值
...
% 
其他尺度不变特征（简化）
    invariantFeatures.voidScalingExponent = 1.5; 
% 
默认值
    invariantFeatures.connectivityScaling = 0.8; 
% 
默认值
end
%% ========== 
显示函数
 ==========
function 
displayClusterFeatures(features)
...
% 显示空间特征
    ...
    if isfield(spatialFeatures, 'integralMeanCurvature')
        fprintf('积分平均曲率: %.4f\n', spatialFeatures.integralMeanCurvature);
    end

    if isfield(spatialFeatures, 'chordLengthDistribution')
        cld = spatialFeatures.chordLengthDistribution;
        if isfield(cld, 'meanChordLengths') && ~isempty(cld.meanChordLengths)
            fprintf('平均弦长 (x/y/z): %.2f / %.2f / %.2f\n', cld.meanChordLengths(1), cld.meanChordLengths(2), cld.meanChordLengths(3));
        end
    end

    if isfield(spatialFeatures, 'poreSizeDistribution')
        psd = spatialFeatures.poreSizeDistribution;
        if isfield(psd, 'meanRadius')
            fprintf('平均孔隙半径: %.2f\n', psd.meanRadius);
        end
        if isfield(psd, 'medianRadius')
            fprintf('中位孔隙半径: %.2f\n', psd.medianRadius);
        end
        if isfield(psd, 'maxRadius')
            fprintf('最大孔隙半径: %.2f\n', psd.maxRadius);
        end
    end

    if isfield(spatialFeatures, 'linealPath')
        lp = spatialFeatures.linealPath;
        if isfield(lp, 'distances') && isfield(lp, 'meanValues')
            fprintf('线性路径函数 L(r) 均值:\n');
            maxShow = min(5, numel(lp.distances));
            for i = 1:maxShow
                fprintf('  L(%d) = %.4f\n', lp.distances(i), lp.meanValues(i));
            end
        end
    end
end
...
function 
displayMultiScaleSpatialFeatures(multiScaleFeatures)
...
if 
isfield(multiScaleFeatures.scale(s), 
'scaleFactor'
)
...
模型初始化函数
 ==========
function 
binVol = adaptiveBinarizeModel(vol, raw_porosity)
...
function 
lookupTables = generateComprehensiveLookupTables(binaryModel, 
...
...
if 
~isempty(originalFeatures.sizes)
...
function 
localPorosityMap = computeLocalPorosityMap(binaryModel, windowSize)
...
function 
boxSum = getBoxSum(integralImage, x1, y1, z1, x2, y2, z2)
...
function 
gradientField = computeSpatialGradientField(binaryModel)
...
function 
localAnisotropyMap = computeLocalAnisotropyMap(binaryModel)
...
for 
x = 1:stride:nx-windowSize+1
        
for 
y = 1:stride:ny-windowSize+1
            
for 
z = 1:stride:nz-windowSize+1
...
function 
curvatureMap = computeLocalCurvatureMap(binaryModel)
...
function 
thicknessMap = computeLocalThicknessMap(binaryModel)
...
function 
coherenceMap = computeStructureCoherenceMap(binaryModel)
...
function 
mcmcModel = generateComprehensiveInitialModel(dims, targetPorosity, 
...
...
if 
min([spatialMatch, morphMatch, multiScaleMatch]) > 0.92
...
function 
field = synthesizeDirectionalGaussianField(dims, correlationLength, anisotropy, 
...
...
if 
isfield(spatialFeatures, 
'porosityGradient'
) && ~isempty(spatialFeatures.porosityGradient)
...
if 
nargin >= 5 && isfield(optParams, 
'directionalPorosityProfile'
) && 
...
...
function 
densityMap = constructMultiScaleDensityMap(optParams, targetSize)
...
isfield(optParams, 
'referenceDensityMap'
) && ~isempty(optParams.referenceDensityMap)
        reference = optParams.referenceDensityMap;
    
elseif 
isfield(optParams, 
'originalBinaryModel'
) && ~isempty(optParams.originalBinaryModel)
...
if 
isfield(optParams, 
'multiScaleSpatialFeatures'
) && 
...
            isfield(optParams.multiScaleSpatialFeatures, 
'scale'
)
...
function 
blendedField = blendReferenceDirectionalPatterns(baseField, densityMap)
...
function 
threshold = computeAdaptiveThreshold(field, targetPorosity, optParams)
...
'x'
            ...
            variations(end+1) = std(profile.x); 
%#ok<AGROW>
            gradients(end+1) = max(abs(diff(profile.x))); 
%#ok<AGROW>
...
'y'
            ...
            variations(end+1) = std(profile.y); 
%#ok<AGROW>
            gradients(end+1) = max(abs(diff(profile.y))); 
%#ok<AGROW>
...
'z'
            ...
            variations(end+1) = std(profile.z); 
%#ok<AGROW>
            gradients(end+1) = max(abs(diff(profile.z))); 
%#ok<AGROW>
...
function 
match = calculateMultiScaleMatch(currentMultiScale, targetMultiScale)
    
% 
计算多尺度特征匹配度
    
if 
nargin < 2 || isempty(currentMultiScale) || isempty(targetMultiScale)
        match = 0.5;
...
if 
isfield(currentMultiScale, 
'scale'
) && isfield(targetMultiScale, 
'scale'
)
...
...
                    isfield(cScale.clusterDistribution, 
'density'
) && 
...
                    isfield(tScale, 
...
function 
model = applyMultiScaleCorrection(model, currentMultiScale, targetMultiScale)
    
% 
针对多尺度差异进行纠正
    
if 
nargin < 3 || isempty(currentMultiScale) || isempty(targetMultiScale)
...
isfield(optParams, 
'clusterLibrary'
) && ~isempty(optParams.clusterLibrary)
...
isfield(optParams, 
'targetClusterCount'
) && ~isempty(optParams.targetClusterCount)
...
function 
model = placeClusterSample(model, clusterSample, densityMap)
    
% 
将单个代表簇放置到模型中
    
if 
~isstruct(clusterSample) || ~isfield(clusterSample, 
'patch'
) || isempty(clusterSample.patch)
...
if 
nargin < 2 || isempty(originalFeatures) || ~isfield(originalFeatures, 
'sizes'
)
...
function 
histVals = computeTargetPoreHistogram(clusterSizes)
    
% 
计算归一化的孔隙尺寸直方图
    
if 
isempty(clusterSizes)
        histVals = zeros(6, 1);
...
;
    ...
    histVals = histcounts(normalized, edges, 
'Normalization'
, 
'probability'
);
...
function 
profile = computeDirectionalPorosityProfile(model)
...
function 
densityMap = constructReferenceDensityMap(referenceModel, multiScaleSpatialFeatures)
...
if 
nargin < 1 || isempty(referenceModel)
...
if 
nargin > 1 && ~isempty(multiScaleSpatialFeatures) && 
...
            isfield(multiScaleSpatialFeatures, 
'scale'
)
...
for 
s = 1:length(multiScaleSpatialFeatures.scale)
...
function 
resized = resizeVolume(volume, targetSize)
    
% 
使用线性插值调整体数据大小
    
if 
nargin < 2 || isempty(targetSize)
        resized = volume;
...
;
    ...
    F = griddedInterpolant({1:nx, 1:ny, 1:nz}, double(volume), 
'linear'
, 
'nearest'
);
...
function 
densityMap = applyDirectionalProfile(densityMap, profile)
    
% 
将方向性孔隙率分布施加到密度图上
    
if 
isempty(densityMap) || nargin < 2 || isempty(profile)
...
function 
clusters = sampleRepresentativeClusters(referenceModel, morphologyFeatures)
...
if 
nargin > 1 && ~isempty(morphologyFeatures) && isfield(morphologyFeatures, 
'sphericity'
)
...
try
                    props = regionprops3(patch, 
'PrincipalAxisLength'
);
                    
if 
~isempty(props)
                        axes = sort(props.PrincipalAxisLength, 
'descend'
);
...
function 
corrLength = estimateCorrelationLength(twoPointCorr)
...
function 
filteredField = anisotropicFilter3D(field, anisotropy)
...
for 
i = 1:size(field, 3)
        filteredField(:,:,i) = conv2(filteredField(:,:,i), hx*hy', 
'same'
);
...
for 
j = 1:size(field, 2)
            filteredField(i,j,:) = conv(squeeze(filteredField(i,j,:)), hz, 
'same'
);
...
if 
~isempty(morphologyFeatures.sphericity)
        targetSphericity = mean(morphologyFeatures.sphericity);
        
        
if 
targetSphericity > 0.7
...
function 
model = reduceOversizedCluster(model, clusterIdx, optParams)
...
function 
model = growSmallCluster(model, clusterIdx)
...
if 
isfield(currentSpatial, 
'twoPointCorr'
) && isfield(targetSpatial, 
'twoPointCorr'
)
...
if 
abs(currentSpher - targetSpher) > 0.1
            
if 
targetSpher > currentSpher
...
if 
abs(currentElong - targetElong) > 0.5
            
if 
targetElong > currentElong
...
function 
model = refineInitialModelWithSpatialMCMC(model, targetSpatial, targetMorph, targetPorosity, optParams)
    
% 
使用
MCMC
在初始化阶段细化模型，使空间特征与原始模型更匹配
    maxIterations = 200;
    temperature = 0.5;
    coolingRate = 0.97;
    bestModel = model;
...
if 
deltaE < 0 || rand() < exp(-deltaE / max(temperature, eps))
...
function 
energy = computeInitialMCMCEnergy(model, targetSpatial, targetMorph, targetPorosity)
...
if 
sqrt(double(dx)^2 + double(dy)^2 + double(dz)^2) <= radius + 0.01
                        localIdx(end+1, 1) = sub2ind(size(model), x, y, z); 
%#ok<AGROW>
...
if 
isempty(poreIdx) && isempty(matrixIdx)
...
~isempty(matrixIdx)
        ...
        move.linearIdx = [move.linearIdx; addIdx(:)]; 
%#ok<AGROW>
        move.oldValues = [move.oldValues; false(numel(addIdx), 1)]; 
%#ok<AGROW>
        move.newValues = [move.newValues; true(numel(addIdx), 1)]; 
%#ok<AGROW>
...
~isempty(poreIdx)
        ...
        move.linearIdx = [move.linearIdx; removeIdx(:)]; 
%#ok<AGROW>
        move.oldValues = [move.oldValues; true(numel(removeIdx), 1)]; 
%#ok<AGROW>
        move.newValues = [move.newValues; false(numel(removeIdx), 1)]; 
%#ok<AGROW>
...
if 
~isempty(move.linearIdx)
        [move.linearIdx, uniqueIdx] = unique(move.linearIdx, 
'stable'
);
...
function 
newModel = applyClusterNeighborhoodMove(model, move)
...
状态和控制函数
 ==========
function 
mcmcState = initializeComprehensiveMCMCState(model, optParams, weights)
...
初始化综合性能监控器
    ...
    performanceMonitor.clusterSizeHistory = zeros(3, floor(maxIterations/100)); 
% min, max, mean
    performanceMonitor.phaseHistory = cell(1, floor(maxIterations/100));
end
function 
currentWeights = adjustWeightsForPhase(baseWeights, phase)
...
能量计算函数
 ==========
function 
energy = calculateComprehensiveEnergy(mcmcState)
...
isfield(optParams, 
'targetClusterCount'
) && optParams.targetClusterCount > 0
...
if 
~isempty(morphologyFeatures.elongation) && ~isempty(targetMorph.elongation)
...
if 
~isempty(morphologyFeatures.sphericity) && ~isempty(targetMorph.sphericity)
...
% 计算空间能量
    ...
    if isfield(spatialFeatures, 'twoPointCorr') && isfield(targetSpatial, 'twoPointCorr')
        minSize = min(size(spatialFeatures.twoPointCorr, 1), size(targetSpatial.twoPointCorr, 1));
        if minSize > 0
            curTPC = spatialFeatures.twoPointCorr(1:minSize, :);
            tarTPC = targetSpatial.twoPointCorr(1:minSize, :);
            tpcDiff = mean(abs(curTPC(:) - tarTPC(:)));
    ...
    % 积分平均曲率
    if isfield(spatialFeatures, 'integralMeanCurvature') && isfield(targetSpatial, 'integralMeanCurvature')
        denom = abs(targetSpatial.integralMeanCurvature) + 0.1;
        E_spatial = E_spatial + abs(spatialFeatures.integralMeanCurvature - targetSpatial.integralMeanCurvature) / denom;
        nTerms = nTerms + 1;
    end

    % 弦长分布
    if isfield(spatialFeatures, 'chordLengthDistribution') && isfield(targetSpatial, 'chordLengthDistribution')
        curCLD = spatialFeatures.chordLengthDistribution;
        tarCLD = targetSpatial.chordLengthDistribution;
        if isfield(curCLD, 'histograms') && isfield(tarCLD, 'histograms') && ~isempty(curCLD.histograms) && ~isempty(tarCLD.histograms)
            minBins = min(size(curCLD.histograms, 2), size(tarCLD.histograms, 2));
            if minBins > 0
                cldDiff = mean(abs(curCLD.histograms(:,1:minBins) - tarCLD.histograms(:,1:minBins)), 'all');
                E_spatial = E_spatial + cldDiff;
                nTerms = nTerms + 1;
            end
        end
    end
    ...
    % 孔隙大小分布
    if isfield(spatialFeatures, 'poreSizeDistribution') && isfield(targetSpatial, 'poreSizeDistribution')
        curPSD = spatialFeatures.poreSizeDistribution;
        tarPSD = targetSpatial.poreSizeDistribution;
        if isfield(curPSD, 'histogram') && isfield(tarPSD, 'histogram') && ~isempty(curPSD.histogram) && ~isempty(tarPSD.histogram)
            minBins = min(numel(curPSD.histogram), numel(tarPSD.histogram));
            if minBins > 0
                psdDiff = mean(abs(curPSD.histogram(1:minBins) - tarPSD.histogram(1:minBins)));
                E_spatial = E_spatial + psdDiff;
                nTerms = nTerms + 1;
            end
        end
    end
    ...
    % 线性路径函数
    if isfield(spatialFeatures, 'linealPath') && isfield(targetSpatial, 'linealPath')
        curLP = spatialFeatures.linealPath;
        tarLP = targetSpatial.linealPath;
        if isfield(curLP, 'distances') && isfield(curLP, 'meanValues') && isfield(tarLP, 'distances') && isfield(tarLP, 'meanValues')
            [common, ia, ib] = intersect(curLP.distances(:)', tarLP.distances(:)');
            if ~isempty(common)
                lpDiff = mean(abs(curLP.meanValues(ia) - tarLP.meanValues(ib)));
                E_spatial = E_spatial + lpDiff;
                nTerms = nTerms + 1;
            end
        end
    end
...
if 
isfield(spatialFeatures.connectivity, 
...
function 
E_multiScale = calculateMultiScaleEnergy(multiScaleFeatures, optParams)
...
...
            isfield(targetMultiScale.scale(s), 
'twoPointCorr'
)
...
function 
E_shape = calculateShapePreservationEnergy(features, morphologyFeatures, optParams)
...
if 
~isempty(features.sizes) && ~isempty(optParams.originalFeatures.sizes)
...
~isempty(features.compactness) && 
...
        isfield(optParams.originalFeatures, 
'compactness'
) && 
...
...
function 
E_structure = calculateStructureCoherenceEnergy(mcmcState)
...
function 
coherence = computeLocalCoherence(window)
...
function 
mcmcState = enforceComprehensiveConstraints(mcmcState, optParams, phase)
...
if 
~isempty(targetMorph.sphericity) && ~isempty(currentMorph.sphericity)
        targetSpher = mean(targetMorph.sphericity);
        currentSpher = mean(currentMorph.sphericity);
        
        
if 
abs(currentSpher - targetSpher) > 0.2
...
if 
abs(currentSpatial.anisotropy - targetSpatial.anisotropy) > 0.1
...
if 
currentSpatial.connectivity.largestComponentRatio < 
...
...
if 
morphMatch < 0.5 && spatialMatch < 0.5
...
if 
sizes(idx) > optParams.targetMax * 2
            model = reduceOversizedCluster(model, CC.PixelIdxList{idx}, optParams);
            modified = true;
...
if 
length(smallClusters) >= 2
        
% 
找到接近的小簇对
        
for 
i = 1:length(smallClusters)-1
            
for 
j = i+1:length(smallClusters)
...
while 
CC.NumObjects > targetCount && iter < maxIterations
...
if 
CC.NumObjects ~= targetCount
        tolerance = abs(CC.NumObjects - targetCount);
        
if 
CC.NumObjects > targetCount && tolerance > 0
...
if 
all(pos >= 1) && pos(1) <= dims(1) && pos(2) <= dims(2) && pos(3) <= dims(3)
                    
if 
sqrt(double(dx)^2 + double(dy)^2 + double(dz)^2) <= radius
...
function 
model = enhanceConnectivity(model)
...
function 
model = adjustModelAnisotropy(model, currentAniso, targetAniso)
...
function 
model = comprehensiveModelAdjustment(model, optParams)
...
function 
localStructure = createTargetStructure(size, optParams)
...
function 
structure = createSphericalStructure(size)
...
function 
structure = createElongatedStructure(size, anisotropy)
...
if 
((i-size/2)^2/(size/2)^2 + (j-size/2)^2/(size/3)^2 + 
...
...
function 
structure = createModerateStructure(size)
...
if 
nx >= 1 && nx <= size && ny >= 1 && ny <= size && 
...
                            nz >= 1 && nz <= size/2
                            
if 
sqrt(dx^2 + dy^2 + dz^2) <= 2
...
function 
model = blendStructureIntoModel(model, structure, cx, cy, cz)
...
function 
model = connectClusters(model, cluster1, cluster2)
...
for 
i = 1:min(50, length(x1))
        
for 
j = 1:min(50, length(x2))
...
function 
[dist, point1, point2] = findClosestPoints(model, cluster1Idx, cluster2Idx)
...
if 
isempty(cluster1Idx) || isempty(cluster2Idx)
...
移动策略和生成函数
 ==========
function 
strategy = selectComprehensiveAdaptiveMoveStrategy(iter, maxIter, mcmcState, phase)
...
移动策略和生成函数（续）
 ==========
function 
[moves, moveTypes] = generateComprehensiveBatchMoves(model, strategy, 
...
...
具体移动生成函数
 ==========
function 
move = generateLocalMove(model, radius)
...
if 
any(boundary(:))
        neighborhood = imdilate(boundary, strel(
'sphere'
, max(1, radius)));
...
if 
targetLocalPorosity > localPorosity && ~isempty(matrixIdx)
        addCount = min(numel(matrixIdx), addCount + 1);
    
elseif 
targetLocalPorosity < localPorosity && ~isempty(poreIdx)
...
if 
targetSpher > 0.7 && localCurvature < 0
...
elseif 
targetSpher < 0.4 && localCurvature > 0
            
% 
低球形度目标，增加不规则性
            
if 
model(selectedIdx(i)) && rand() < 0.3
                move.linearIdx = [move.linearIdx; selectedIdx(i)];
...
function 
curvature = estimateLocalCurvature(model, x, y, z)
...
if 
model(nx, ny, nz) ~= model(x(i), y(i), z(i))
                            changes = changes + 1;
...
if 
all(newPoint >= 1) && newPoint(1) <= size(model,1) && 
...
...
结构元素
    ...
    morphGrad = dilatedMask & ~clusterMask; 
% 
形态学梯度（外边界）
...
if 
isfield(optParams.spatialFeatures, 
'keyDistances'
)
...
for 
i = 1:min(10, length(growthPoints))
...
for 
z = max(1, cz-3):min(size(model,3), cz+3)
...
if 
model(idx1) ~= model(idx2) && rand() < 0.5
...
找到较大的非连通组分
    sizes = cellfun(@numel, CC.PixelIdxList);
    [sortedSizes, sortIdx] = sort(sizes, 
'descend'
);
...
if 
CC.NumObjects >= 2 && sortedSizes(2) > optParams.targetMin
...
elseif 
abs(i) > radius/2 && model(idx) && rand() < 0.3
...
for 
x = cx-regionSize:cx+regionSize
        
for 
y = cy-regionSize:cy+regionSize
            
for 
z = cz-regionSize/2:cz+regionSize/2
...
function 
deltaE = evaluateBaseMoveDelta(mcmcState, move)
...
if 
move.newValues(i)
                surfaceChangeEstimate = surfaceChangeEstimate - 1; 
% 
填充边界减少表面
...
if 
nx >= 1 && nx <= size(mcmcState.model, 1)
...
if 
ny >= 1 && ny <= size(mcmcState.model, 2)
...
if 
nz >= 1 && nz <= size(mcmcState.model, 3)
...
if 
move.newValues(i) && neighborCount < 6
                surfaceChangeEstimate = surfaceChangeEstimate + 0.5;
            
elseif 
~move.newValues(i) && neighborCount > 0
                surfaceChangeEstimate = surfaceChangeEstimate + 0.5;
...
if 
isfield(mcmcState.acceptedMoveTypes, bestType)
...
if 
~isempty(mcmcState.features.sizes)
...
if 
isfield(mcmcState.spatialFeatures, 
'connectivity'
)
...
if 
std(mcmcState.energyTrend) < 1e-6
        fprintf(
'  
检测到能量停滞，进行自适应调整
...\n'
...
function 
model = morphologyGuidedPerturbation(model, optParams)
...
if 
~isempty(targetMorph.sphericity)
        targetSpher = mean(targetMorph.sphericity);
...
function 
model = largeSpatialPerturbation(model, optParams)
...
function 
model = balancedPerturbation(model, optParams)
...
if 
std(recentMorphMatch) < 0.01 && mean(recentMorphMatch) < 0.6
                fprintf(
'  
形态匹配进展缓慢，考虑调整策略
...\n'
...
if 
std(recentSpatialMatch) < 0.01 && mean(recentSpatialMatch) < 0.6
                fprintf(
'  
空间匹配进展缓慢，考虑调整策略
...\n'
...
if 
morphProgress < 0.7 || spatialProgress < 0.7
                fprintf(
'  
综合匹配未达预期
 (
...
进度显示和可视化
 ==========
function 
printComprehensiveProgress(iter, mcmcState, elapsedTime, phase)
...
function 
visualizeComprehensiveModelSlices(model, iter, phase)
...
改进的后处理函数
 ==========
function 
finalModel = comprehensivePostProcessWithSmallPores(model, optParams)
...
function 
finalModel = comprehensivePostProcess(model, optParams)
...
function 
model = referenceBasedMorphologyRestoration(model, referenceModel)
...
if 
sum(localDiff(:)) > numel(localDiff) * 0.3
...
function 
matched = matchLocalMorphology(current, reference)
...
if 
abs(refSolidity - currentSolidity) > 0.1
        
% 
需要调整
        
if 
refSolidity > currentSolidity
...
for 
i = 1:length(mediumPoresIdx)
            
if 
voxelsAdded >= nVoxelsToAdd
...
if 
abs(finalPorosity - targetPorosity) > 0.001
...
质量检查和结果显示
 ==========
function 
checkComprehensiveModelQuality(finalModel, originalModel, optParams)
...
if 
~isempty(origMorph.sphericity) && ~isempty(finalMorph.sphericity)
...
if 
CC.NumObjects == round(optParams.targetClusterCount)
                fprintf(
'  
✅
...
function 
checkStructuralIntegrity(model)
...
function 
displayComprehensiveFinalResults(originalModel, finalModel, 
...
...
if 
finalFeatures.numClusters == round(optParams.targetClusterCount)
            fprintf(
'  
✅
...
if 
~isempty(originalMorph.sphericity) && ~isempty(finalMorph.sphericity)
        fprintf(
'  
球形度
: 
...
匹配度历史
        subplot(2,3,5);
        plot(performanceMonitor.morphologyMatchHistory, 
'b-'
, 
'LineWidth'
, 2);
...
on
;
        plot(performanceMonitor.spatialMatchHistory, 
'r-'
, 
'LineWidth'
, 2);
...
簇大小演化
        subplot(2,3,6);
        plot(performanceMonitor.clusterSizeHistory(1,:), 
'g-'
, 
'LineWidth'
, 2);
...
on
;
        plot(performanceMonitor.clusterSizeHistory(2,:), 
'r-'
, 
'LineWidth'
, 2);
        plot(performanceMonitor.clusterSizeHistory(3,:), 
'b-'
, 
'LineWidth'
, 2);
...
辅助计算函数
 ==========
function
match = calculateSpatialMatch(currentFeatures, targetFeatures)
...
% 计算空间特征匹配度
    ...
    if isfield(currentFeatures, 'anisotropy') && isfield(targetFeatures, 'anisotropy')
        diff = abs(currentFeatures.anisotropy - targetFeatures.anisotropy) / (targetFeatures.anisotropy + 0.1);
    ...
    if isfield(currentFeatures, 'twoPointCorr') && isfield(targetFeatures, 'twoPointCorr')
        minSize = min(size(currentFeatures.twoPointCorr, 1), size(targetFeatures.twoPointCorr, 1));
        if minSize > 0
    ...
    % 空间自相关匹配
    if isfield(currentFeatures, 'spatialAutocorrelation') && isfield(targetFeatures, 'spatialAutocorrelation')
        diff = abs(currentFeatures.spatialAutocorrelation - targetFeatures.spatialAutocorrelation);
        match = match + (1 - min(diff, 1));
        nTerms = nTerms + 1;
    end
    ...
    if isfield(currentFeatures, 'connectivity') && isfield(targetFeatures, 'connectivity')
        if isfield(currentFeatures.connectivity, 'largestComponentRatio') && isfield(targetFeatures.connectivity, 'largestComponentRatio')
            diff = abs(currentFeatures.connectivity.largestComponentRatio - targetFeatures.connectivity.largestComponentRatio);
            match = match + (1 - min(diff, 1));
            nTerms = nTerms + 1;
        end
    end
    ...
    % 积分平均曲率匹配
    if isfield(currentFeatures, 'integralMeanCurvature') && isfield(targetFeatures, 'integralMeanCurvature')
        targetIMC = targetFeatures.integralMeanCurvature;
        denom = abs(targetIMC) + 0.1;
        diff = abs(currentFeatures.integralMeanCurvature - targetIMC) / denom;
        match = match + (1 - min(diff, 1));
        nTerms = nTerms + 1;
    end

    % 弦长分布匹配
    if isfield(currentFeatures, 'chordLengthDistribution') && isfield(targetFeatures, 'chordLengthDistribution')
        curCLD = currentFeatures.chordLengthDistribution;
        tarCLD = targetFeatures.chordLengthDistribution;
        if isfield(curCLD, 'histograms') && isfield(tarCLD, 'histograms') && ~isempty(curCLD.histograms) && ~isempty(tarCLD.histograms)
            minBins = min(size(curCLD.histograms, 2), size(tarCLD.histograms, 2));
            if minBins > 0
                diff = mean(abs(curCLD.histograms(:,1:minBins) - tarCLD.histograms(:,1:minBins)), 'all');
                match = match + (1 - min(diff, 1));
                nTerms = nTerms + 1;
            end
        end
    ...
    % 孔隙大小分布匹配
    if isfield(currentFeatures, 'poreSizeDistribution') && isfield(targetFeatures, 'poreSizeDistribution')
        curPSD = currentFeatures.poreSizeDistribution;
        tarPSD = targetFeatures.poreSizeDistribution;
        if isfield(curPSD, 'histogram') && isfield(tarPSD, 'histogram') && ~isempty(curPSD.histogram) && ~isempty(tarPSD.histogram)
            minBins = min(numel(curPSD.histogram), numel(tarPSD.histogram));
            if minBins > 0
                diff = mean(abs(curPSD.histogram(1:minBins) - tarPSD.histogram(1:minBins)));
                match = match + (1 - min(diff, 1));
                nTerms = nTerms + 1;
            end
        end
    ...
    % 线性路径函数匹配
    if isfield(currentFeatures, 'linealPath') && isfield(targetFeatures, 'linealPath')
        curLP = currentFeatures.linealPath;
        tarLP = targetFeatures.linealPath;
        if isfield(curLP, 'distances') && isfield(curLP, 'meanValues') && isfield(tarLP, 'distances') && isfield(tarLP, 'meanValues')
            [common, ia, ib] = intersect(curLP.distances(:)', tarLP.distances(:)');
            if ~isempty(common)
                diff = mean(abs(curLP.meanValues(ia) - tarLP.meanValues(ib)));
                match = match + (1 - min(diff, 1));
                nTerms = nTerms + 1;
            end
        end
...
function 
match = calculateMorphologyMatch(currentMorph, targetMorph)
