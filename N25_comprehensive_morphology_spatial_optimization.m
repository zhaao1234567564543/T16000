...
if isempty(target_porosity) || ~isfinite(target_porosity) || target_porosity <= 0 || target_porosity >= 1
    warning('未提供有效的孔隙率，默认使用原始模型孔隙率。');
    target_porosity = raw_porosity;
end
adaptiveTargets = initializeAdaptiveClusterTargets(originalFeatures, target_porosity, dims, raw_porosity);
fprintf('
自动探索簇目标初始值：最小=%d，最大=%d，预估簇数=%d\n', ...
    adaptiveTargets.currentMin, adaptiveTargets.currentMax, adaptiveTargets.currentCount);
% 询问是否保留小孔隙特征
fprintf('\n小孔隙保留选项：\n');
...
optParams.targetMax = adaptiveTargets.currentMax;
optParams.targetMin = adaptiveTargets.currentMin;
optParams.targetClusterCount = adaptiveTargets.currentCount;
optParams.clusterTargetState = adaptiveTargets;
...
for iter = 1:maxIterations
    ...
    if mod(iter, adaptiveTargets.updateInterval) == 0
        [updatedOptParams, adaptiveTargets] = updateAdaptiveClusterTargets(mcmcState, adaptiveTargets, iter, maxIterations);
        optParams = updatedOptParams;
        mcmcState.optParams = updatedOptParams;
    end
    % 自适应调整
    if mod(iter, 200) == 0
...
function controller = initializeAdaptiveClusterTargets(originalFeatures, targetPorosity, modelDims, rawPorosity)
    % 初始化自适应簇目标控制器
    if nargin < 4 || isempty(rawPorosity) || ~isfinite(rawPorosity)
        rawPorosity = targetPorosity;
    end
    if nargin < 3 || isempty(modelDims)
        error('缺少模型尺寸信息用于自适应簇控制');
    end
    controller = struct();
    controller.totalVoxels = prod(modelDims);
    controller.targetPorosity = max(eps, min(1, targetPorosity));
    sizes = [];
    if isstruct(originalFeatures)
        if isfield(originalFeatures, 'sizes') && ~isempty(originalFeatures.sizes)
            sizes = double(originalFeatures.sizes(:));
        end
    end
    if isempty(sizes)
        baseMin = max(1, round(controller.totalVoxels * controller.targetPorosity * 0.001));
        baseMax = max(baseMin + 5, round(controller.totalVoxels * controller.targetPorosity * 0.01));
        baseMean = max(baseMin, (baseMin + baseMax) / 2);
        baseCount = max(1, round(controller.totalVoxels * controller.targetPorosity / max(baseMean, 1)));
        refCount = baseCount;
    else
        refMin = max(1, round(quantile(sizes, 0.2)));
        refMax = max(refMin + 1, round(quantile(sizes, 0.9)));
        refMean = max(1, mean(sizes));
        porosityScale = controller.targetPorosity / max(rawPorosity, eps);
        scaledMin = max(1, round(refMin * max(0.5, sqrt(porosityScale))));
        scaledMax = max(scaledMin + 1, round(refMax * min(1.8, sqrt(max(porosityScale, eps)) + 0.5)));
        desiredVolume = controller.totalVoxels * controller.targetPorosity;
        countFromVolume = desiredVolume / max(refMean, 1);
        if isfield(originalFeatures, 'numClusters') && ~isempty(originalFeatures.numClusters)
            refCount = max(1, double(originalFeatures.numClusters));
        else
            refCount = max(1, numel(sizes));
        end
        scaledCount = max(1, refCount * porosityScale);
        baseCount = max(1, round(0.5 * countFromVolume + 0.5 * scaledCount));
        baseMin = scaledMin;
        baseMax = scaledMax;
        baseMean = max(1, 0.5 * refMean * (porosityScale^0.2 + 1));
    end
    controller.currentMin = max(1, round(baseMin));
    controller.currentMax = max(controller.currentMin + 1, round(baseMax));
    controller.currentCount = max(1, round(baseCount));
    controller.referenceMin = controller.currentMin;
    controller.referenceMax = controller.currentMax;
    controller.referenceMean = max(1, round(baseMean));
    if exist('refCount', 'var') && ~isempty(refCount)
        controller.referenceCount = max(1, round(refCount));
    else
        controller.referenceCount = controller.currentCount;
    end
    controller.updateInterval = 75;
    controller.smoothingFactor = 0.65;
    controller.acceptanceLow = 0.15;
    controller.acceptanceHigh = 0.45;
    controller.explorationScale = max(5, round(controller.referenceMax * 0.15));
    controller.minLimit = max(1, round(controller.referenceMin * 0.5));
    controller.maxLimit = max(controller.referenceMax + 10, round(controller.referenceMax * 2.5));
    controller.history = struct();
    controller.history.iterations = 0;
    controller.history.targetMin = controller.currentMin;
    controller.history.targetMax = controller.currentMax;
    controller.history.targetCount = controller.currentCount;
    controller.history.morphology = [];
    controller.history.spatial = [];
    controller.history.acceptance = [];
end
function [optParams, controller] = updateAdaptiveClusterTargets(mcmcState, controller, iter, maxIterations)
    % 自适应更新簇目标，探索最优簇大小和数量
    optParams = mcmcState.optParams;
    if ~isstruct(controller) || ~isfield(controller, 'currentMin')
        optParams.clusterTargetState = controller;
        return;
    end
    if ~isfield(mcmcState, 'features') || isempty(mcmcState.features) || ...
            ~isfield(mcmcState.features, 'numClusters') || mcmcState.features.numClusters == 0
        optParams.clusterTargetState = controller;
        return;
    end
    features = mcmcState.features;
    if ~isfield(features, 'sizes') || isempty(features.sizes)
        optParams.clusterTargetState = controller;
        return;
    end
    ...
    morphMatch = 0;
    spatialMatch = 0;
    if isfield(mcmcState, 'currentMorphMatch') && ~isempty(mcmcState.currentMorphMatch)
        morphMatch = mcmcState.currentMorphMatch;
    end
    if isfield(mcmcState, 'currentSpatialMatch') && ~isempty(mcmcState.currentSpatialMatch)
        spatialMatch = mcmcState.currentSpatialMatch;
    end
    acceptance = 0;
    if isfield(mcmcState, 'acceptanceRate') && ~isempty(mcmcState.acceptanceRate)
        acceptance = mcmcState.acceptanceRate;
    end
    controller.history.iterations(end+1) = iter; %#ok<AGROW>
    controller.history.morphology(end+1) = morphMatch; %#ok<AGROW>
    controller.history.spatial(end+1) = spatialMatch; %#ok<AGROW>
    controller.history.acceptance(end+1) = acceptance; %#ok<AGROW>
    stats = features.sizeStats;
    currentMinObs = max(1, stats(1));
    currentMaxObs = max(currentMinObs + 1, stats(2));
    currentMeanObs = max(1, stats(3));
    observedCount = max(1, features.numClusters);
    matchScore = max(0, min(1, (morphMatch + spatialMatch) / 2));
    progress = max(0, min(1, iter / maxIterations));
    blend = min(0.9, 0.35 + 0.55 * matchScore);
    targetMinCandidate = (1 - blend) * controller.referenceMin + blend * currentMinObs;
    targetMaxCandidate = (1 - blend) * controller.referenceMax + blend * currentMaxObs;
    explorationWidth = controller.explorationScale * max(0.1, 1 - matchScore);
    targetMinCandidate = max(controller.minLimit, targetMinCandidate - explorationWidth);
    targetMaxCandidate = min(controller.maxLimit, targetMaxCandidate + explorationWidth);
    if acceptance < controller.acceptanceLow
        targetMinCandidate = max(controller.minLimit, targetMinCandidate * 0.9);
        targetMaxCandidate = min(controller.maxLimit, targetMaxCandidate * 1.15);
    elseif acceptance > controller.acceptanceHigh
        tighten = 0.92;
        targetMinCandidate = max(controller.minLimit, targetMinCandidate * tighten);
        targetMaxCandidate = max(targetMinCandidate + 1, targetMaxCandidate * tighten);
    end
    adaptStrength = max(0.05, 0.45 * (1 - matchScore)) * (1 - 0.5 * progress);
    controller.currentMin = round(controller.currentMin * (1 - adaptStrength) + targetMinCandidate * adaptStrength);
    controller.currentMax = round(controller.currentMax * (1 - adaptStrength) + targetMaxCandidate * adaptStrength);
    controller.currentMin = max(controller.minLimit, controller.currentMin);
    controller.currentMax = min(controller.maxLimit, controller.currentMax);
    if controller.currentMax <= controller.currentMin
        controller.currentMax = controller.currentMin + max(2, round(controller.referenceMean * 0.1));
    end
    targetMean = max(1, (controller.currentMin + controller.currentMax) / 2);
    desiredVolume = controller.totalVoxels * controller.targetPorosity;
    countFromVolume = desiredVolume / targetMean;
    weightedObserved = matchScore * observedCount + (1 - matchScore) * controller.referenceCount;
    targetCountCandidate = 0.5 * countFromVolume + 0.5 * weightedObserved;
    adaptCountStrength = max(0.05, 0.35 * (1 - matchScore));
    controller.currentCount = round(controller.currentCount * (1 - adaptCountStrength) + targetCountCandidate * adaptCountStrength);
    controller.currentCount = max(1, controller.currentCount);
    maxCountByVolume = ceil(desiredVolume / max(controller.currentMin, 1));
    if isfinite(maxCountByVolume)
        controller.currentCount = min(controller.currentCount, maxCountByVolume);
    end
    controller.history.targetMin(end+1) = controller.currentMin; %#ok<AGROW>
    controller.history.targetMax(end+1) = controller.currentMax; %#ok<AGROW>
    controller.history.targetCount(end+1) = controller.currentCount; %#ok<AGROW>
    optParams.targetMin = controller.currentMin;
    optParams.targetMax = controller.currentMax;
    optParams.targetClusterCount = controller.currentCount;
    optParams.clusterTargetState = controller;
end
...
function model = enforceClusterSizeConstraints(model, optParams)
    ...
    if ~isempty(targetMax)
        largeThreshold = max(targetMax * 1.2, targetMax + max(10, round(0.05 * targetMax)));
    else
        largeThreshold = prctile(double(sizes), 95);
    end

    if ~isempty(targetMin)
        removeThreshold = max(1, floor(targetMin * 0.6));
        growThreshold = max(3, floor(targetMin * 0.85));
    else
        removeThreshold = max(1, round(prctile(double(sizes), 10)));
        growThreshold = max(removeThreshold + 1, round(prctile(double(sizes), 20)));
    end
...
function displayComprehensiveFinalResults(originalModel, finalModel, ...
    ...
    if isfield(optParams, 'clusterTargetState') && isstruct(optParams.clusterTargetState)
        finalTargetState = optParams.clusterTargetState;
        if isfield(finalTargetState, 'currentMin') && isfield(finalTargetState, 'currentMax')
            fprintf('  自适应目标簇范围: [%d, %d]\n', round(finalTargetState.currentMin), round(finalTargetState.currentMax));
        end
        if isfield(finalTargetState, 'currentCount')
            fprintf('  自适应目标簇数量: %d\n', round(finalTargetState.currentCount));
        end
        if isfield(finalTargetState, 'history') && isstruct(finalTargetState.history) && ...
                isfield(finalTargetState.history, 'iterations')
            nUpdates = numel(finalTargetState.history.iterations);
            if nUpdates > 1
                fprintf('  目标动态调整次数: %d\n', nUpdates - 1);
        end
    end
...
function visualizeFinalComparison(originalModel, finalModel, performanceMonitor)
    ...
    try
        ...
        % 匹配度与接受率历史
        subplot(2,3,5);
        morphHistory = performanceMonitor.morphologyMatchHistory;
        spatialHistory = performanceMonitor.spatialMatchHistory;
        acceptanceHistory = performanceMonitor.acceptanceRatio;
        nHistory = min([numel(morphHistory), numel(spatialHistory), numel(acceptanceHistory)]);
        if nHistory < 1
            nHistory = max([numel(morphHistory), numel(spatialHistory), numel(acceptanceHistory)]);
        end
        morphHistory = morphHistory(1:nHistory);
        spatialHistory = spatialHistory(1:nHistory);
        acceptanceHistory = acceptanceHistory(1:nHistory);
        validMask = (morphHistory > 0) | (spatialHistory > 0) | (acceptanceHistory > 0);
        lastValid = find(validMask, 1, 'last');
        if isempty(lastValid)
            lastValid = nHistory;
        end
        morphHistory = morphHistory(1:lastValid);
        spatialHistory = spatialHistory(1:lastValid);
        acceptanceHistory = acceptanceHistory(1:lastValid);
        iterAxis = (1:lastValid) * 100;
        if isempty(iterAxis)
            iterAxis = 1;
            morphHistory = zeros(size(iterAxis));
            spatialHistory = zeros(size(iterAxis));
            acceptanceHistory = zeros(size(iterAxis));
        end
        yyaxis left;
        hMorph = plot(iterAxis, morphHistory, 'b-', 'LineWidth', 2);
        hold on;
        hSpatial = plot(iterAxis, spatialHistory, 'r-', 'LineWidth', 2);
        ylabel('匹配度');
        ylim([0, 1]);
        yyaxis right;
        hAccept = plot(iterAxis, acceptanceHistory, 'k--', 'LineWidth', 1.5);
        ylabel('接受率');
        ylim([0, 1]);
        xlabel('迭代');
        title('匹配度与接受率演化');
        legend([hMorph, hSpatial, hAccept], {'形态匹配', '空间匹配', '接受率'}, 'Location', 'best');
        grid on;

        % 簇大小与目标演化
        subplot(2,3,6);
        clusterHistory = performanceMonitor.clusterSizeHistory;
        validClusterIdx = find(any(clusterHistory > 0, 1), 1, 'last');
        if isempty(validClusterIdx)
            validClusterIdx = size(clusterHistory, 2);
        end
        iterCluster = (1:validClusterIdx) * 100;
        hMin = plot(iterCluster, clusterHistory(1,1:validClusterIdx), 'Color', [0.2, 0.6, 0.2], 'LineWidth', 1.5);
        hold on;
        hMax = plot(iterCluster, clusterHistory(2,1:validClusterIdx), 'Color', [0.8, 0.2, 0.2], 'LineWidth', 1.5);
        hMean = plot(iterCluster, clusterHistory(3,1:validClusterIdx), 'Color', [0.2, 0.4, 0.8], 'LineWidth', 1.5);
        legendHandles = [hMin, hMax, hMean];
        legendText = {'最小簇', '最大簇', '平均簇'};
        if exist('optParams', 'var') && isstruct(optParams) && isfield(optParams, 'clusterTargetState') && ...
                isstruct(optParams.clusterTargetState) && isfield(optParams.clusterTargetState, 'history')
            targetHistory = optParams.clusterTargetState.history;
            if isfield(targetHistory, 'iterations') && ~isempty(targetHistory.iterations)
                hTargetMin = plot(targetHistory.iterations, targetHistory.targetMin, 'g--', 'LineWidth', 1.2);
                hTargetMax = plot(targetHistory.iterations, targetHistory.targetMax, 'r--', 'LineWidth', 1.2);
                legendHandles = [legendHandles, hTargetMin, hTargetMax];
                legendText = [legendText, {'目标最小', '目标最大'}];
            end
        end
        xlabel('迭代');
        ylabel('簇大小');
        legend(legendHandles, legendText, 'Location', 'best');
        title('簇大小演化与目标');
        grid on;
