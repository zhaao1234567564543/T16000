...
function mcmcModel = generateComprehensiveInitialModel(dims, targetPorosity, ...
    ...
    model = matchPoreSizeDistribution(model, originalFeatures, optParams);
    model = balanceClusterStatistics(model, optParams);
    model = adjustPorosity(model, targetPorosity);
    model = balanceClusterStatistics(model, optParams);
    fprintf('  初始模型生成完成');
    mcmcModel = model;
...
function model = adjustPorosity(model, targetPorosity)
    % 调整孔隙率到目标值（迭代方式，优先保留结构边界）
    if nargin < 2 || ~isfinite(targetPorosity)
        return;
    end
    ...
    for iter = 1:maxIterations
        ...
        neighborCount = convn(double(model), neighborKernel, 'same');
        nChange = max(1, round(abs(diff) * totalVoxels));

        if diff > 0
            % 需要增加孔隙：优先在已有簇边界附近填充
            candidateMask = ~model;
            boundaryMask = candidateMask & (neighborCount > 0);
            if any(boundaryMask(:))
            ...
            scores = neighborCount(candidates);
            [~, order] = sort(scores, 'descend');
            nSelect = min(nChange, numel(order));
            addIdx = candidates(order(1:nSelect));
            model(addIdx) = true;
        else
            % 需要减少孔隙：优先移除枝杈和孤立体素
            candidateMask = model;
            boundaryMask = candidateMask & (neighborCount <= 18);
            if any(boundaryMask(:))
                candidateMask = boundaryMask;
            end
            candidates = find(candidateMask);
            ...
            nSelect = min(nChange, numel(order));
            removeIdx = candidates(order(1:nSelect));
    ...
    if CC.NumObjects > 0
        ...
        tiny = find(sizes < 3);
        for i = 1:length(tiny)
            model(CC.PixelIdxList{tiny(i)}) = false;
        end
    end
end
...
function mcmcState = enforceComprehensiveConstraints(mcmcState, optParams, phase, parallelHint)
    ...
    mcmcState.model = balanceClusterStatistics(mcmcState.model, optParams);
...
function [model, added] = addRepresentativeCluster(model, CC, optParams)
    ...
    librarySizes = cellfun(@(c) c.size, library);
    sortedSizes = sort(librarySizes);
    lowIdx = max(1, round(0.2 * numel(sortedSizes)));
    highIdx = max(lowIdx, min(numel(sortedSizes), round(0.8 * numel(sortedSizes))));
    defaultMin = sortedSizes(lowIdx);
    defaultMax = sortedSizes(highIdx);
    targetMin = getfieldwithdefault(optParams, 'targetMin', defaultMin); %#ok<GFLD>
    targetMax = getfieldwithdefault(optParams, 'targetMax', defaultMax); %#ok<GFLD>
    targetMin = max(1, round(targetMin));
    targetMax = max(targetMin, round(targetMax));
    lowerBound = max(1, floor(0.9 * targetMin));
    upperBound = ceil(1.1 * targetMax);
    desiredSize = round((targetMin + targetMax) / 2);

    penalties = zeros(length(librarySizes), 1);
    for i = 1:length(librarySizes)
        s = librarySizes(i);
        if s < lowerBound
            penalties(i) = lowerBound - s;
        elseif s > upperBound
            penalties(i) = s - upperBound;
        else
            penalties(i) = abs(s - desiredSize) * 0.1;
        end
    end
    ...
    for k = 1:length(order)
        template = library{order(k)};
        if template.size < lowerBound * 0.6 || template.size > upperBound * 1.6
            continue;
        end

        candidate = placeClusterSample(model, template, densityMap);
        CC_new = bwconncomp(candidate, 26);
        if CC_new.NumObjects <= beforeCount
            continue;
        end

        delta = candidate & ~model;
        if ~any(delta(:))
            continue;
        end

        newIdx = [];
        for i = 1:CC_new.NumObjects
            if any(delta(CC_new.PixelIdxList{i}))
                newIdx = CC_new.PixelIdxList{i};
                break;
            end
        end
        if isempty(newIdx)
        ...
        newSize = numel(newIdx);
        if newSize < lowerBound || newSize > upperBound
            adjustedCandidate = candidate;
            if newSize > upperBound
                trimmedIdx = reduceOversizedClusterIndices(newIdx, size(candidate), targetMax);
                adjustedCandidate(newIdx) = false;
                adjustedCandidate(trimmedIdx) = true;
            else
                grownIdx = growSmallClusterIndices(newIdx, size(candidate));
                adjustedCandidate(newIdx) = false;
                adjustedCandidate(grownIdx) = true;
            end
            CC_adjusted = bwconncomp(adjustedCandidate, 26);
            deltaAdjusted = adjustedCandidate & ~model;
            if any(deltaAdjusted(:))
                newIdx = [];
                for i = 1:CC_adjusted.NumObjects
                    if any(deltaAdjusted(CC_adjusted.PixelIdxList{i}))
                        newIdx = CC_adjusted.PixelIdxList{i};
                        break;
                    end
                end
            end
...
function model = balanceClusterStatistics(model, optParams)
    % 调整簇数量与大小，使其更接近目标设定
    if nargin < 2 || ~isstruct(optParams)
        return;
    end
    ...
    targetCount = max(1, round(optParams.targetClusterCount));
    targetMin = max(1, round(getfieldwithdefault(optParams, 'targetMin', 1)));
    targetMax = max(targetMin, round(getfieldwithdefault(optParams, 'targetMax', targetMin)));
    lowerBound = max(1, floor(0.95 * targetMin));
    upperBound = ceil(1.05 * targetMax);
    toleranceCount = max(1, round(max(targetCount * 0.02, 1)));
    targetPorosity = getfieldwithdefault(optParams, 'targetPorosity', mean(model(:))); %#ok<GFLD>
    ...
    for pass = 1:maxPasses %#ok<NASGU>
        ...
        % 优先处理过大的簇
        oversizeIdx = find(sizes > upperBound);
        if ~isempty(oversizeIdx)
            [~, order] = sort(sizes(oversizeIdx), 'descend');
            for k = 1:length(order)
                idx = oversizeIdx(order(k));
                model = reduceOversizedCluster(model, CC.PixelIdxList{idx}, optParams);
            end
            adjusted = true;
        end
        ...
        CC = bwconncomp(model, 26);
        sizes = cellfun(@numel, CC.PixelIdxList);
        undersizeIdx = find(sizes < lowerBound);
        if ~isempty(undersizeIdx)
            for k = 1:length(undersizeIdx)
                idx = undersizeIdx(k);
                if isfield(optParams, 'preserveSmallPores') && optParams.preserveSmallPores
        ...
        CC = bwconncomp(model, 26);
        sizes = cellfun(@numel, CC.PixelIdxList);
        countDiff = CC.NumObjects - targetCount;

        if abs(countDiff) > toleranceCount
            if countDiff > 0
                removalOrder = prioritizeRemovalOrder(sizes, lowerBound, upperBound);
                removeCount = min(abs(countDiff), numel(removalOrder));
                for k = 1:removeCount
                    model(CC.PixelIdxList{removalOrder(k)}) = false;
                end
            else
                missing = abs(countDiff);
                for k = 1:missing
                    CC = bwconncomp(model, 26);
                    [model, added] = addRepresentativeCluster(model, CC, optParams);
                    if ~added
        ...
        CC = bwconncomp(model, 26);
        sizes = cellfun(@numel, CC.PixelIdxList);
        if clusterStatisticsSatisfied(CC.NumObjects, sizes, targetCount, lowerBound, upperBound, toleranceCount)
            break;
        end

        if nnz(model ~= previousModel) == 0
            break;
        end
    end
end

function order = prioritizeRemovalOrder(sizes, lowerBound, upperBound)
    % 根据与目标范围的偏离程度决定移除顺序
    targetMid = (lowerBound + upperBound) / 2;
    penalties = zeros(length(sizes), 1);
    for i = 1:length(sizes)
        if sizes(i) < lowerBound
            penalties(i) = (lowerBound - sizes(i)) + abs(sizes(i) - targetMid);
        elseif sizes(i) > upperBound
            penalties(i) = (sizes(i) - upperBound) + abs(sizes(i) - targetMid);
        else
            penalties(i) = abs(sizes(i) - targetMid) * 0.1;
        end
    end
    [~, order] = sort(penalties, 'descend');
end

function satisfied = clusterStatisticsSatisfied(count, sizes, targetCount, lowerBound, upperBound, toleranceCount)
    if count == 0
        satisfied = false;
        return;
    end
    withinRange = sum(sizes >= lowerBound & sizes <= upperBound);
    satisfied = abs(count - targetCount) <= toleranceCount && ...
        withinRange >= max(1, round(0.85 * numel(sizes)));
end
...
function value = getfieldwithdefault(structure, fieldName, defaultValue)
    ...
    if isnumeric(candidate) && all(isfinite(candidate(:)))
        value = candidate;
    end
end
