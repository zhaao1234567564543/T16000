...
optParams.energyWeights = deriveDefaultEnergyWeights();
...
function tpc = computeFastTwoPointCorrelation(binaryModel, keyDistances)
    % 快速计算两点相关函数，使用零填充避免卷绕伪影
    nDistances = length(keyDistances);
    tpc = zeros(nDistances, 3);
    if isempty(binaryModel)
        return;
    end
    meanVal = mean(binaryModel(:));
    for i = 1:nDistances
        lag = max(1, round(keyDistances(i)));
        sampleStep = max(1, round(lag / 8));
        sampled = binaryModel(1:sampleStep:end, 1:sampleStep:end, 1:sampleStep:end);
        if size(sampled, 1) <= lag
            tpc(i, 1) = meanVal;
        else
            base = sampled(1:end-lag, :, :);
            shifted = sampled(1+lag:end, :, :);
            tpc(i, 1) = mean(base(:) .* shifted(:));
        end
        if size(sampled, 2) <= lag
            tpc(i, 2) = meanVal;
        else
            base = sampled(:, 1:end-lag, :);
            shifted = sampled(:, 1+lag:end, :);
            tpc(i, 2) = mean(base(:) .* shifted(:));
        end
        if size(sampled, 3) <= lag
            tpc(i, 3) = meanVal;
        else
            base = sampled(:, :, 1:end-lag);
            shifted = sampled(:, :, 1+lag:end);
            tpc(i, 3) = mean(base(:) .* shifted(:));
        end
    end
end
...
function mcmcModel = generateComprehensiveInitialModel(dims, targetPorosity, ...
    ...
    for step = 1:nOptSteps
        ...
        switch primaryTarget
            case 1
                model = applySpatialCorrection(model, currentSpatial, spatialFeatures, optParams, optParams.energyWeights);
            case 2
                model = applyMorphologyCorrection(model, currentMorph, morphologyFeatures, optParams);
            case 3
...
function model = calibrateInitialModelToTargets(model, targetSpatial, targetMorph, optParams, densityMap)
    ...
    for pass = 1:maxPasses
        baseCache = struct();
        baseCache.spatialFeatures = computeEnhancedSpatialFeatures(model);
        baseCache.morphologyFeatures = computeDetailedMorphologyFeatures(model);
        baseCache.multiScaleSpatialFeatures = computeMultiScaleSpatialFeatures(model);
        baseEnergy = computeUnifiedEnergy(model, optParams, weights, baseCache);
        ...
        if abs(currentPorosity - targetPorosity) > 5e-4
            ...
            candidateEnergy = computeUnifiedEnergy(candidate, optParams, weights);
            if candidateEnergy < baseEnergy
                model = candidate;
                baseEnergy = candidateEnergy;
            end
            if ~isempty(densityMap)
        ...
        currentMorph = computeDetailedMorphologyFeatures(model);
        currentMultiScale = computeMultiScaleSpatialFeatures(model);
        if calculateSpatialMatch(currentSpatial, targetSpatial) < 0.97
            candidate = applySpatialCorrection(model, currentSpatial, targetSpatial, optParams, weights);
            candidateEnergy = computeUnifiedEnergy(candidate, optParams, weights);
            if candidateEnergy < baseEnergy
                model = candidate;
                baseEnergy = candidateEnergy;
        ...
        if calculateMorphologyMatch(currentMorph, targetMorph) < 0.97
            candidate = applyMorphologyCorrection(model, currentMorph, targetMorph, optParams);
            candidateEnergy = computeUnifiedEnergy(candidate, optParams, weights);
            if candidateEnergy < baseEnergy
                model = candidate;
                baseEnergy = candidateEnergy;
        ...
        if isfield(optParams, 'multiScaleSpatialFeatures') && ~isempty(optParams.multiScaleSpatialFeatures)
            if calculateMultiScaleMatch(currentMultiScale, optParams.multiScaleSpatialFeatures) < 0.95
                candidate = applyMultiScaleCorrection(model, currentMultiScale, optParams.multiScaleSpatialFeatures);
                candidateEnergy = computeUnifiedEnergy(candidate, optParams, weights);
                if candidateEnergy < baseEnergy
                    model = candidate;
                    baseEnergy = candidateEnergy;
        ...
        if abs(mean(model(:)) - targetPorosity) < 5e-4
            break;
        end
    end
end
...
function model = applyTargetMorphology(model, morphologyFeatures)
    ...
    if ~isempty(morphologyFeatures.sphericity)
        ...
        elseif targetSphericity < 0.4
            ...
            dirs = {'x', 'y', 'z'};
            for d = 1:numel(dirs)
                se = createDirectionalStructuringElement(dirs{d}, 5, [1, 1, 1]);
                model = imopen(model, se);
            end
        else
            % 中等球形度：轻微形态学操作
...
function model = applySpatialCorrection(model, currentSpatial, targetSpatial, optParams, weights)
    % 能量驱动的空间修正，避免随机翻转导致的轴向偏置
    if nargin < 4 || isempty(optParams)
        optParams = struct();
    end
    if nargin < 5
    ...
    cache = struct();
    if nargin >= 2 && ~isempty(currentSpatial)
        cache.spatialFeatures = currentSpatial;
    end
    if nargin < 3 || isempty(targetSpatial)
        return;
    end
    weights = getEnergyWeights(optParams, weights);
    baseEnergy = computeUnifiedEnergy(model, optParams, weights, cache);
    ...
    if isfield(targetSpatial, 'keyDistances') && ~isempty(targetSpatial.keyDistances)
        keyDistances = targetSpatial.keyDistances;
    elseif isfield(cache, 'spatialFeatures') && isfield(cache.spatialFeatures, 'keyDistances')
        keyDistances = cache.spatialFeatures.keyDistances;
    ...
    if isempty(keyDistances)
        keyDistances = 1:max(1, min(size(targetSpatial.twoPointCorr, 1), 25));
    end
    if ~isfield(cache, 'spatialFeatures') || isempty(cache.spatialFeatures)
        cache.spatialFeatures = computeEnhancedSpatialFeatures(model);
    end
    currentTPC = cache.spatialFeatures.twoPointCorr;
    if isempty(currentTPC)
        currentTPC = computeFastTwoPointCorrelation(model, keyDistances);
    end
    minRows = min(size(currentTPC, 1), size(targetSpatial.twoPointCorr, 1));
    if minRows == 0
        return;
    end
    currentTPC = currentTPC(1:minRows, :);
    targetTPC = targetSpatial.twoPointCorr(1:minRows, :);
    residual = currentTPC - targetTPC;
    nWeights = min(length(keyDistances), size(residual, 1));
    localResidual = residual(1:nWeights, :);
    localDistances = keyDistances(1:nWeights);
    weightsTPC = exp(-((localDistances - localDistances(1)).^2) / (2 * (max(localDistances) + eps).^2));
    directionalResidual = mean(localResidual, 2);
    weightedResidual = sum(directionalResidual(:) .* weightsTPC(:));
    if abs(weightedResidual) < 1e-4
        return;
    end
    mode = 'increase';
    if weightedResidual > 0
        mode = 'decrease';
    end
    spacing = getVoxelSpacing(optParams);
    neighborKernel = ones(3, 3, 3);
    neighborKernel(2, 2, 2) = 0;
    neighborCount = convn(double(model), neighborKernel, 'same');
    candidateIdx = selectSpatialEditCandidates(model, mode, neighborCount, spacing);
    if isempty(candidateIdx)
        return;
    end
    maxEvaluations = min(48, numel(candidateIdx));
    candidateIdx = candidateIdx(1:maxEvaluations);
    bestEnergy = baseEnergy;
    bestModel = [];
    for idx = candidateIdx(:)'
        [candidateModel, changedIdx] = applySpatialEditPatch(model, idx, mode, spacing);
        if isempty(changedIdx)
            continue;
        end
        if ~isEditTopologicallySafe(model, candidateModel, changedIdx, mode)
            continue;
        end
        candidateEnergy = computeUnifiedEnergy(candidateModel, optParams, weights);
        if candidateEnergy + 1e-6 < bestEnergy
...
function model = applyMorphologyCorrection(model, currentMorph, targetMorph, optParams)
    % 3D 各向同性的形态学修正，避免逐层处理导致的轴向偏置
    if nargin < 4 || isempty(optParams)
        optParams = struct();
    end
    spacing = getVoxelSpacing(optParams);
    smoothSE = createAnisotropicBall(1, spacing);
    if ~isempty(currentMorph.sphericity) && ~isempty(targetMorph.sphericity)
        ...
        if abs(currentSpher - targetSpher) > 0.05
            if targetSpher > currentSpher
                se = createAnisotropicBall(1.5, spacing);
                model = imclose(imopen(model, smoothSE), se);
            else
                shrinkDirs = {'x', 'y', 'z'};
                for dirIdx = 1:numel(shrinkDirs)
                    dirSe = createDirectionalStructuringElement(shrinkDirs{dirIdx}, 3, spacing);
                    model = imerode(model, dirSe);
                    model = imdilate(model, dirSe);
                end
                model = imopen(model, smoothSE);
            end
        end
    end
    if ~isempty(currentMorph.elongation) && ~isempty(targetMorph.elongation)
        ...
        currentProfile = computeDirectionalPorosityProfile(model);
        if isfield(optParams, 'directionalPorosityProfile') && ~isempty(optParams.directionalPorosityProfile)
            targetProfile = optParams.directionalPorosityProfile;
        else
            targetProfile = computeDirectionalPorosityProfile(optParams.originalBinaryModel);
        end
        gradientDiff = targetProfile.gradientStrength - currentProfile.gradientStrength;
        [~, dominantDirIdx] = max(abs(gradientDiff));
        dirNames = {'x', 'y', 'z'};
        dominantDir = dirNames{dominantDirIdx};
        if abs(currentElong - targetElong) > 0.3
            if gradientDiff(dominantDirIdx) > 0
                extendSe = createDirectionalStructuringElement(dominantDir, 4, spacing);
                model = imdilate(model, extendSe);
                for dirIdx = setdiff(1:3, dominantDirIdx)
                    contractSe = createDirectionalStructuringElement(dirNames{dirIdx}, 2, spacing);
                    model = imerode(model, contractSe);
                end
            else
                contractSe = createDirectionalStructuringElement(dominantDir, 3, spacing);
                model = imerode(model, contractSe);
                balanceSe = createAnisotropicBall(1, spacing);
                model = imclose(model, balanceSe);
            end
        end
    end
    model = imclose(imopen(model, smoothSE), smoothSE);
end
...
function energy = computeInitialMCMCEnergy(model, targetSpatial, targetMorph, targetPorosity)
    % 使用统一能量函数计算初始MCMC能量
    optParams = struct();
    optParams.targetPorosity = targetPorosity;
    optParams.spatialFeatures = targetSpatial;
    optParams.morphologyFeatures = targetMorph;
    optParams.multiScaleSpatialFeatures = [];
    optParams.originalFeatures = struct();
    weights = deriveDefaultEnergyWeights();
    energy = computeUnifiedEnergy(model, optParams, weights);
end
...
function energy = calculateComprehensiveEnergy(mcmcState, parallelHint)
    ...
    weights = getEnergyWeights(mcmcState.optParams, mcmcState.weights);
    [energy, ~] = evaluateGlobalEnergyComponents(mcmcState.model, ...
        mcmcState.features, mcmcState.spatialFeatures, mcmcState.morphologyFeatures, ...
        mcmcState.multiScaleSpatialFeatures, mcmcState.optParams, weights, parallelEnabled);
end
function weights = deriveDefaultEnergyWeights()
    weights = struct();
    weights.cluster = 0.8;
    weights.porosity = 3.0;
    weights.morphology = 12.0;
    weights.spatial = 10.0;
    weights.connectivity = 5.0;
    weights.multiScale = 4.0;
    weights.shapePreservation = 8.0;
    weights.structureCoherence = 5.5;
end
function weights = getEnergyWeights(optParams, override)
    if nargin >= 2 && ~isempty(override)
        weights = override;
        return;
    end
    if nargin >= 1 && isfield(optParams, 'energyWeights') && ~isempty(optParams.energyWeights)
        weights = optParams.energyWeights;
    else
        weights = deriveDefaultEnergyWeights();
    end
end
function energy = computeUnifiedEnergy(model, optParams, weights, cache)
    if nargin < 4
        cache = struct();
    end
    if ~isfield(cache, 'features') || isempty(cache.features)
        cache.features = extractEfficientClusterFeatures(model);
    end
    if ~isfield(cache, 'spatialFeatures') || isempty(cache.spatialFeatures)
        cache.spatialFeatures = computeEnhancedSpatialFeatures(model);
    end
    if ~isfield(cache, 'morphologyFeatures') || isempty(cache.morphologyFeatures)
        cache.morphologyFeatures = computeDetailedMorphologyFeatures(model);
    end
    if ~isfield(cache, 'multiScaleSpatialFeatures') || isempty(cache.multiScaleSpatialFeatures)
        cache.multiScaleSpatialFeatures = computeMultiScaleSpatialFeatures(model);
    end
    [energy, ~] = evaluateGlobalEnergyComponents(model, cache.features, cache.spatialFeatures, ...
        cache.morphologyFeatures, cache.multiScaleSpatialFeatures, optParams, ...
        getEnergyWeights(optParams, weights), false);
end
function [energy, components] = evaluateGlobalEnergyComponents(model, features, spatialFeatures, morphologyFeatures, multiScaleFeatures, optParams, weights, parallelEnabled)
    if nargin < 8
    ...
    if parallelEnabled
        ...
        parfor compIdx = 1:8
            componentResults(compIdx) = evaluateEnergyComponent(compIdx, model, features, spatialFeatures, morphologyFeatures, multiScaleFeatures, optParams);
        end
    else
        componentResults = zeros(8, 1);
        for compIdx = 1:8
    ...
    components = struct('porosity', componentResults(1), 'cluster', componentResults(2), ...
        'morphology', componentResults(3), 'spatial', componentResults(4), ...
        'connectivity', componentResults(5), 'multiScale', componentResults(6), ...
        'shapePreservation', componentResults(7), 'structureCoherence', componentResults(8));
...
function value = evaluateEnergyComponent(index, model, features, spatialFeatures, morphologyFeatures, multiScaleFeatures, optParams)
    switch index
        case 1
            value = calculatePorosityEnergy(features, optParams);
        case 2
            value = calculateClusterEnergy(features, optParams);
        case 3
            value = calculateMorphologyEnergy(morphologyFeatures, optParams);
        case 4
            value = calculateSpatialEnergy(spatialFeatures, optParams);
        case 5
            value = calculateConnectivityEnergy(spatialFeatures, optParams);
        case 6
            value = calculateMultiScaleEnergy(multiScaleFeatures, optParams);
        case 7
            value = calculateShapePreservationEnergy(features, morphologyFeatures, optParams);
        case 8
            value = calculateStructureCoherenceEnergy(struct('model', model));
        otherwise
            value = 0;
    end
end

function spacing = getVoxelSpacing(optParams)
    spacing = [1, 1, 1];
    if nargin >= 1 && isfield(optParams, 'voxelSize') && ~isempty(optParams.voxelSize)
        spacing = optParams.voxelSize;
        if numel(spacing) == 1
            spacing = repmat(spacing, 1, 3);
        end
    end
    spacing = spacing(:)';
    spacing(spacing <= 0) = 1;
end
function candidateIdx = selectSpatialEditCandidates(model, mode, neighborCount, spacing)
    boundary = bwperim(model, 26);
    if strcmp(mode, 'decrease')
        candidateMask = boundary & model & (neighborCount >= 10);
        if any(candidateMask(:))
            distInterior = bwdist(~model);
            scores = neighborCount(candidateMask) .* (distInterior(candidateMask) + 1);
        else
            candidateMask = boundary & model;
            scores = neighborCount(candidateMask) + 1;
        end
    else
        growthBand = imdilate(model, createAnisotropicBall(1.2, spacing)) & ~model;
        candidateMask = growthBand & (neighborCount >= 4);
        if any(candidateMask(:))
            distToPore = bwdist(model);
            scores = (neighborCount(candidateMask) + 1) ./ (distToPore(candidateMask) + 1);
        else
            candidateMask = growthBand;
            scores = neighborCount(candidateMask) + 1;
        end
    end
    ...
    if isempty(candidateIdx)
        ...
        scores = neighborCount(candidateIdx) + 1;
    end
    [~, order] = sort(scores, 'descend');
    candidateIdx = candidateIdx(order);
end
function [candidateModel, changedIdx] = applySpatialEditPatch(model, centerIdx, mode, spacing)
    dims = size(model);
    [cx, cy, cz] = ind2sub(dims, centerIdx);
    se = createAnisotropicBall(1.2, spacing);
    seSize = size(se);
    half = floor((seSize - 1) / 2);
    [ox, oy, oz] = ind2sub(seSize, find(se));
    ox = ox - (half(1) + 1);
    oy = oy - (half(2) + 1);
    oz = oz - (half(3) + 1);
    ...
    for k = 1:numel(ox)
        ...
        ty = cy + oy(k);
        tz = cz + oz(k);
        if tx < 1 || ty < 1 || tz < 1 || tx > dims(1) || ty > dims(2) || tz > dims(3)
            continue;
        end
        idx = sub2ind(dims, tx, ty, tz);
        if strcmp(mode, 'increase')
            if ~candidateModel(idx)
                candidateModel(idx) = true;
                changedIdx(end+1, 1) = idx; %#ok<AGROW>
            end
        else
            if candidateModel(idx)
                candidateModel(idx) = false;
                changedIdx(end+1, 1) = idx; %#ok<AGROW>
            end
        end
    end
    if ~isempty(changedIdx)
        changedIdx = unique(changedIdx, 'stable');
    end
end
function safe = isEditTopologicallySafe(originalModel, candidateModel, changedIdx, mode)
    safe = false;
    if isempty(changedIdx)
        return;
    end
    dims = size(originalModel);
    [x, y, z] = ind2sub(dims, changedIdx);
    margin = 2;
    xRange = max(1, min(x) - margin):min(dims(1), max(x) + margin);
    yRange = max(1, min(y) - margin):min(dims(2), max(y) + margin);
    zRange = max(1, min(z) - margin):min(dims(3), max(z) + margin);
    localBefore = originalModel(xRange, yRange, zRange);
    localAfter = candidateModel(xRange, yRange, zRange);
    CCBefore = bwconncomp(localBefore, 26);
    CCAfter = bwconncomp(localAfter, 26);
    if strcmp(mode, 'increase')
        if ~any(localBefore(:))
            safe = false;
            return;
        end
        safe = CCAfter.NumObjects <= CCBefore.NumObjects + 1;
    else
        if CCBefore.NumObjects <= 1
            safe = CCAfter.NumObjects <= CCBefore.NumObjects && CCAfter.NumObjects >= 1;
        else
            safe = CCAfter.NumObjects >= CCBefore.NumObjects - 1;
        end
        if safe && CCBefore.NumObjects > 0
            beforeSizes = cellfun(@numel, CCBefore.PixelIdxList);
            afterSizes = cellfun(@numel, CCAfter.PixelIdxList);
            safe = max(afterSizes, [], 'omitnan') >= 0.5 * max(beforeSizes, [], 'omitnan');
        end
    end
end
function se = createAnisotropicBall(radius, spacing)
    if nargin < 2 || isempty(spacing)
        spacing = [1, 1, 1];
    end
    spacing = spacing(:)';
    spacing(spacing <= 0) = 1;
    maxSteps = ceil(radius ./ spacing) + 1;
    x = -maxSteps(1):maxSteps(1);
    y = -maxSteps(2):maxSteps(2);
    z = -maxSteps(3):maxSteps(3);
    [X, Y, Z] = ndgrid(x * spacing(1), y * spacing(2), z * spacing(3));
    se = (X.^2 + Y.^2 + Z.^2) <= (radius + eps)^2;
    se = logical(se);
end
function se = createDirectionalStructuringElement(direction, len, spacing)
    if nargin < 3
        spacing = [1, 1, 1];
    end
    if ischar(direction)
    ...
    spacing = spacing(:)';
    step = max(1, round(len / max(spacing(axisIdx), eps)));
    dims = [1, 1, 1];
    dims(axisIdx) = 2 * step + 1;
    se = false(dims);
    if axisIdx == 1
        se(:, 1, 1) = true;
    elseif axisIdx == 2
...
function costField = computeConnectionCostField(localModel, spacing)
    distToPore = bwdist(localModel);
    distToPore = distToPore / (max(distToPore(:)) + eps);
    costField = 0.1 + distToPore;
    costField(localModel) = 0.02;
    smoothPenalty = imgaussfilt3(double(~localModel), max(spacing));
    costField = costField + 0.1 * smoothPenalty;
end
function pathIdx = shortestPath3D(costField, startIdx, goalIdx)
    dims = size(costField);
    numNodes = numel(costField);
    ...
    mask = ~(dx == 0 & dy == 0 & dz == 0);
    neighborOffsets = [dx(mask), dy(mask), dz(mask)];
    while true
        tmpDistances = distances;
        tmpDistances(visited) = inf;
        [minDist, current] = min(tmpDistances);
        if ~isfinite(minDist)
            pathIdx = [];
            return;
        end
        ...
        [cx, cy, cz] = ind2sub(dims, current);
        for n = 1:size(neighborOffsets, 1)
            nx = cx + neighborOffsets(n, 1);
            ny = cy + neighborOffsets(n, 2);
            nz = cz + neighborOffsets(n, 3);
            if nx < 1 || ny < 1 || nz < 1 || nx > dims(1) || ny > dims(2) || nz > dims(3)
                continue;
            end
            neighborIdx = sub2ind(dims, nx, ny, nz);
            if visited(neighborIdx)
                continue;
            end
            stepCost = (costField(neighborIdx) + costField(current)) * 0.5;
            stepPenalty = sum(abs(neighborOffsets(n, :)) > 0);
            stepCost = stepCost * (1 + 0.2 * max(0, stepPenalty - 1));
            newDist = minDist + stepCost;
    ...
    if previous(goalIdx) == 0 && goalIdx ~= startIdx
        pathIdx = [];
        return;
    end
    pathIdx = goalIdx;
    ...
    while current ~= startIdx && current ~= 0
        ...
        pathIdx(end+1, 1) = current; %#ok<AGROW>
    end
end
            ...
            if CC.NumObjects > 0
                ...
                for i = 1:nAdjust
                    ...
                    if targetSpher > currentSpher
                        ...
                        % 减少球形度，沿随机方向进行3D拉伸
                        dirs = {'x', 'y', 'z'};
                        chosenDir = dirs{randi(numel(dirs))};
                        se = createDirectionalStructuringElement(chosenDir, 3, getVoxelSpacing(optParams));
    ...
    if length(smallClusters) >= 2
        for i = 1:length(smallClusters)-1
            for j = i+1:length(smallClusters)
                ...
                if dist < 5
                    model = connectClusters(model, CC.PixelIdxList{idx1}, CC.PixelIdxList{idx2}, optParams);
                    modified = true;
                    break;
                end
            end
...
function model = mergeClosestClusters(model, CC)
    ...
    for idx = 1:length(order)
        ...
        if ~isempty(bestPartner)
            model = connectClusters(model, CC.PixelIdxList{currentIdx}, CC.PixelIdxList{bestPartner}, optParams);
            merged = true;
            break;
        end
    end
...
function model = connectClusters(model, cluster1, cluster2, optParams)
    % 使用受约束的最短路径连接簇，避免直线穿透
    if nargin < 4
        optParams = struct();
    end
    ...
    volumeSize = size(model);
    [closestDist, p1, p2] = findClosestPoints(model, cluster1, cluster2);
    if isempty(p1) || isempty(p2) || ~isfinite(closestDist)
        return;
    end
    margin = 6;
    xRange = max(1, min(p1(1), p2(1)) - margin):min(volumeSize(1), max(p1(1), p2(1)) + margin);
    yRange = max(1, min(p1(2), p2(2)) - margin):min(volumeSize(2), max(p1(2), p2(2)) + margin);
    zRange = max(1, min(p1(3), p2(3)) - margin):min(volumeSize(3), max(p1(3), p2(3)) + margin);
    localModel = model(xRange, yRange, zRange);
    spacing = getVoxelSpacing(optParams);
    costField = computeConnectionCostField(localModel, spacing);
    localStart = [p1(1) - xRange(1) + 1, p1(2) - yRange(1) + 1, p1(3) - zRange(1) + 1];
    localGoal = [p2(1) - xRange(1) + 1, p2(2) - yRange(1) + 1, p2(3) - zRange(1) + 1];
    startIdx = sub2ind(size(localModel), localStart(1), localStart(2), localStart(3));
    goalIdx = sub2ind(size(localModel), localGoal(1), localGoal(2), localGoal(3));
    pathIdx = shortestPath3D(costField, startIdx, goalIdx);
    if isempty(pathIdx)
        fallback = imdilate(localModel, createAnisotropicBall(1.2, spacing));
        pathIdx = find(fallback & ~localModel);
    end
    if isempty(pathIdx)
        return;
    end
    localMask = localModel;
    localMask(pathIdx) = true;
    smoothSE = createAnisotropicBall(1, spacing);
    localMask = imclose(imopen(localMask, smoothSE), smoothSE);
    model(xRange, yRange, zRange) = localMask;
end
...
function move = generateSpatialCorrelationMove(model, optParams)
    ...
    weights = getEnergyWeights(optParams, []);
    baseEnergy = computeUnifiedEnergy(model, optParams, weights);
    currentSpatial = computeEnhancedSpatialFeatures(model);
    targetSpatial = optParams.spatialFeatures;
    if isempty(targetSpatial)
        return;
    end
    modelAfter = applySpatialCorrection(model, currentSpatial, targetSpatial, optParams, weights);
    if isequal(modelAfter, model)
        move = generateLocalMove(model, 4);
        return;
    end
    changed = find(modelAfter ~= model);
    if isempty(changed)
        move = generateLocalMove(model, 4);
        return;
    end
    move.linearIdx = changed(:);
    move.oldValues = model(changed);
    move.newValues = modelAfter(changed);
    candidateEnergy = computeUnifiedEnergy(modelAfter, optParams, weights);
    if candidateEnergy >= baseEnergy
        move = generateLocalMove(model, 4);
    end
end
...
function model = localMorphologyPerturbation(model, optParams)
    ...
    if targetSphericity > 0.6
        se = createAnisotropicBall(1, spacing);
        adjusted = imclose(clusterMask, se);
    else
        dirs = {'x', 'y', 'z'};
        contractDir = dirs{randi(numel(dirs))};
        seContract = createDirectionalStructuringElement(contractDir, 3, spacing);
        adjusted = imerode(clusterMask, seContract);
        expandDirs = dirs(~strcmp(dirs, contractDir));
        for d = 1:numel(expandDirs)
            seExpand = createDirectionalStructuringElement(expandDirs{d}, 2, spacing);
            adjusted = imdilate(adjusted, seExpand);
        end
    end
    ...
    if length(smallPoresIdx) >= 2
        ...
        for i = 1:length(smallPoresIdx)-1
            ...
            for j = i+1:length(smallPoresIdx)
                ...
                if dist < 5
                    % 合并这两个孔隙
                    model = connectClusters(model, CC.PixelIdxList{smallPoresIdx(i)}, ...
                        CC.PixelIdxList{smallPoresIdx(j)}, optParams);
                    merged([i, j]) = true;
                end
            end
        end
