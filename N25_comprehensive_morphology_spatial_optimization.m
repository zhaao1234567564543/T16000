...
% 自适应能量函数权重
weights = struct();
weights.cluster = 10.0; % 强化簇拓扑约束
weights.porosity = 3.0;
weights.morphology = 10.0; % 形态保持
weights.spatial = 10.0; % 空间特征匹配
...
optParams.initialEnergy = mcmcState.currentEnergy;
optParams.bestEnergy = mcmcState.bestEnergy;
% 自适应温度控制
T0 = 0.4;
cooling_rate = 0.9996;
...
for iter = 1:maxIterations
    ...
    % 生成批量候选移动
    [moves, moveTypes, mcmcState] = generateComprehensiveBatchMoves(mcmcState, ...
        batchSize, optParams, parallelEnabled);
...
optParams.bestEnergy = mcmcState.bestEnergy;
optParams.finalMCMCEnergy = mcmcState.currentEnergy;
%% 6. 综合后处理
fprintf('正在进行综合后处理...\n');
...
finalModel = enforcePorosityHardConstraint(finalModel, optParams.targetPorosity, 500);
% 检查模型质量
...
function mcmcState = initializeComprehensiveMCMCState(model, optParams, weights, parallelHint)
    ...
    mcmcState.initialEnergy = mcmcState.currentEnergy;
    % 初始化匹配度跟踪
    mcmcState.currentMorphMatch = calculateMorphologyMatch(mcmcState.morphologyFeatures, ...
        mcmcState.optParams.morphologyFeatures);
    mcmcState.currentSpatialMatch = calculateSpatialMatch(mcmcState.spatialFeatures, ...
...
function currentWeights = adjustWeightsForPhase(baseWeights, phase)
    ...
    switch phase
        case 'morphology'
            ...
            currentWeights.cluster = max(5.0, baseWeights.cluster * 1.0);
        ...
        case 'balanced'
            ...
            currentWeights.cluster = max(5.0, baseWeights.cluster);
    end
end
%% ========== 能量计算函数 ==========
...
function E_cluster = calculateClusterEnergy(features, optParams)
    ...
    % 建立目标统计量
    N_current = max(1, features.numClusters);
    N_target = max(1, getfieldOrDefault(clusterRef, 'numClusters', N_current)); %#ok<GFLD>

    s_max_current = max(sizes);
    s_max_target = max(1, getfieldOrDefault(clusterRef, 'maxSize', s_max_current));

    s_mean_current = mean(sizes);
    s_mean_target = max(1, getfieldOrDefault(clusterRef, 'meanSize', s_mean_current));

    % 归一化能量项
    E_num = ((N_current - N_target) / N_target) ^ 2;
    E_mean = ((s_mean_current - s_mean_target) / s_mean_target) ^ 2;
    E_max = max(0, (s_max_current / s_max_target - 1)) ^ 2;

    % 加强对超大簇的惩罚，避免严重合并
    w_num = 1.0;
    w_mean = 1.0;
    w_max = 6.0;
    E_cluster = w_num * E_num + w_mean * E_mean + w_max * E_max;

    % 附加分布差异惩罚（保持原有直方图距离，但作为弱约束）
    if isfield(clusterRef, 'histEdges') && isfield(clusterRef, 'histogram')
        ...
        E_cluster = E_cluster + 0.5 * histPenalty;
    end

    % 保证能量范围稳定
    E_cluster = max(0, min(E_cluster, 20));
end
...
function mcmcState = updateAllFeatures(mcmcState, parallelHint)
    ...
    persistent featureParallelDisabled
    ...
    if parallelEnabled
        try
            ...
            mcmcState.multiScaleSpatialFeatures = featureResults{4};
             ...
             warning(ME.identifier, '并行特征提取失败，切换到串行以避免闪退：%s', ME.message);
            featureParallelDisabled = true;
            parallelEnabled = false;
        end
    ...
    % 更新边界缓存，确保移动生成可以直接使用
    [mcmcState.poreBoundary, mcmcState.solidBoundary] = computeBoundaryMasks(mcmcState.model);
    mcmcState.poreMask = mcmcState.model;
    mcmcState.solidMask = ~mcmcState.model;
    mcmcState.boundaryDirty = false;
    % 如果能量有所改进，更新最佳记录
...
%% ========== 移动策略和生成函数（续） ==========
function [moves, moveTypes, mcmcState] = generateComprehensiveBatchMoves(mcmcState, ...
    batchSize, optParams, parallelEnabled)
    % 生成综合批量移动（边界优先、孔隙率硬约束）
    if batchSize <= 0
        moves = {};
        moveTypes = {};
        return;
    ...
    % 确保边界缓存可用
    mcmcState = ensureBoundaryCaches(mcmcState);

    moves = cell(batchSize, 1);
    moveTypes = cell(batchSize, 1);

    if nargin < 4 || isempty(parallelEnabled)
        parallelEnabled = shouldUseParallel(batchSize, numel(mcmcState.model));
    end

    pSwap = 0.7;      % 边界交换占比
    pAdjust = 0.25;   % 边界生灭占比
    pRandom = 0.05;   % 极少量全局随机跳转

    if parallelEnabled
        parfor i = 1:batchSize
            [moves{i}, moveTypes{i}] = generateSingleBoundaryMove(mcmcState, optParams, ...
                pSwap, pAdjust, pRandom);
        end
    else
        for i = 1:batchSize
            [moves{i}, moveTypes{i}] = generateSingleBoundaryMove(mcmcState, optParams, ...
...
function [move, moveType] = generateSingleBoundaryMove(mcmcState, optParams, pSwap, pAdjust, pRandom)
    r = rand();
    if r < pSwap
        move = generateBoundarySwapMove(mcmcState);
        moveType = 'boundary_swap';
    elseif r < pSwap + pAdjust
        move = generateBoundaryBirthDeathMove(mcmcState, optParams);
        moveType = 'boundary_adjust';
    else
        move = generateRandomFlipMove(mcmcState.model);
        moveType = 'random_escape';
    end
end

function mcmcState = ensureBoundaryCaches(mcmcState)
    if ~isfield(mcmcState, 'boundaryDirty') || mcmcState.boundaryDirty || ...
            ~isfield(mcmcState, 'poreBoundary') || isempty(mcmcState.poreBoundary)
        [mcmcState.poreBoundary, mcmcState.solidBoundary] = computeBoundaryMasks(mcmcState.model);
        mcmcState.poreMask = mcmcState.model;
        mcmcState.solidMask = ~mcmcState.model;
        mcmcState.boundaryDirty = false;
...
function [poreBoundary, solidBoundary] = computeBoundaryMasks(model)
    poreBoundary = bwperim(model, 6);
    solidBoundary = bwperim(~model, 6);
end

function move = generateBoundarySwapMove(mcmcState)
    model = mcmcState.model;
    poreIdx = find(mcmcState.poreBoundary);
    solidIdx = find(mcmcState.solidBoundary);
    move = struct('linearIdx', [], 'oldValues', [], 'newValues', []);
    if isempty(poreIdx) || isempty(solidIdx)
        return;
    end
    seed = poreIdx(randi(numel(poreIdx)));
    [sx, sy, sz] = ind2sub(size(model), seed);
    radius = randi([3, 5]);
    [cx, cy, cz] = ind2sub(size(model), solidIdx);
    dist = sqrt((cx - sx).^2 + (cy - sy).^2 + (cz - sz).^2);
    candidates = solidIdx(dist <= radius);
    if isempty(candidates)
        return;
    end
    partner = candidates(randi(numel(candidates)));
    move.linearIdx = [seed; partner];
    move.oldValues = [true; false];
    move.newValues = [false; true];
end

function move = generateBoundaryBirthDeathMove(mcmcState, optParams)
    model = mcmcState.model;
    currentPorosity = mean(model(:));
    targetPorosity = optParams.targetPorosity;
    move = struct('linearIdx', [], 'oldValues', [], 'newValues', []);
    if currentPorosity < targetPorosity
        candidates = find(mcmcState.solidBoundary);
        desiredNewValue = true;
    else
        candidates = find(mcmcState.poreBoundary);
        desiredNewValue = false;
    end
    if isempty(candidates)
        return;
    end
    maxChange = min(5, max(1, round(abs(targetPorosity - currentPorosity) * numel(model))));
    nSelect = min(maxChange, numel(candidates));
    selected = candidates(randperm(numel(candidates), nSelect));
    move.linearIdx = selected(:);
    move.oldValues = model(selected);
    move.newValues = repmat(desiredNewValue, numel(selected), 1);
end

function move = generateRandomFlipMove(model)
    move = struct('linearIdx', [], 'oldValues', [], 'newValues', []);
    idx = randi(numel(model));
    move.linearIdx = idx;
    move.oldValues = model(idx);
    move.newValues = ~model(idx);
end
...
function deltaE = evaluateComprehensiveMoveDelta(mcmcState, move, moveType, ...
    ...
    candidateModel = applyMoveToModel(mcmcState.model, move);
    if ~passesTopologyScreen(candidateModel, mcmcState.optParams)
        deltaE = inf;
        return;
    end

    deltaE = evaluateBaseMoveDelta(mcmcState, move, localStats, candidateModel);
    deltaE = deltaE * (0.98 + 0.04 * randn());
end

function deltaE = evaluateBaseMoveDelta(mcmcState, move, localStats, candidateModel)
    % 评估基础移动能量变化（精确簇约束）
    if isempty(move.linearIdx)
        deltaE = 0;
        return;
    ...
    if nargin < 4 || isempty(candidateModel)
        candidateModel = applyMoveToModel(mcmcState.model, move);
    end
    ...
    % 精确簇能量变化
    currentClusterEnergy = calculateClusterEnergy(mcmcState.features, mcmcState.optParams);
    candidateClusterFeatures = extractEfficientClusterFeatures(candidateModel);
    candidateClusterEnergy = calculateClusterEnergy(candidateClusterFeatures, mcmcState.optParams);
    deltaE_cluster = (candidateClusterEnergy - currentClusterEnergy) * mcmcState.weights.cluster;
    ...
    deltaE_spatial = normalizedSpread * mcmcState.weights.spatial * 0.05;

    deltaE = deltaE_porosity + deltaE_surface + deltaE_cluster + deltaE_spatial;
end

function isValid = passesTopologyScreen(candidateModel, optParams)
    clusterRef = getfieldOrDefault(optParams, 'clusterReference', struct());
    CC = bwconncomp(candidateModel, 26);
    if CC.NumObjects == 0
        isValid = false;
        return;
    end
    ...
    targetNum = max(1, getfieldOrDefault(clusterRef, 'numClusters', numClusters));
    targetMax = max(1, getfieldOrDefault(clusterRef, 'maxSize', maxSize));
    targetMean = max(1, getfieldOrDefault(clusterRef, 'meanSize', meanSize));

    if maxSize > 1.2 * targetMax
        isValid = false;
        return;
    end
    if abs(numClusters - targetNum) > 0.5 * targetNum
        isValid = false;
        return;
    end
    if abs(meanSize - targetMean) > 1.5 * targetMean
        isValid = false;
        return;
    end
    isValid = true;
...
function finalModel = comprehensivePostProcessWithSmallPores(model, optParams)
    % 轻量化后处理：移除极小孤立簇并轻微平滑
    fprintf('执行综合后处理（保留小孔隙版本）...\n');
    model = cleanTinyComponents(model, 3, true);
    model = cleanTinyComponents(model, 3, false);
    model = lightBoundarySmooth(model);
    finalModel = model;
end
function finalModel = comprehensivePostProcess(model, optParams)
    % 轻平滑版本：适度平滑边界但避免破坏拓扑
    fprintf('执行综合后处理（轻平滑版本）...\n');
    model = cleanTinyComponents(model, 3, true);
    model = cleanTinyComponents(model, 3, false);
    model = lightBoundarySmooth(model);
    finalModel = model;
end
function model = cleanTinyComponents(model, maxSize, isPore)
    if nargin < 3
        isPore = true;
    end
    if isPore
    ...
    for i = 1:numel(removeIdx)
        if isPore
            ...
            model(CC.PixelIdxList{removeIdx(i)}) = true;
        end
    end
end

function model = lightBoundarySmooth(model)
    se = strel('cube', 3);
    opened = imopen(model, se);
...
function model = enforcePorosityHardConstraint(model, targetPorosity, maxIters)
    ...
    targetCount = round(targetPorosity * numel(model));
    for iter = 1:maxIters
        poreCount = nnz(model);
        diff = targetCount - poreCount;
        if diff == 0
            break;
        end
        [poreBoundary, solidBoundary] = computeBoundaryMasks(model);
        if diff > 0
            candidates = find(solidBoundary);
            desired = true;
        else
            candidates = find(poreBoundary);
            desired = false;
        end
        if isempty(candidates)
            break;
        end
        step = min(max(1, ceil(abs(diff) / maxIters * 2)), 5);
        select = candidates(randperm(numel(candidates), min(step, numel(candidates))));
        model(select) = desired;
    end
end
%% ========== 核心后处理函数 ==========
...
function checkComprehensiveModelQuality(finalModel, originalModel, optParams)
    ...
    % 1. 形态一致性检查
    fprintf('\n1. 形态一致性：\n');
    fprintf('  最终孔隙率: %.4f (目标 %.4f)\n', mean(finalModel(:)), optParams.targetPorosity);
...
function displayComprehensiveFinalResults(originalModel, finalModel, ...
    ...
    finalSpatial = computeEnhancedSpatialFeatures(finalModel);
    finalMorph = computeDetailedMorphologyFeatures(finalModel);
    finalEnergy = computeUnifiedEnergySnapshot(finalModel, optParams, optParams.energyWeights);
    initialEnergy = getfieldOrDefault(optParams, 'initialEnergy', finalEnergy);
    bestEnergy = getfieldOrDefault(optParams, 'bestEnergy', min(finalEnergy, initialEnergy));
    fprintf('\n1. 基本统计：\n');
    ...
    fprintf('\n6. 优化性能概览：\n');
    fprintf('  初始能量: %.6f | 历史最佳能量: %.6f | 最终能量: %.6f\n', ...
        initialEnergy, bestEnergy, finalEnergy);
    relDrop = (initialEnergy - bestEnergy) / max(initialEnergy, eps);
    fprintf('  能量相对下降: %.2f%%\n', relDrop * 100);
