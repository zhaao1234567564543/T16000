...
% 自适应能量函数权重
weights = struct();
weights.cluster = 0.8; % 大幅降低簇能量的权重，实现软约束
weights.porosity = 3.0;
optParams.unifiedWeights = weights;
weights.morphology = 14.0; % 显著强调形态保持
weights.spatial = 12.0; % 显著强调空间特征匹配
weights.connectivity = 5.0;
weights.multiScale = 4.5;
weights.shapePreservation = 10.0;
weights.structureCoherence = 6.5; % 新增：结构一致性权重
% 初始化MCMC状态
...
function gradientModel = computeSpatialParameterGradients(rawBinaryModel, baseSpatialFeatures, rawPorosity)
    ...
    if nargin < 3 || isempty(rawPorosity)
        rawPorosity = mean(rawBinaryModel(:));
    end
...
function [targetSpatialFeatures, summaryStruct] = predictSpatialFeaturesFromGradients(gradientModel, targetPorosity, baseSpatialFeatures, targetVolume)
    ...
    for i = 1:numel(baseNames)
        ...
        if ~isfield(summaryStruct, name)
            summaryStruct.(name) = gradientModel.baseSummaryStruct.(name);
        end
    end
...
function mcmcModel = generateComprehensiveInitialModel(dims, targetPorosity, ...
    ...
    % 6. 多指标确定性优化
    nOptSteps = 40;
    weights = getUnifiedEnergyWeights(optParams);
    [~, energyState] = computeUnifiedEnergyForModel(model, optParams, weights, false);
    for step = 1:nOptSteps
        currentSpatial = struct();
        currentMorph = struct();
        currentMultiScale = [];
        if isfield(energyState, 'spatialFeatures')
            currentSpatial = energyState.spatialFeatures;
        end
        if isfield(energyState, 'morphologyFeatures')
            currentMorph = energyState.morphologyFeatures;
        end
        if isfield(energyState, 'multiScaleSpatialFeatures')
            currentMultiScale = energyState.multiScaleSpatialFeatures;
        end
        if isempty(currentSpatial)
            currentSpatial = computeEnhancedSpatialFeatures(model);
            energyState.spatialFeatures = currentSpatial;
        end
        if isempty(currentMorph)
            currentMorph = computeDetailedMorphologyFeatures(model);
            energyState.morphologyFeatures = currentMorph;
        end
        if isempty(currentMultiScale)
            currentMultiScale = computeMultiScaleSpatialFeatures(model);
            energyState.multiScaleSpatialFeatures = currentMultiScale;
        end
        ...
        switch primaryTarget
            case 1
                [model, energyState] = applySpatialCorrection(model, energyState, spatialFeatures, optParams, weights);
            case 2
                [model, energyState] = applyMorphologyCorrection(model, energyState, morphologyFeatures, optParams, weights);
            case 3
                [model, energyState] = applyMultiScaleCorrection(model, energyState, optParams.multiScaleSpatialFeatures, optParams, weights);
        end
        candidate = adjustPorosity(model, targetPorosity, densityMap);
        [model, energyState] = updateModelIfImproved(model, candidate, optParams, weights, energyState);
        if ~isempty(densityMap)
            candidate = reinforceDensityTargets(model, densityMap, targetPorosity);
            [model, energyState] = updateModelIfImproved(model, candidate, optParams, weights, energyState);
        end
    end
...
function [model, energyState] = applyMultiScaleCorrection(model, energyState, targetMultiScale, optParams, weights)
    % 针对多尺度差异的能量驱动校正
    ...
    if nargin < 2 || isempty(energyState) || ~isfield(energyState, 'currentEnergy')
        [~, energyState] = computeUnifiedEnergyForModel(model, optParams, weights, false);
    else
        energyState.model = model;
    end
    if nargin < 3 || isempty(targetMultiScale)
        if isfield(optParams, 'multiScaleSpatialFeatures')
    ...
    if isempty(currentMultiScale) || isempty(targetMultiScale) || ...
            ~isfield(targetMultiScale, 'scale')
        return;
    end
    baseEnergy = energyState.currentEnergy;
    baseTopology = summarizeTopology(model);
    ...
    for s = 1:nScales
        cScale = currentMultiScale.scale(s);
        tScale = targetMultiScale.scale(s);
        if ~isfield(cScale, 'clusterDistribution') || ~isfield(tScale, 'clusterDistribution')
            continue;
        end
        currentDensity = getfielddef(cScale.clusterDistribution, 'density', 0);
        desiredDensity = getfielddef(tScale.clusterDistribution, 'density', currentDensity);
        diffVal = desiredDensity - currentDensity;
        if abs(diffVal) < 1e-4
            continue;
        end
        radius = max(1, round(getfielddef(cScale, 'scaleFactor', 2) / 2));
        se = strel('sphere', radius);
        if diffVal > 0
            growthMask = imdilate(candidateModel, se) & ~candidateModel;
            if ~any(growthMask(:))
                continue;
            end
            scores = bwdist(candidateModel);
            candidates = find(growthMask);
            [~, order] = sort(scores(candidates), 'ascend');
            targetCount = min(numel(order), max(1, round(diffVal * numel(candidateModel) * 0.02)));
            toggles = [];
            for k = 1:targetCount
                voxel = candidates(order(k));
                if isLocalToggleSafe(candidateModel, voxel, true)
                    toggles(end+1, 1) = voxel; %#ok<AGROW>
                end
            end
            if isempty(toggles)
                continue;
            end
            candidateModel(toggles) = true;
            changed = true;
        else
            shrinkMask = candidateModel & ~imerode(candidateModel, se);
            if ~any(shrinkMask(:))
                continue;
            end
            scores = bwdist(~candidateModel);
            candidates = find(shrinkMask);
            [~, order] = sort(scores(candidates), 'ascend');
            targetCount = min(numel(order), max(1, round(-diffVal * numel(candidateModel) * 0.02)));
            toggles = [];
            for k = 1:targetCount
                voxel = candidates(order(k));
                if isLocalToggleSafe(candidateModel, voxel, false)
                    toggles(end+1, 1) = voxel; %#ok<AGROW>
                end
            end
            if isempty(toggles)
    ...
    if ~respectsTopologicalConstraints(model, candidateModel, baseTopology)
        return;
    end
    [candidateEnergy, candidateState] = computeUnifiedEnergyForModel(candidateModel, optParams, weights, false);
    if candidateEnergy < baseEnergy - 1e-6
        model = candidateModel;
        energyState = candidateState;
    end
end
end
...
function model = calibrateInitialModelToTargets(model, targetSpatial, targetMorph, optParams, densityMap)
    ...
    weights = getUnifiedEnergyWeights(optParams);
    [~, energyState] = computeUnifiedEnergyForModel(model, optParams, weights, false);
    targetPorosity = getfielddef(optParams, 'targetPorosity', mean(model(:)));
    maxPasses = 6;
    for pass = 1:maxPasses
        currentPorosity = mean(model(:));
        if isfinite(targetPorosity) && abs(currentPorosity - targetPorosity) > 5e-4
            candidate = adjustPorosity(model, targetPorosity, densityMap);
            if ~isempty(densityMap)
                candidate = reinforceDensityTargets(candidate, densityMap, targetPorosity);
            end
            [model, energyState] = updateModelIfImproved(model, candidate, optParams, weights, energyState);
        end
        if nargin >= 2 && ~isempty(targetSpatial)
            [model, energyState] = applySpatialCorrection(model, energyState, targetSpatial, optParams, weights);
        end
        if nargin >= 3 && ~isempty(targetMorph)
            [model, energyState] = applyMorphologyCorrection(model, energyState, targetMorph, optParams, weights);
        end
        if isfield(optParams, 'multiScaleSpatialFeatures') && ~isempty(optParams.multiScaleSpatialFeatures)
            [model, energyState] = applyMultiScaleCorrection(model, energyState, optParams.multiScaleSpatialFeatures, optParams, weights);
        end
        candidate = enforceClusterSizeConstraints(model, optParams);
        [model, energyState] = updateModelIfImproved(model, candidate, optParams, weights, energyState);
        candidate = enforceTargetClusterCount(model, optParams);
        [model, energyState] = updateModelIfImproved(model, candidate, optParams, weights, energyState);
        ...
        morphMatch = 1;
        multiScaleMatch = 1;
        if ~isempty(currentSpatial) && ~isempty(targetSpatial)
        ...
        if isfield(optParams, 'multiScaleSpatialFeatures') && ~isempty(optParams.multiScaleSpatialFeatures)
            multiScaleMatch = calculateMultiScaleMatch(energyState.multiScaleSpatialFeatures, optParams.multiScaleSpatialFeatures);
        end
        if spatialMatch >= 0.95 && morphMatch >= 0.95 && ...
                abs(mean(model(:)) - targetPorosity) < 5e-4 && ...
                (isempty(optParams.multiScaleSpatialFeatures) || multiScaleMatch >= 0.92)
            break;
        end
    end
end
...
function model = applyTargetMorphology(model, morphologyFeatures)
    % 根据目标形态特征在3D域中调整结构，避免二维偏置
    if ~isempty(morphologyFeatures) && isfield(morphologyFeatures, 'sphericity') && ...
        ...
        elseif targetSphericity < 0.4
            ...
            for d = 1:size(directions, 1)
                se = create3DLineStructuringElement(directions(d, :), 2, 1);
...
function [model, energyState] = applySpatialCorrection(model, energyState, targetSpatial, optParams, weights)
    % 应用空间修正，使用统一能量函数与拓扑约束控制局部调整
    if nargin < 4 || isempty(optParams)
        optParams = struct();
    end
    if nargin < 5 || isempty(weights)
    ...
    if nargin < 3 || isempty(targetSpatial)
        if isfield(optParams, 'spatialFeatures')
            targetSpatial = optParams.spatialFeatures;
        else
            targetSpatial = struct();
        end
    end
    if ~isfield(energyState, 'spatialFeatures') || isempty(energyState.spatialFeatures) || ...
            ~isfield(energyState.spatialFeatures, 'twoPointCorr') || ...
            isempty(energyState.spatialFeatures.twoPointCorr) || ...
            ~isfield(targetSpatial, 'twoPointCorr') || isempty(targetSpatial.twoPointCorr)
        return;
    end
    baseState = energyState;
    [patchIdx, newValues, candidateState] = proposeSpatialPatch(model, baseState, targetSpatial, optParams, weights);
    if isempty(patchIdx) || isempty(candidateState) || ...
            candidateState.currentEnergy >= baseState.currentEnergy - 1e-6
        return;
    end
    if isequal(model(patchIdx), newValues)
        return;
    end
    model(patchIdx) = newValues;
    energyState = candidateState;
end
end

function [model, energyState] = applyMorphologyCorrection(model, energyState, targetMorph, optParams, weights)
    % 应用形态学修正，采用3D结构元素并结合统一能量函数筛选候选
    if nargin < 4 || isempty(optParams)
        optParams = struct();
    end
    if nargin < 5 || isempty(weights)
    ...
    if nargin < 3 || isempty(targetMorph)
        if isfield(optParams, 'morphologyFeatures')
    ...
    baseState = energyState;
    currentMorph = struct();
    if isfield(baseState, 'morphologyFeatures')
        currentMorph = baseState.morphologyFeatures;
    end
    candidateModels = {};
    baseTopology = summarizeTopology(model);
    baseEnergy = baseState.currentEnergy;
    if ~isempty(currentMorph) && isfield(currentMorph, 'sphericity') && ...
            ~isempty(targetMorph) && isfield(targetMorph, 'sphericity') && ...
        ...
        if targetSpher - currentSpher > 0.05
            se = strel('sphere', 2);
            candidateModels{end+1} = imopen(imclose(model, se), strel('sphere', 1)); %#ok<AGROW>
        elseif currentSpher - targetSpher > 0.05
            se = strel('sphere', 1);
            candidateModels{end+1} = imopen(model, se); %#ok<AGROW>
        end
    end
    if ~isempty(currentMorph) && isfield(currentMorph, 'elongation') && ...
            ~isempty(targetMorph) && isfield(targetMorph, 'elongation') && ...
        ...
        radiiSet = [3, 2, 2; 2, 3, 2; 2, 2, 3];
        if targetElong - currentElong > 0.2
            for rIdx = 1:size(radiiSet, 1)
                se = createEllipsoidStructuringElement(radiiSet(rIdx, :));
                candidateModels{end+1} = imerode(imdilate(model, se), strel('sphere', 1)); %#ok<AGROW>
            end
        elseif currentElong - targetElong > 0.2
            for rIdx = 1:size(radiiSet, 1)
                se = createEllipsoidStructuringElement(radiiSet(rIdx, :));
                candidateModels{end+1} = imdilate(imerode(model, se), strel('sphere', 1)); %#ok<AGROW>
            end
        end
    end
    ...
    bestState = [];
    bestModel = [];
    for idx = 1:numel(candidateModels)
        candidateModel = candidateModels{idx};
        if isempty(candidateModel) || isequal(candidateModel, model)
            continue;
        end
        if ~respectsTopologicalConstraints(model, candidateModel, baseTopology)
            continue;
        ...
        [candidateEnergy, candidateState] = computeUnifiedEnergyForModel(candidateModel, optParams, weights, false);
        if candidateEnergy < bestEnergy - 1e-6
...
function E_cluster = calculateClusterEnergy(features, optParams)
    ...
    clusterRef = [];
    if isfield(optParams, 'clusterReference')
        clusterRef = optParams.clusterReference;
    end
    ...
    if isempty(clusterRef) || ~isfield(clusterRef, 'meanSize') || clusterRef.meanSize <= 0
        clusterRef.meanSize = mean(sizes);
        clusterRef.stdSize = std(sizes);
        clusterRef.numClusters = features.numClusters;
        clusterRef.minSize = min(sizes);
        clusterRef.maxSize = max(sizes);
        clusterRef.histEdges = linspace(log10(min(sizes)+1), log10(max(sizes)+1), 11);
        clusterRef.histogram = histcounts(log10(sizes+1), clusterRef.histEdges, 'Normalization', 'probability');
    end
    meanPenalty = abs(mean(sizes) - clusterRef.meanSize) / (clusterRef.meanSize + 1);
    stdPenalty = abs(std(sizes) - clusterRef.stdSize) / (clusterRef.stdSize + 1);
    % 与原始簇分布的差异（使用对数尺度直方图）
    currentHist = histcounts(log10(sizes + 1), clusterRef.histEdges, 'Normalization', 'probability');
    if numel(currentHist) ~= numel(clusterRef.histogram)
        currentHist = interp1(linspace(0, 1, numel(currentHist)), currentHist, ...
            linspace(0, 1, numel(clusterRef.histogram)), 'linear', 'extrap');
        currentHist(isnan(currentHist)) = 0;
    end
    histPenalty = sum(abs(currentHist - clusterRef.histogram));
    % 极端情况惩罚：簇过少或过多
    refCount = max(1, clusterRef.numClusters);
    countPenalty = max(0, abs(features.numClusters - refCount) / (refCount * 2));
    if features.numClusters <= 1
        countPenalty = countPenalty + 1.0;
    elseif features.numClusters > refCount * 3
        countPenalty = countPenalty + 0.8;
    end
    % 大小范围偏离惩罚（使用对数比率）
    minRatio = log((min(sizes) + 1) / (clusterRef.minSize + 1));
    maxRatio = log((max(sizes) + 1) / (clusterRef.maxSize + 1));
    rangePenalty = abs(minRatio) + abs(maxRatio);
    % 聚合能量，限制在合理范围
    E_cluster = 0.6 * histPenalty + 0.5 * meanPenalty + 0.4 * stdPenalty + ...
        0.3 * rangePenalty + 0.4 * countPenalty;
    E_cluster = max(0, min(E_cluster, 10));
end
            ...
            if CC.NumObjects > 0
                ...
                for i = 1:nAdjust
                    ...
                    if targetSpher > currentSpher
                        ...
                        % 减少球形度
                        randomDir = randn(1, 3);
                        se = create3DLineStructuringElement(randomDir, 2, 1);
...
function model = connectClusters(model, cluster1, cluster2)
    % 使用加权地质路径连接两个簇，避免直线穿透造成的粗糙喉道
    if isempty(cluster1) || isempty(cluster2)
        return;
    end
    volumeSize = size(model);
    [x1, y1, z1] = ind2sub(volumeSize, cluster1);
    [x2, y2, z2] = ind2sub(volumeSize, cluster2);
    pad = 3;
    minX = max(1, min([x1; x2]) - pad);
    maxX = min(volumeSize(1), max([x1; x2]) + pad);
    minY = max(1, min([y1; y2]) - pad);
    maxY = min(volumeSize(2), max([y1; y2]) + pad);
    minZ = max(1, min([z1; z2]) - pad);
    maxZ = min(volumeSize(3), max([z1; z2]) + pad);
    subModel = model(minX:maxX, minY:maxY, minZ:maxZ);
    subSize = size(subModel);
    mask1 = false(subSize);
    mask2 = false(subSize);
    idx1 = sub2ind(subSize, x1 - minX + 1, y1 - minY + 1, z1 - minZ + 1);
    idx2 = sub2ind(subSize, x2 - minX + 1, y2 - minY + 1, z2 - minZ + 1);
    mask1(idx1) = true;
    mask2(idx2) = true;
    surface1 = bwperim(mask1, 26);
    surface2 = bwperim(mask2, 26);
    if ~any(surface1(:))
        surface1 = mask1;
    end
    if ~any(surface2(:))
    ...
    [minVal, startIdx] = min(totalCost(:));
    if ~isfinite(minVal)
        return;
    end
    pathMask = traceGeodesicPath(dist1, startIdx) | traceGeodesicPath(dist2, startIdx);
    if ~any(pathMask(:))
        return;
    end
    pathMask = imdilate(pathMask, strel('sphere', 1));
    smoothPath = smooth3(double(pathMask), 'box', 3) > 0.2;
    candidateSubModel = subModel | smoothPath;
    candidateModel = model;
    candidateModel(minX:maxX, minY:maxY, minZ:maxZ) = candidateSubModel;
    baseTopology = summarizeTopology(model);
    tolerance = struct('largestLoss', 0.2, 'clusterIncrease', 3, 'smallIncrease', 5);
    if respectsTopologicalConstraints(model, candidateModel, baseTopology, tolerance)
        model = candidateModel;
    end
end
end
...
function [move, moveType] = dispatchComprehensiveMove(model, strategy, optParams, lookupTables)
    ...
    switch strategy
        ...
        case 'spatial_aware'
            move = generateSpatialAwareMove(model, lookupTables, optParams);
        ...
        case 'anisotropy_adjust'
            move = generateAnisotropyAdjustMove(model, lookupTables, optParams);
...
function move = generateSpatialAwareMove(model, lookupTables, optParams)
    % 基于梯度的空间感知移动，仅在表面进行拓扑安全调整
    if nargin < 3 || isempty(optParams)
        optParams = struct();
    end
    move = struct('linearIdx', [], 'oldValues', [], 'newValues', []);
    if ~isfield(lookupTables, 'spatialGradient') || isempty(lookupTables.spatialGradient)
    ...
    gradValues = gradientMap(boundary);
    threshold = quantile(double(gradValues), 0.8);
    candidateMask = boundary & (gradientMap >= threshold);
    candidateIdx = find(candidateMask);
    if isempty(candidateIdx)
        move = generateLocalMove(model, 4);
        return;
    end
    [~, order] = sort(gradientMap(candidateIdx), 'descend');
    candidateIdx = candidateIdx(order);
    maxCandidates = min(12, numel(candidateIdx));
    matrixBoundary = bwperim(~model, 26);
    baseTopology = summarizeTopology(model);
    globalPorosity = mean(model(:));
    for idx = 1:maxCandidates
        center = candidateIdx(idx);
        patchIdx = extractPatchIndices(center, size(model), 2);
        if isempty(patchIdx)
            continue;
        end
        localValues = model(patchIdx);
        localPorosity = mean(localValues);
        if localPorosity < globalPorosity
            mode = 'grow';
            candidateVoxels = patchIdx(~model(patchIdx) & matrixBoundary(patchIdx));
        else
            mode = 'thin';
            candidateVoxels = patchIdx(model(patchIdx) & boundary(patchIdx));
        end
        if isempty(candidateVoxels)
            continue;
        end
        voxelScores = gradientMap(candidateVoxels);
        if strcmp(mode, 'grow')
            [~, localOrder] = sort(voxelScores, 'descend');
        else
            [~, localOrder] = sort(voxelScores, 'ascend');
        end
        toggles = [];
        newVals = [];
        maxOps = min(6, numel(localOrder));
        for k = 1:maxOps
            voxel = candidateVoxels(localOrder(k));
            desired = strcmp(mode, 'grow');
            if isLocalToggleSafe(model, voxel, desired)
                toggles(end+1, 1) = voxel; %#ok<AGROW>
                newVals(end+1, 1) = desired; %#ok<AGROW>
            end
        end
        if isempty(toggles)
            continue;
        ...
        candidateModel = model;
        candidateModel(toggles) = newVals;
        if ~respectsTopologicalConstraints(model, candidateModel, baseTopology)
            continue;
        end
        move.linearIdx = toggles;
        move.oldValues = model(toggles);
        move.newValues = newVals;
        return;
    end
    move = generateLocalMove(model, 4);
end
...
function move = generateSpatialCorrelationMove(model, optParams)
    ...
    [patchIdx, newValues, candidateState] = proposeSpatialPatch(model, energyState, targetSpatial, optParams, weights);
    if isempty(patchIdx) || isempty(candidateState)
        move = generateLocalMove(model, 4);
        return;
    end
    if candidateState.currentEnergy >= energyState.currentEnergy - 1e-6
        move = generateLocalMove(model, 4);
        return;
    end
    move.linearIdx = patchIdx;
    move.oldValues = model(patchIdx);
    move.newValues = newValues;
end
end

function move = generateAnisotropyAdjustMove(model, lookupTables, optParams)
    % 生成各向异性调整移动，限制在拓扑安全的表面区域
    if nargin < 3 || isempty(optParams)
        optParams = struct();
    end
    move = struct('linearIdx', [], 'oldValues', [], 'newValues', []);
    if ~isfield(lookupTables, 'localAnisotropy') || isempty(lookupTables.localAnisotropy) || ...
            ~isfield(lookupTables, 'targetSpatialSignature')
        move = generateLocalMove(model, 3);
        return;
    end
    anisoMap = lookupTables.localAnisotropy;
    targetSignature = lookupTables.targetSpatialSignature;
    targetAniso = targetSignature.anisotropy;
    boundary = bwperim(model, 26);
    matrixBoundary = bwperim(~model, 26);
    difference = anisoMap - targetAniso;
    if targetAniso > mean(anisoMap(:))
        candidateMask = matrixBoundary & (difference < -0.05);
        mode = 'grow';
    else
        candidateMask = boundary & (difference > 0.05);
        mode = 'thin';
    end
    candidateIdx = find(candidateMask);
    if isempty(candidateIdx)
    ...
    [~, order] = sort(abs(difference(candidateIdx)), 'descend');
    candidateIdx = candidateIdx(order);
    baseTopology = summarizeTopology(model);
    maxCandidates = min(10, numel(candidateIdx));
    for idx = 1:maxCandidates
        center = candidateIdx(idx);
        patchIdx = extractPatchIndices(center, size(model), 2);
        if isempty(patchIdx)
        ...
        voxelScores = abs(difference(candidateVoxels));
        [~, localOrder] = sort(voxelScores, 'descend');
        toggles = [];
        newVals = [];
        maxOps = min(5, numel(localOrder));
        for k = 1:maxOps
            voxel = candidateVoxels(localOrder(k));
            desired = strcmp(mode, 'grow');
            if isLocalToggleSafe(model, voxel, desired)
...
function model = localMorphologyPerturbation(model, optParams)
    ...
    if targetSphericity > 0.6
        ...
        se = createEllipsoidStructuringElement([3, 2, 2]);
        adjusted = imdilate(imerode(clusterMask, se), strel('sphere', 1));
    end
...
function weights = getUnifiedEnergyWeights(optParams)
    if nargin < 1 || isempty(optParams)
        weights = struct('cluster', 0.8, 'porosity', 3.0, 'morphology', 14.0, ...
            'spatial', 12.0, 'connectivity', 5.0, 'multiScale', 4.5, ...
            'shapePreservation', 10.0, 'structureCoherence', 6.5);
        return;
    end
    if isfield(optParams, 'unifiedWeights') && ~isempty(optParams.unifiedWeights)
        weights = optParams.unifiedWeights;
        return;
    end
    weights = struct('cluster', 0.8, 'porosity', 3.0, 'morphology', 14.0, ...
        'spatial', 12.0, 'connectivity', 5.0, 'multiScale', 4.5, ...
...
function value = getfielddef(S, fieldName, defaultValue)
...
function [energy, state] = computeUnifiedEnergyForModel(model, optParams, weights, parallelHint)
    state = struct('model', logical(model), 'optParams', optParams, 'weights', weights);
    state = updateAllFeatures(state, parallelHint);
    energy = state.currentEnergy;
end
function [model, state, improved] = updateModelIfImproved(model, candidateModel, optParams, weights, baseState, tolerance)
    if nargin < 6 || isempty(tolerance)
        tolerance = 1e-6;
    end
    if nargin < 5 || isempty(baseState)
        [baseEnergy, baseState] = computeUnifiedEnergyForModel(model, optParams, weights, false);
    else
        baseEnergy = baseState.currentEnergy;
    end
    improved = false;
    ...
    [candidateEnergy, candidateState] = computeUnifiedEnergyForModel(candidateModel, optParams, weights, false);
    if candidateEnergy < baseEnergy - tolerance
        model = candidateModel;
...
function topo = summarizeTopology(model)
    topo = struct('numClusters', 0, 'largestCluster', 0, 'smallClusters', 0, ...
        'percolation', struct('x', false, 'y', false, 'z', false));
    CC = bwconncomp(model, 26);
    topo.numClusters = CC.NumObjects;
    if CC.NumObjects == 0
        return;
    end
    sizes = cellfun(@numel, CC.PixelIdxList);
    topo.largestCluster = max(sizes);
    topo.smallClusters = sum(sizes < 10);
    topo.percolation = computePercolationFlags(CC, size(model));
end
function flags = computePercolationFlags(CC, dims)
    flags = struct('x', false, 'y', false, 'z', false);
    if CC.NumObjects == 0
        return;
    end
    for c = 1:CC.NumObjects
        idx = CC.PixelIdxList{c};
        [cx, cy, cz] = ind2sub(dims, idx);
        if ~flags.x && min(cx) == 1 && max(cx) == dims(1)
            flags.x = true;
        end
        if ~flags.y && min(cy) == 1 && max(cy) == dims(2)
            flags.y = true;
        end
        if ~flags.z && min(cz) == 1 && max(cz) == dims(3)
            flags.z = true;
        end
        if flags.x && flags.y && flags.z
            break;
        end
    end
end
function ok = respectsTopologicalConstraints(oldModel, newModel, baseTopo, tolerance)
    if nargin < 3 || isempty(baseTopo)
        baseTopo = summarizeTopology(oldModel);
    end
    if nargin < 4 || isempty(tolerance)
        tolerance = struct('largestLoss', 0.12, 'clusterIncrease', 3, 'smallIncrease', 4);
    end
    newTopo = summarizeTopology(newModel);
    if newTopo.largestCluster < baseTopo.largestCluster * (1 - tolerance.largestLoss)
        ok = false;
        return;
    end
    if newTopo.numClusters > baseTopo.numClusters + tolerance.clusterIncrease
        ok = false;
        return;
    end
    if newTopo.smallClusters > baseTopo.smallClusters + tolerance.smallIncrease
        ok = false;
        return;
    end
    axes = fieldnames(baseTopo.percolation);
    for i = 1:numel(axes)
        axisName = axes{i};
        if baseTopo.percolation.(axisName) && ~newTopo.percolation.(axisName)
            ok = false;
            return;
        end
    end
...
function idx = extractPatchIndices(centerIdx, volumeSize, radius)
    [cx, cy, cz] = ind2sub(volumeSize, centerIdx);
    xRange = max(1, cx-radius):min(volumeSize(1), cx+radius);
    yRange = max(1, cy-radius):min(volumeSize(2), cy+radius);
    zRange = max(1, cz-radius):min(volumeSize(3), cz+radius);
    [X, Y, Z] = ndgrid(xRange, yRange, zRange);
    idx = unique(sub2ind(volumeSize, X(:), Y(:), Z(:)));
end
function neighborIdx = getNeighborIndices(volumeSize, centerIdx)
    [cx, cy, cz] = ind2sub(volumeSize, centerIdx);
    neighborIdx = [];
    for dx = -1:1
        for dy = -1:1
            for dz = -1:1
                ...
                ny = cy + dy;
                nz = cz + dz;
                if nx >= 1 && nx <= volumeSize(1) && ...
                        ny >= 1 && ny <= volumeSize(2) && ...
                        nz >= 1 && nz <= volumeSize(3)
                    neighborIdx(end+1, 1) = sub2ind(volumeSize, nx, ny, nz); %#ok<AGROW>
                end
            end
        end
    end
end
function safe = isLocalToggleSafe(model, idx, newValue)
    dims = size(model);
    [cx, cy, cz] = ind2sub(dims, idx);
    xRange = max(1, cx-1):min(dims(1), cx+1);
    yRange = max(1, cy-1):min(dims(2), cy+1);
    zRange = max(1, cz-1):min(dims(3), cz+1);
    local = model(xRange, yRange, zRange);
    localSize = size(local);
    localIdx = sub2ind(localSize, cx - xRange(1) + 1, cy - yRange(1) + 1, cz - zRange(1) + 1);
    if local(localIdx) == newValue
        safe = true;
        return;
    end
    neighbors = getNeighborIndices(localSize, localIdx);
    if newValue
        safe = any(local(neighbors));
        return;
    end
    if sum(local(neighbors)) < 2
        safe = false;
        return;
    end
    original = local(localIdx);
    local(localIdx) = false;
    CC = bwconncomp(local, 26);
    if CC.NumObjects <= 1
        safe = true;
        local(localIdx) = original;
        return;
    end
    labels = zeros(localSize);
    for c = 1:CC.NumObjects
        labels(CC.PixelIdxList{c}) = c;
    end
    neighborLabels = labels(neighbors);
    neighborLabels = unique(neighborLabels(neighborLabels > 0));
    safe = numel(neighborLabels) <= 1;
    local(localIdx) = original;
end
function [patchIdx, newValues, bestState] = proposeSpatialPatch(model, energyState, targetSpatial, optParams, weights)
    patchIdx = [];
    newValues = [];
    bestState = [];
    if nargin < 5 || isempty(weights)
    ...
    if isempty(currentSpatial) || ~isfield(currentSpatial, 'twoPointCorr') || isempty(currentSpatial.twoPointCorr)
        return;
    end
    if nargin < 3 || isempty(targetSpatial)
        if isfield(optParams, 'spatialFeatures')
    ...
    if ~isfield(targetSpatial, 'twoPointCorr') || isempty(targetSpatial.twoPointCorr)
        return;
    end
    currentTPC = currentSpatial.twoPointCorr;
    targetTPC = targetSpatial.twoPointCorr;
    ...
    diffVec = currentMean - targetMean;
    if isempty(diffVec)
        return;
    end
    if isfield(targetSpatial, 'keyDistances') && ~isempty(targetSpatial.keyDistances)
        distances = targetSpatial.keyDistances(:);
    elseif isfield(optParams, 'spatialFeatures') && isfield(optParams.spatialFeatures, 'keyDistances')
        distances = optParams.spatialFeatures.keyDistances(:);
    else
        distances = (1:length(diffVec))';
    end
    distances = distances(1:min(length(distances), length(diffVec)));
    ...
    kernel = exp(- (distances / maxDist).^2);
    signedError = sum(diffVec(1:length(kernel)) .* kernel);
    if abs(signedError) < 1e-4
        return;
    end
    mode = 'grow';
    if signedError > 0
    ...
    boundary = bwperim(model, 26);
    matrixBoundary = bwperim(~model, 26);
    poreDistance = bwdist(~model);
    matrixDistance = bwdist(model);
    [candidateList, ~] = selectSpatialCandidates(boundary, matrixBoundary, poreDistance, matrixDistance, mode);
    if isempty(candidateList)
        return;
    end
    baseTopo = summarizeTopology(model);
    baseEnergy = energyState.currentEnergy;
    maxCandidates = min(20, numel(candidateList));
    bestEnergy = baseEnergy;
    bestState = [];
    for idx = 1:maxCandidates
        center = candidateList(idx);
        [candidateIdx, candidateValues] = buildSpatialPatch(model, center, mode, boundary, matrixBoundary, poreDistance, matrixDistance);
        if isempty(candidateIdx)
            continue;
        end
        candidateModel = model;
        candidateModel(candidateIdx) = candidateValues;
        if ~respectsTopologicalConstraints(model, candidateModel, baseTopo)
            continue;
        end
        [candidateEnergy, candidateState] = computeUnifiedEnergyForModel(candidateModel, optParams, weights, false);
        if candidateEnergy < bestEnergy - 1e-6
            bestEnergy = candidateEnergy;
            patchIdx = candidateIdx;
            newValues = candidateValues;
            bestState = candidateState;
        end
    end
end
function [candidateList, activeMask] = selectSpatialCandidates(boundary, matrixBoundary, poreDistance, matrixDistance, mode)
    if strcmp(mode, 'thin')
        candidateIdx = find(boundary);
        if isempty(candidateIdx)
            candidateList = [];
            activeMask = boundary;
            return;
        end
        scores = poreDistance(candidateIdx);
        [~, order] = sort(scores, 'ascend');
        candidateList = candidateIdx(order);
        activeMask = boundary;
    else
        candidateIdx = find(matrixBoundary);
        if isempty(candidateIdx)
            candidateList = [];
            activeMask = matrixBoundary;
            return;
        end
        scores = matrixDistance(candidateIdx);
        [~, order] = sort(scores, 'ascend');
        candidateList = candidateIdx(order);
        activeMask = matrixBoundary;
    end
end
function [toggleIdx, newValues] = buildSpatialPatch(model, centerIdx, mode, boundary, matrixBoundary, poreDistance, matrixDistance)
    patchIdx = extractPatchIndices(centerIdx, size(model), 2);
    toggleIdx = [];
    newValues = [];
    if isempty(patchIdx)
        return;
    ...
    if strcmp(mode, 'thin')
        ...
        scores = poreDistance(candidateVoxels);
        [~, order] = sort(scores, 'ascend');
        maxOps = min(8, numel(order));
        for k = 1:maxOps
            voxel = candidateVoxels(order(k));
            if isLocalToggleSafe(model, voxel, false)
                toggleIdx(end+1, 1) = voxel; %#ok<AGROW>
                newValues(end+1, 1) = false; %#ok<AGROW>
            end
        end
    else
        candidateVoxels = patchIdx(~model(patchIdx) & matrixBoundary(patchIdx));
        ...
        scores = matrixDistance(candidateVoxels);
        [~, order] = sort(scores, 'ascend');
        maxOps = min(8, numel(order));
        for k = 1:maxOps
            voxel = candidateVoxels(order(k));
            if isLocalToggleSafe(model, voxel, true)
                toggleIdx(end+1, 1) = voxel; %#ok<AGROW>
                newValues(end+1, 1) = true; %#ok<AGROW>
            end
        end
    end
end
function pathMask = traceGeodesicPath(distanceMap, startIdx)
    pathMask = false(size(distanceMap));
    if ~isfinite(distanceMap(startIdx))
        return;
    end
    currentIdx = startIdx;
    currentDist = distanceMap(currentIdx);
    while isfinite(currentDist) && currentDist > 0
        pathMask(currentIdx) = true;
        neighbors = getNeighborIndices(size(distanceMap), currentIdx);
        neighborDistances = distanceMap(neighbors);
        [minVal, minIdx] = min(neighborDistances);
        if ~isfinite(minVal) || minVal >= currentDist
            break;
        end
        currentIdx = neighbors(minIdx);
        currentDist = minVal;
    end
    pathMask(currentIdx) = true;
end
function se = createEllipsoidStructuringElement(radii)
    radii = max(1, round(radii));
    [x, y, z] = ndgrid(-radii(1):radii(1), -radii(2):radii(2), -radii(3):radii(3));
    mask = (x ./ radii(1)).^2 + (y ./ radii(2)).^2 + (z ./ radii(3)).^2 <= 1;
    se = strel('arbitrary', mask);
end
function se = create3DLineStructuringElement(direction, halfLength, thickness)
    if nargin < 1 || isempty(direction)
        direction = [1, 0, 0];
    end
    if nargin < 2 || isempty(halfLength)
        halfLength = 2;
    end
    ...
    direction = direction / norm(direction);
    halfLength = max(1, round(halfLength));
    thickness = max(1, round(thickness));
    radius = max(halfLength, thickness);
    [x, y, z] = ndgrid(-radius:radius, -radius:radius, -radius:radius);
    parallel = direction(1) * x + direction(2) * y + direction(3) * z;
    perp = sqrt((x - parallel * direction(1)).^2 + (y - parallel * direction(2)).^2 + (z - parallel * direction(3)).^2);
    mask = (abs(parallel) <= halfLength + 0.25) & (perp <= thickness + 0.25);
    if ~any(mask(:))
        mask(radius+1, radius+1, radius+1) = true;
    end
    se = strel('arbitrary', mask);
end
function normVals = normalizeVolume(vals)
    if isempty(vals)
        normVals = vals;
        return;
    end
