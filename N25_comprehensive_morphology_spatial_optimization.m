...
baseEnergyWeights = struct();
baseEnergyWeights.cluster = 0.8;
baseEnergyWeights.porosity = 3.0;
baseEnergyWeights.morphology = 14.0;
baseEnergyWeights.spatial = 12.0;
baseEnergyWeights.connectivity = 5.0;
baseEnergyWeights.multiScale = 4.5;
baseEnergyWeights.shapePreservation = 10.0;
baseEnergyWeights.structureCoherence = 6.5;
optParams.energyWeights = baseEnergyWeights;
% 生成快速查找表
fprintf('正在生成快速查找表...\n');
...
stallCounter = 0;
stallLimit = max(ceil(maxIterations * 0.25), 400);
energyImprovementTol = 1e-8;
for iter = 1:maxIterations
    ...
    if mod(iter, 40) == 0
        ...
        stallCounter = max(0, floor(stallCounter * 0.5));
    end
    
    % 自适应选择移动策略
    moveStrategy = selectComprehensiveAdaptiveMoveStrategy(iter, maxIterations, ...
        mcmcState, optimization_phase);
    prevBestEnergy = mcmcState.bestEnergy;
    prevBestMorphMatch = mcmcState.bestMorphMatch;
    prevBestSpatialMatch = mcmcState.bestSpatialMatch;
    % 生成批量候选移动
    ...
    if accepted
        stallCounter = 0;
        mcmcState.lastAcceptedIter = iter;
    else
        bestEnergyImproved = mcmcState.bestEnergy < prevBestEnergy - energyImprovementTol;
        bestMorphImproved = mcmcState.bestMorphMatch > prevBestMorphMatch + matchTolerance;
        bestSpatialImproved = mcmcState.bestSpatialMatch > prevBestSpatialMatch + matchTolerance;
        if bestEnergyImproved || bestMorphImproved || bestSpatialImproved
            stallCounter = 0;
        else
    ...
    if stallCounter >= stallLimit
        lastAcceptGap = iter - mcmcState.lastAcceptedIter;
        fprintf('  长时间无进展 (超过 %d 次迭代)，提前结束MCMC优化。(距上次接受 %d 次迭代)\n', ...
            stallLimit, max(lastAcceptGap, 0));
        finalIteration = iter;
        break;
    end
    % 记录优化历史
    ...
    if mod(iter, 200) == 0
        mcmcState = comprehensiveAdaptiveAdjustment(mcmcState, performanceMonitor, ...
         ...
         stallCounter = max(0, stallCounter - max(5, round(stallLimit * 0.1)));
    end
end
executedIterations = min(finalIteration, iter);
executedIterations = max(1, executedIterations);
totalTime = toc(startTime);
fprintf('MCMC优化完成，总用时：%.2f秒\n', totalTime);
fprintf('平均每次迭代用时：%.4f秒\n', totalTime/executedIterations);
%% 6. 综合后处理
fprintf('正在进行综合后处理...\n');
...
function mcmcModel = generateComprehensiveInitialModel(dims, targetPorosity, ...
    ...
    for step = 1:nOptSteps
        ...
        switch primaryTarget
            case 1
                model = applySpatialCorrection(model, currentSpatial, spatialFeatures, optParams);
            case 2
...
function model = calibrateInitialModelToTargets(model, targetSpatial, targetMorph, optParams, densityMap)
    ...
    baselineEnergy = computeUnifiedEnergy(model, optParams);
    for pass = 1:maxPasses
        ...
        if abs(currentPorosity - targetPorosity) > 5e-4
            ...
            candidateEnergy = computeUnifiedEnergy(candidate, optParams);
            if candidateEnergy < baselineEnergy
                model = candidate;
                baselineEnergy = candidateEnergy;
                improved = true;
            end
        end
        currentSpatial = computeEnhancedSpatialFeatures(model);
        ...
        if spatialMatch < 0.93
            candidate = applySpatialCorrection(model, currentSpatial, targetSpatial, optParams);
            candidateEnergy = computeUnifiedEnergy(candidate, optParams);
            if candidateEnergy < baselineEnergy
                model = candidate;
                baselineEnergy = candidateEnergy;
        ...
        if morphMatch < 0.93
            candidate = applyMorphologyCorrection(model, currentMorph, targetMorph);
            candidateEnergy = computeUnifiedEnergy(candidate, optParams);
            if candidateEnergy < baselineEnergy
                model = candidate;
                baselineEnergy = candidateEnergy;
        ...
        if ~isempty(currentMultiScale) && multiScaleMatch < 0.9
            candidate = applyMultiScaleCorrection(model, currentMultiScale, ...
                optParams.multiScaleSpatialFeatures);
            candidateEnergy = computeUnifiedEnergy(candidate, optParams);
            if candidateEnergy < baselineEnergy
                model = candidate;
...
function model = applyTargetMorphology(model, morphologyFeatures)
    ...
    if ~isempty(morphologyFeatures.sphericity)
        ...
        if targetSphericity > 0.7
            % 高球形度：使用球形结构元素
            se = createEllipsoidalStructuringElement(2, 2, 2);
            model = applySurfaceLimitedMorphology(model, se, 'open');
            model = applySurfaceLimitedMorphology(model, se, 'close');
        elseif targetSphericity < 0.4
            % 低球形度：沿多个方向的3D细线形结构元素
            principalAxes = eye(3);
            for axisIdx = 1:3
                se = createDirectionalStructuringElement(principalAxes(axisIdx, :), 7, 1);
                model = applySurfaceLimitedMorphology(model, se, 'open');
            end
        else
            % 中等球形度：轻微各向异性操作
            se = createEllipsoidalStructuringElement(1, 1, 1);
            model = applySurfaceLimitedMorphology(model, se, 'open');
        end
    end
end
function model = adjustClusterSizeDistribution(model, optParams)
    ...
    cluster1 = normalizeClusterIndices(cluster1);
    cluster2 = normalizeClusterIndices(cluster2);
...
function model = applySpatialCorrection(model, currentSpatial, targetSpatial, optParams)
    % 应用空间修正，避免简单的随机翻转或粗暴生长
    % 通过能量驱动的边界体素调整，使两点相关函数逐渐逼近目标
    if nargin < 4
        optParams = struct();
    end
    ...
    if isfield(targetSpatial, 'keyDistances')
    ...
    elseif isfield(optParams, 'spatialFeatures') && isfield(optParams.spatialFeatures, 'keyDistances')
        ...
        keyDistances = (1:minRows)';
    end
    % 使用高斯核对两点相关差异进行加权，以强调短程偏差
    sigma = max(1, median(keyDistances));
    weights = exp(-0.5 * (keyDistances(1:minRows) / sigma).^2);
    weights = weights / max(sum(weights), eps);
    diffVector = mean(currentTPC - targetTPC, 2);
    weightedDiff = sum(diffVector .* weights(1:minRows));
    tolerance = 5e-3;
    if abs(weightedDiff) < tolerance
        return;
    end
    % 仅在拓扑安全的边界区域进行调整
    ...
    if weightedDiff > 0
        % 相关性过高 -> 需要轻微侵蚀
        candidateMask = boundaryMask;
        desiredState = false;
    else
        % 相关性过低 -> 需要受限膨胀
        candidateMask = bwperim(~model, 26);
        desiredState = true;
    end
    ...
    % 根据局部各向异性与梯度排序，优先在平坦区域操作
    localGrad = computeLocalGradientNorm(model, candidateIdx);
    [~, order] = sort(localGrad, 'ascend');
    candidateIdx = candidateIdx(order);
    maxAdjust = min(120, numel(candidateIdx));
    % 预先计算统一能量，确保与MCMC一致
    baseEnergy = computeUnifiedEnergy(model, optParams);
    accepted = 0;
    for n = 1:maxAdjust
        idx = candidateIdx(n);
        if model(idx) == desiredState
            continue;
        end
        if ~isTopologicallySafeToggle(model, idx, desiredState)
            continue;
        end
        trialModel = model;
        trialModel(idx) = desiredState;
        if ~isSurfaceConsistent(trialModel, idx)
            continue;
        end
        trialEnergy = computeUnifiedEnergy(trialModel, optParams);
        if trialEnergy < baseEnergy - 1e-5
            model = trialModel;
            baseEnergy = trialEnergy;
            accepted = accepted + 1;
        end
        if accepted >= 15 && abs(weightedDiff) < tolerance * 0.2
            break;
        end
    end
    % 使用轻度各向同性滤波保持表面光滑
    if accepted > 0
        smoothingSe = createEllipsoidalStructuringElement(1, 1, 1);
        model = imclose(imopen(model, smoothingSe), smoothingSe);
    end
end
function model = applyMorphologyCorrection(model, currentMorph, targetMorph)
    % 应用形态学修正，使用严格的3D操作并约束在界面区域
    if nargin < 3 || isempty(targetMorph)
        return;
    end
    boundaryMask = imdilate(bwperim(model, 26), ones(3, 3, 3));
    % 球形度修正
    if ~isempty(currentMorph.sphericity) && ~isempty(targetMorph.sphericity)
        ...
        if abs(currentSpher - targetSpher) > 0.05
            if targetSpher > currentSpher
                se = createEllipsoidalStructuringElement(1, 1, 1);
                candidate = imclose(model, se);
            else
                dirSe = createDirectionalStructuringElement([1, 0, 0], 5, 1);
                candidate = imerode(model, dirSe);
                dirSe = createDirectionalStructuringElement([0, 1, 0], 5, 1);
                candidate = imdilate(candidate, dirSe);
            end
            model(boundaryMask) = candidate(boundaryMask);
        end
    end
    % 伸长率修正
    if ~isempty(currentMorph.elongation) && ~isempty(targetMorph.elongation)
        ...
        if abs(currentElong - targetElong) > 0.3
            principalDirs = estimatePrincipalDirections(model);
            majorDir = principalDirs(:, 3);
            minorDir1 = principalDirs(:, 1);
            minorDir2 = principalDirs(:, 2);
            if targetElong > currentElong
                elongSe = createDirectionalStructuringElement(majorDir', 7, 1);
                candidate = imdilate(model, elongSe);
                candidate = applySurfaceLimitedMorphology(candidate, createDirectionalStructuringElement(minorDir1', 3, 1), 'erode');
                candidate = applySurfaceLimitedMorphology(candidate, createDirectionalStructuringElement(minorDir2', 3, 1), 'erode');
            else
                smoothingSe = createEllipsoidalStructuringElement(1, 1, 1);
                candidate = imclose(model, smoothingSe);
            end
            model(boundaryMask) = candidate(boundaryMask);
        end
    end
...
function gradVals = computeLocalGradientNorm(model, linearIdx)
    % 计算候选位置的局部梯度范数，用于排序
    if isempty(linearIdx)
        gradVals = [];
        return;
    end
    smoothed = imgaussfilt3(double(model), 1.0);
    [gx, gy, gz] = gradient(smoothed);
    gradMag = sqrt(gx.^2 + gy.^2 + gz.^2);
    gradVals = gradMag(linearIdx);
end
function safe = isTopologicallySafeToggle(model, linearIdx, desiredState)
    % 判断在指定体素位置翻转是否保持拓扑连通
    if model(linearIdx) == desiredState
        safe = true;
        return;
    end
    volumeSize = size(model);
    [x, y, z] = ind2sub(volumeSize, linearIdx);
    neighborIdx = getNeighborIndices([x, y, z], volumeSize);
    neighborValues = model(neighborIdx);
    if desiredState
        if ~any(neighborValues)
            safe = false;
            return;
        end
    else
    ...
    originalState = model(linearIdx);
    originalCC = bwconncomp(model, 26);
    originalNum = originalCC.NumObjects;
    largestBefore = 0;
    if originalNum > 0
        sizes = cellfun(@numel, originalCC.PixelIdxList);
        largestBefore = max(sizes);
    end
    model(linearIdx) = desiredState;
    newCC = bwconncomp(model, 26);
    model(linearIdx) = originalState;
    newNum = newCC.NumObjects;
    if desiredState
        safe = newNum <= originalNum + 1;
        if safe && newNum > 0 && largestBefore > 0
            newSizes = cellfun(@numel, newCC.PixelIdxList);
            largestAfter = max(newSizes);
            safe = largestAfter >= 0.5 * largestBefore;
        end
    else
        ...
        if largestBefore > 0 && newNum > 0
            newSizes = cellfun(@numel, newCC.PixelIdxList);
            largestAfter = max(newSizes);
            safe = largestAfter >= 0.7 * largestBefore;
        else
            safe = newNum <= originalNum;
        end
    end
end
function ok = isSurfaceConsistent(model, linearIdx)
    % 判断修改后的体素是否仍位于界面附近，避免产生孤立孔或尖刺
    volumeSize = size(model);
    [x, y, z] = ind2sub(volumeSize, linearIdx);
    radius = 1;
    xr = max(1, x-radius):min(volumeSize(1), x+radius);
    yr = max(1, y-radius):min(volumeSize(2), y+radius);
    zr = max(1, z-radius):min(volumeSize(3), z+radius);
    local = model(xr, yr, zr);
    fillRatio = mean(local(:));
    ok = fillRatio > 0 && fillRatio < 1;
end
function se = createEllipsoidalStructuringElement(rx, ry, rz)
    % 构造3D椭球结构元素
    if nargin < 3
        rz = rx;
    end
    ...
    mask = (X.^2 / (rx^2 + eps)) + (Y.^2 / (ry^2 + eps)) + (Z.^2 / (rz^2 + eps)) <= 1;
    se = strel('arbitrary', mask);
end
function se = createDirectionalStructuringElement(direction, len, thickness)
    % 创建沿指定方向的3D细线结构元素
    if nargin < 3
        thickness = 1;
    end
    ...
    len = max(3, round(len));
    halfLen = (len - 1) / 2;
    span = ceil(halfLen + thickness + 1);
    [X, Y, Z] = ndgrid(-span:span, -span:span, -span:span);
    points = [X(:), Y(:), Z(:)];
    proj = points * dir;
    closest = proj .* dir';
    diff = points - closest;
    radialDist = sqrt(sum(diff.^2, 2));
    mask = radialDist <= thickness & abs(proj) <= halfLen + thickness;
    mask = reshape(mask, size(X));
    se = strel('arbitrary', mask);
end
function dirs = estimatePrincipalDirections(model)
    % 根据体素点云估计主方向
    coords = ind2submat(size(model), find(model));
    if size(coords, 1) < 3
        dirs = eye(3);
        return;
    end
...
function out = ind2submat(volumeSize, idx)
    % 将线性索引转换为坐标矩阵
    [x, y, z] = ind2sub(volumeSize, idx);
    out = double([x, y, z]);
end
function neighborIdx = getNeighborIndices(coord, volumeSize)
    % 获取26邻域索引
    x = coord(1); y = coord(2); z = coord(3);
    xr = max(1, x-1):min(volumeSize(1), x+1);
    yr = max(1, y-1):min(volumeSize(2), y+1);
    zr = max(1, z-1):min(volumeSize(3), z+1);
    [X, Y, Z] = ndgrid(xr, yr, zr);
    neighborIdx = sub2ind(volumeSize, X(:), Y(:), Z(:));
    centerIdx = sub2ind(volumeSize, x, y, z);
    neighborIdx(neighborIdx == centerIdx) = [];
end
function model = applySurfaceLimitedMorphology(model, se, operation)
    % 仅在表面附近应用指定的3D形态学操作
    boundaryMask = imdilate(bwperim(model, 26), ones(3, 3, 3));
    switch lower(operation)
        case 'dilate'
            candidate = imdilate(model, se);
        case 'erode'
            candidate = imerode(model, se);
        case 'open'
            candidate = imopen(model, se);
        case 'close'
            candidate = imclose(model, se);
        otherwise
            candidate = model;
...
function model = steerLocalAdjustment(model, center, direction, desiredState)
    % 沿梯度方向对局部进行温和调整，保持拓扑安全
    steps = 2;
    volumeSize = size(model);
    for s = 1:steps
        pos = round(center + s * direction);
        if any(pos < 1) || pos(1) > volumeSize(1) || pos(2) > volumeSize(2) || pos(3) > volumeSize(3)
            break;
        end
        idx = sub2ind(volumeSize, pos(1), pos(2), pos(3));
        if model(idx) == desiredState
            continue;
        end
        if ~isTopologicallySafeToggle(model, idx, desiredState)
        ...
        trial = model;
        trial(idx) = desiredState;
        if isSurfaceConsistent(trial, idx)
            model = trial;
        end
    end
end
function energy = computeUnifiedEnergy(model, optParams)
    % 计算统一的能量函数，使初始校准与MCMC一致
    if nargin < 2 || isempty(optParams)
        optParams = struct();
    end
    ...
    if weights.porosity > 0
        energy = energy + weights.porosity * calculatePorosityEnergy(features, optParams);
        totalWeight = totalWeight + weights.porosity;
    end
    if weights.cluster > 0
        energy = energy + weights.cluster * calculateClusterEnergy(features, optParams);
        totalWeight = totalWeight + weights.cluster;
    end
    morphologyFeatures = computeDetailedMorphologyFeatures(model);
    if weights.morphology > 0
        energy = energy + weights.morphology * calculateMorphologyEnergy(morphologyFeatures, optParams);
        totalWeight = totalWeight + weights.morphology;
    end
    spatialFeatures = computeEnhancedSpatialFeatures(model);
    if weights.spatial > 0
        energy = energy + weights.spatial * calculateSpatialEnergy(spatialFeatures, optParams);
        totalWeight = totalWeight + weights.spatial;
    end
    if weights.connectivity > 0
        energy = energy + weights.connectivity * calculateConnectivityEnergy(spatialFeatures, optParams);
        totalWeight = totalWeight + weights.connectivity;
    end
    multiScaleFeatures = [];
    if isfield(optParams, 'multiScaleSpatialFeatures') && ~isempty(optParams.multiScaleSpatialFeatures)
        ...
        if weights.multiScale > 0
            energy = energy + weights.multiScale * calculateMultiScaleEnergy(multiScaleFeatures, optParams);
            totalWeight = totalWeight + weights.multiScale;
        end
    end
    if weights.shapePreservation > 0
        energy = energy + weights.shapePreservation * calculateShapePreservationEnergy(features, morphologyFeatures, optParams);
        totalWeight = totalWeight + weights.shapePreservation;
    end
    if weights.structureCoherence > 0
        energy = energy + weights.structureCoherence * calculateStructureCoherenceEnergy(struct('model', model));
        totalWeight = totalWeight + weights.structureCoherence;
    end
    if totalWeight > 0
        energy = energy / totalWeight;
    end
end
function weights = getEnergyWeights(optParams)
    if isfield(optParams, 'energyWeights') && ~isempty(optParams.energyWeights)
        weights = optParams.energyWeights;
        return;
    end
    weights = struct('cluster', 1.0, 'porosity', 1.0, 'morphology', 1.0, ...
        'spatial', 1.0, 'connectivity', 1.0, 'multiScale', 1.0, ...
        'shapePreservation', 1.0, 'structureCoherence', 1.0);
end
            ...
            if CC.NumObjects > 0
                ...
                for i = 1:nAdjust
                    ...
                    if targetSpher > currentSpher
                        ...
                        randDir = randn(1, 3);
                        se = createDirectionalStructuringElement(randDir, 5, 1);
...
function model = connectClusters(model, cluster1, cluster2, varargin)
    % 使用地质距离规划连接路径，避免穿墙
    params = struct('MaxDilationIterations', 5, ...
        'SafetyLimit', 500);
    if ~isempty(varargin)
        ...
        if isstruct(extraParams)
            ...
            for pIdx = 1:numel(paramNames)
                name = paramNames{pIdx};
                if isfield(extraParams, name) && ~isempty(extraParams.(name))
    ...
    clusterMask = false(volumeSize);
    clusterMask(cluster1) = true;
    clusterMask(cluster2) = true;
    [x1, y1, z1] = ind2sub(volumeSize, cluster1);
    [x2, y2, z2] = ind2sub(volumeSize, cluster2);
    minCorner = max([min([x1; x2]) - 3, min([y1; y2]) - 3, min([z1; z2]) - 3], 1);
    maxCorner = min([max([x1; x2]) + 3, max([y1; y2]) + 3, max([z1; z2]) + 3], volumeSize);
    subMask = false(volumeSize);
    subMask(minCorner(1):maxCorner(1), minCorner(2):maxCorner(2), minCorner(3):maxCorner(3)) = true;
    searchRegion = subMask & (imdilate(clusterMask, createEllipsoidalStructuringElement(2, 2, 2)) | ~model);
    if ~any(searchRegion(:))
        searchRegion = subMask;
    end
    seed1 = false(volumeSize); seed1(cluster1) = true;
    seed2 = false(volumeSize); seed2(cluster2) = true;
    try
        d1 = bwdistgeodesic(searchRegion, seed1, 'quasi-euclidean');
        d2 = bwdistgeodesic(searchRegion, seed2, 'quasi-euclidean');
    catch
        d1 = []; d2 = [];
    end
    if isempty(d1) || isempty(d2)
        % 回退：局部膨胀直到相交
        dilationSe = createEllipsoidalStructuringElement(1, 1, 1);
        mask1 = false(volumeSize); mask1(cluster1) = true;
        mask2 = false(volumeSize); mask2(cluster2) = true;
        for iter = 1:params.MaxDilationIterations
            mask1 = imdilate(mask1, dilationSe);
            mask2 = imdilate(mask2, dilationSe);
            if any(mask1 & mask2)
                break;
            end
        end
        pathMask = bwmorph3(mask1 & mask2, 'thicken', 1);
    else
        totalCost = d1 + d2;
        totalCost(~isfinite(totalCost)) = inf;
        [minVal, idx] = min(totalCost(:));
        if ~isfinite(minVal)
            pathMask = false(volumeSize);
        else
            pathMask = false(volumeSize);
            current = idx;
            pathMask(current) = true;
            safetyLimit = params.SafetyLimit;
            currentDist = d1(current);
            steps = 0;
            while currentDist > 0 && steps < safetyLimit
                neighbors = getNeighborIndices(ind2submat(volumeSize, current), volumeSize);
                neighborVals = d1(neighbors);
                neighborVals(~isfinite(neighborVals)) = inf;
                [nextDist, minNeighborIdx] = min(neighborVals);
                if ~isfinite(nextDist) || nextDist >= currentDist
                    break;
                end
                current = neighbors(minNeighborIdx);
                currentDist = nextDist;
                pathMask(current) = true;
                steps = steps + 1;
            end
            current = idx;
            currentDist = d2(current);
            steps = 0;
            while currentDist > 0 && steps < safetyLimit
                neighbors = getNeighborIndices(ind2submat(volumeSize, current), volumeSize);
                neighborVals = d2(neighbors);
                neighborVals(~isfinite(neighborVals)) = inf;
                [nextDist, minNeighborIdx] = min(neighborVals);
                if ~isfinite(nextDist) || nextDist >= currentDist
                    break;
    ...
    if any(pathMask(:))
        smoothingSe = createEllipsoidalStructuringElement(1, 1, 1);
        corridor = imdilate(pathMask, smoothingSe);
        additions = corridor & ~model;
        additions = bwmorph3(additions, 'majority', 1);
        model(additions) = true;
    end
    model = logical(model);
end
function indices = normalizeClusterIndices(clusterInput)
    % 将不同格式的簇输入统一转换为线性索引向量
    if isstruct(clusterInput) && isfield(clusterInput, 'PixelIdxList')
        clusterInput = clusterInput.PixelIdxList;
    end
    if iscell(clusterInput)
        if isempty(clusterInput)
            indices = [];
            return;
        end
        clusterInput = vertcat(clusterInput{:});
    end
    if isempty(clusterInput)
        indices = [];
        return;
    end
    indices = unique(clusterInput(:));
end
...
function move = generateSpatialAwareMove(model, lookupTables, optParams)
    ...
    gradThreshold = quantile(gradientMap(:), 0.8);
    highGradMask = gradientMap >= gradThreshold;
    boundaryRemove = highGradMask & bwperim(model, 26);
    boundaryAdd = highGradMask & bwperim(~model, 26);
    candidateIdx = find(boundaryRemove | boundaryAdd);
    if isempty(candidateIdx)
        move = generateLocalMove(model, 4);
        return;
    end
    maxCandidates = min(80, numel(candidateIdx));
    candidateIdx = candidateIdx(randperm(numel(candidateIdx), maxCandidates));
    smoothed = imgaussfilt3(double(model), 1.0);
    [gx, gy, gz] = gradient(smoothed);
    baseEnergy = computeUnifiedEnergy(model, optParams);
    bestIdx = [];
    bestEnergy = baseEnergy;
    desiredStates = model(candidateIdx) == 0;
    for i = 1:length(candidateIdx)
        ...
        [cx, cy, cz] = ind2sub(size(model), idx);
        gradDir = [gx(cx, cy, cz), gy(cx, cy, cz), gz(cx, cy, cz)];
        if norm(gradDir) > 0.01
            gradDir = gradDir / norm(gradDir);
            trial = steerLocalAdjustment(trial, [cx, cy, cz], gradDir, desiredState);
        end
        trialEnergy = computeUnifiedEnergy(trial, optParams);
        if trialEnergy < bestEnergy - 1e-5
            bestEnergy = trialEnergy;
            bestIdx = idx;
            bestState = desiredState;
        end
    end
    if isempty(bestIdx)
        move = generateLocalMove(model, 4);
    ...
    move.linearIdx = bestIdx;
    move.oldValues = model(bestIdx);
    move.newValues = bestState;
end
function move = generateSpatialCorrelationMove(model, optParams)
    % 生成空间相关性移动
    move = struct('linearIdx', [], 'oldValues', [], 'newValues', []);
    if nargin < 2 || ~isfield(optParams, 'spatialFeatures') || ...
            ~isfield(optParams.spatialFeatures, 'twoPointCorr')
        move = generateLocalMove(model, 4);
        return;
    end
    ...
    currentTPC = currentTPC(1:minRows, :);
    targetTPC = targetTPC(1:minRows, :);
    sigma = max(1, median(keyDistances(1:minRows)));
    weights = exp(-0.5 * (keyDistances(1:minRows) / sigma).^2);
    weights = weights / max(sum(weights), eps);
    diffVector = mean(currentTPC - targetTPC, 2);
    weightedDiff = sum(diffVector .* weights);
    tolerance = 5e-3;
    if abs(weightedDiff) < tolerance
        move = generateLocalMove(model, 4);
        return;
    ...
    gradVals = computeLocalGradientNorm(model, candidateIdx);
    [~, order] = sort(gradVals, 'ascend');
    candidateIdx = candidateIdx(order);
    maxCandidates = min(100, numel(candidateIdx));
    baseEnergy = computeUnifiedEnergy(model, optParams);
    bestIdx = [];
    bestEnergy = baseEnergy;
...
function move = generateAnisotropyAdjustMove(model, lookupTables, optParams)
    ...
    if ~isfield(lookupTables, 'localAnisotropy') || ...
            ...
            ~isfield(lookupTables.targetSpatialSignature, 'anisotropy')
        move = generateLocalMove(model, 3);
        return;
    end
    anisoMap = lookupTables.localAnisotropy;
    targetAniso = lookupTables.targetSpatialSignature.anisotropy;
    currentAniso = mean(anisoMap(:));
    if targetAniso >= currentAniso
        candidateMask = (anisoMap < quantile(anisoMap(:), 0.2)) & bwperim(~model, 26);
        desiredState = true;
    else
        candidateMask = (anisoMap > quantile(anisoMap(:), 0.8)) & bwperim(model, 26);
        desiredState = false;
    end
    candidateIdx = find(candidateMask);
    if isempty(candidateIdx)
    ...
    axisVec = [0, 0, 1];
    if isfield(optParams, 'directionalPorosityProfile') && ...
            isfield(optParams.directionalPorosityProfile, 'gradientStrength') && ...
            ~isempty(optParams.directionalPorosityProfile.gradientStrength)
        [~, axisIdx] = max(optParams.directionalPorosityProfile.gradientStrength);
        axisVec = eye(3);
        axisVec = axisVec(axisIdx, :);
    end
    baseEnergy = computeUnifiedEnergy(model, optParams);
    bestIdx = [];
    bestEnergy = baseEnergy;
    maxCandidates = min(60, numel(candidateIdx));
    candidateIdx = candidateIdx(randperm(numel(candidateIdx), maxCandidates));
    for i = 1:length(candidateIdx)
        idx = candidateIdx(i);
        if model(idx) == desiredState
        ...
        [cx, cy, cz] = ind2sub(size(model), idx);
        trial = steerLocalAdjustment(trial, [cx, cy, cz], axisVec, desiredState);
        trialEnergy = computeUnifiedEnergy(trial, optParams);
        if trialEnergy < bestEnergy - 1e-5
            bestEnergy = trialEnergy;
            bestIdx = idx;
...
function move = generateConnectivityEnhanceMove(model, optParams)
    ...
    candidatePairs = [];
    maxPairs = min(4, numel(sortIdx) - 1);
    for i = 2:maxPairs+1
        candidatePairs(end+1, :) = [sortIdx(1), sortIdx(i)]; %#ok<AGROW>
    end
    baseEnergy = computeUnifiedEnergy(model, optParams);
    bestModel = [];
    bestEnergy = baseEnergy;
    for i = 1:size(candidatePairs, 1)
        idxA = candidatePairs(i, 1);
        idxB = candidatePairs(i, 2);
        candidateModel = connectClusters(model, CC.PixelIdxList{idxA}, CC.PixelIdxList{idxB});
        trialEnergy = computeUnifiedEnergy(candidateModel, optParams);
        if trialEnergy < bestEnergy - 1e-5
            bestEnergy = trialEnergy;
    ...
    diffMask = xor(bestModel, model);
    linearIdx = find(diffMask);
    move.linearIdx = linearIdx;
    move.oldValues = model(linearIdx);
    move.newValues = bestModel(linearIdx);
end
...
function model = localMorphologyPerturbation(model, optParams)
    ...
    if targetSphericity > 0.6
        ...
        se = createDirectionalStructuringElement([1, 0, 0], 5, 1);
