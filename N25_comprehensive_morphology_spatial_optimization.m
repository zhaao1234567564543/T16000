...
% 基于原始模型构建孔隙率-空间特征梯度模型

fprintf('\n正在分析孔隙率与空间特征之间的梯度关系...\n');

spatialGradientModel = computeSpatialParameterGradients(rawBinaryModel, spatialFeatures, raw_porosity);

fprintf('梯度分析完成，获得 %d 个有效区域样本。\n', spatialGradientModel.numSamples);

%% 1. 二值化并保存原始模型

binVol = rawBinaryModel;

fprintf('使用固定灰度阈值生成二值模型，孔隙率：%.4f\n', mean(binVol(:)));
...
while isempty(target_porosity) || ~isfinite(target_porosity) || ...

        target_porosity < 0 || target_porosity > 1

    prompt = sprintf('请输入目标孔隙率（0-1之间，原始参考 %.4f）: ', raw_porosity);

    target_porosity = input(prompt);

end

% 根据梯度模型推断新的空间特征目标

new_target_spatial_features = predictSpatialFeaturesFromGradient(spatialGradientModel, target_porosity, spatialFeatures);

fprintf('\n根据目标孔隙率自动推断的关键空间特征摘要：\n');

displaySpatialFeatureSummary(new_target_spatial_features);
...
optParams.originalSpatialFeatures = spatialFeatures;
...
optParams.newTargetSpatialFeatures = new_target_spatial_features;

optParams.originalPorosity = raw_porosity;

% 为兼容旧流程提供基于原始模型的簇统计信息（作为软约束参考）

optParams.targetClusterCount = originalFeatures.numClusters;

optParams.targetMin = max(1, originalFeatures.sizeStats(1));

optParams.targetMax = max(optParams.targetMin + 1, originalFeatures.sizeStats(2));
...
mcmcModel = generateComprehensiveInitialModel(dims, target_porosity, originalFeatures, ...
    ...
    new_target_spatial_features, morphologyFeatures, optParams);
...
weights.morphology = 14.0;

weights.spatial = 13.0;
...
function gradientModel = computeSpatialParameterGradients(binaryModel, baseSpatialFeatures, basePorosity)

    % 通过局部采样构建孔隙率与空间特征之间的回归模型

    if nargin < 2 || isempty(baseSpatialFeatures)

        baseSpatialFeatures = computeEnhancedSpatialFeatures(binaryModel);

    end

    if nargin < 3 || isempty(basePorosity)

        basePorosity = mean(binaryModel(:));

    end

    featureNames = getSpatialFeatureRegressionList();

    [baseVector, featureNames] = flattenSpatialFeaturesForRegression(baseSpatialFeatures, featureNames);

    dims = size(binaryModel);

    windowSize = max(round(dims ./ [3, 3, max(2, ceil(dims(3) / 15))]), [20, 20, 8]);

    windowSize = min(windowSize, dims);

    stride = max(round(windowSize ./ 2), [10, 10, 5]);
    ...
    xStarts = unique([1:stride(1):max(1, dims(1) - windowSize(1) + 1), max(1, dims(1) - windowSize(1) + 1)]);

    yStarts = unique([1:stride(2):max(1, dims(2) - windowSize(2) + 1), max(1, dims(2) - windowSize(2) + 1)]);

    zStarts = unique([1:stride(3):max(1, dims(3) - windowSize(3) + 1), max(1, dims(3) - windowSize(3) + 1)]);
    ...
    for ix = 1:length(xStarts)
        ...
        for iy = 1:length(yStarts)
            ...
            for iz = 1:length(zStarts)
                ...
                x2 = min(x1 + windowSize(1) - 1, dims(1));

                y2 = min(y1 + windowSize(2) - 1, dims(2));

                z2 = min(z1 + windowSize(3) - 1, dims(3));

                subVolume = binaryModel(x1:x2, y1:y2, z1:z2);

                if numel(subVolume) < 1000
                ...
                [vec, ~] = flattenSpatialFeaturesForRegression(localFeatures, featureNames);

                samples = [samples; vec(:)']; %#ok<AGROW>

                porositySamples = [porositySamples; localPorosity]; %#ok<AGROW>

                if size(samples, 1) >= maxSamples
    ...
    if isempty(samples)
        ...
        gradientModel = struct('numSamples', 0, 'featureNames', {featureNames}, ...

            'coefficients', baseVector(:), 'polynomialDegree', 0, ...

            'validPorosityRange', [basePorosity, basePorosity], ...

            'referencePorosity', basePorosity, 'featureStats', [baseVector(:)'; baseVector(:)'; baseVector(:)']', ...

            'samplePorosity', [], 'sampleFeatureMatrix', [], ...

            'baseFeatureVector', baseVector(:)');
    ...
    porositySamples = double(porositySamples);

    targetRange = [max(0, basePorosity - 0.2), min(1, basePorosity + 0.2)];

    inRange = porositySamples >= targetRange(1) & porositySamples <= targetRange(2);

    if sum(inRange) < 5

        % 扩大范围确保样本数量

        expanded = porositySamples >= targetRange(1) - 0.05 & porositySamples <= targetRange(2) + 0.05;
        ...
        if sum(expanded) >= 5
            ...
            inRange = true(size(porositySamples));

        end

    end

    selectedFeatures = samples(inRange, :);

    selectedPorosity = porositySamples(inRange);

    % 处理缺失值

    for c = 1:size(selectedFeatures, 2)

        col = selectedFeatures(:, c);

        if all(isnan(col))

            selectedFeatures(:, c) = baseVector(c);

        else

            meanVal = mean(col, 'omitnan');

            col(isnan(col)) = meanVal;

            selectedFeatures(:, c) = col;

        end

    end

    featureStats = zeros(length(featureNames), 3);

    for c = 1:length(featureNames)

        col = selectedFeatures(:, c);

        featureStats(c, :) = [min(col), max(col), mean(col, 'omitnan')];

    end

    if size(selectedFeatures, 1) >= 6
    ...
    coefficients = zeros(length(featureNames), degree + 1);

    for c = 1:length(featureNames)

        y = selectedFeatures(:, c);

        if all(abs(y - y(1)) < 1e-8)

            coefficients(c, :) = [zeros(1, degree), y(1)];
        ...
        while currentDegree > 0 && numel(unique(round(y, 6))) <= currentDegree
        ...
        try
            ...
            coefficients(c, 1:currentDegree+1) = polyfit(selectedPorosity, y, currentDegree);

        catch

            coefficients(c, :) = [zeros(1, degree), y(end)];

        end

        if currentDegree < degree

            coefficients(c, currentDegree+2:end) = 0;
    ...
    gradientModel.numSamples = size(selectedFeatures, 1);

    gradientModel.featureNames = featureNames;

    gradientModel.coefficients = coefficients;

    gradientModel.polynomialDegree = degree;

    gradientModel.validPorosityRange = targetRange;

    gradientModel.referencePorosity = basePorosity;

    gradientModel.featureStats = featureStats;

    gradientModel.samplePorosity = selectedPorosity;

    gradientModel.sampleFeatureMatrix = selectedFeatures;

    gradientModel.baseFeatureVector = baseVector(:)';

end

function [values, featureNames] = flattenSpatialFeaturesForRegression(spatialFeatures, featureNames)
    ...
    values = nan(length(featureNames), 1);

    for i = 1:length(featureNames)

        values(i) = extractScalarSpatialFeature(spatialFeatures, featureNames{i});

    end

end

function featureNames = getSpatialFeatureRegressionList()
    ...
    featureNames = {
        ...
        'connectivity.eulerNumber', ...

        'connectivity.numComponents', ...

        'connectivity.largestComponentRatio', ...

        'connectivity.connectivityDensity', ...

        'chordLengthDistribution.meanLength', ...

        'chordLengthDistribution.stdLength', ...

        'poreSizeDistribution.meanRadius', ...

        'poreSizeDistribution.stdRadius', ...

        'poreSizeDistribution.maxRadius', ...

        'minkowskiFunctionals.surfaceArea', ...

        'minkowskiFunctionals.integralMeanCurvature', ...

        'minkowskiFunctionals.eulerCharacteristic', ...

        'minkowskiFunctionals.meanBreadth', ...

        'minkowskiFunctionals.volume'

        };

end

function value = extractScalarSpatialFeature(spatialFeatures, name)
    ...
    switch name
        ...
        case 'anisotropy'
            ...
            if isfield(spatialFeatures, 'anisotropy')

                value = spatialFeatures.anisotropy;

            end

        case 'porosityGradient'

            if isfield(spatialFeatures, 'porosityGradient')

                value = spatialFeatures.porosityGradient;
        ...
        case 'spatialAutocorrelation'
            ...
            if isfield(spatialFeatures, 'spatialAutocorrelation')
                ...
                value = spatialFeatures.spatialAutocorrelation;

            end

        case 'surfaceToVolumeRatio'

            if isfield(spatialFeatures, 'surfaceToVolumeRatio')

                value = spatialFeatures.surfaceToVolumeRatio;

            end

        case 'tortuosityEstimate'

            if isfield(spatialFeatures, 'tortuosityEstimate')

                value = spatialFeatures.tortuosityEstimate;

            end

        case 'connectivity.eulerNumber'

            if isfield(spatialFeatures, 'connectivity') && isfield(spatialFeatures.connectivity, 'eulerNumber')

                value = spatialFeatures.connectivity.eulerNumber;

            end

        case 'connectivity.numComponents'

            if isfield(spatialFeatures, 'connectivity') && isfield(spatialFeatures.connectivity, 'numComponents')

                value = spatialFeatures.connectivity.numComponents;

            end

        case 'connectivity.largestComponentRatio'

            if isfield(spatialFeatures, 'connectivity') && isfield(spatialFeatures.connectivity, 'largestComponentRatio')

                value = spatialFeatures.connectivity.largestComponentRatio;

            end

        case 'connectivity.connectivityDensity'

            if isfield(spatialFeatures, 'connectivity') && isfield(spatialFeatures.connectivity, 'connectivityDensity')

                value = spatialFeatures.connectivity.connectivityDensity;

            end

        case 'chordLengthDistribution.meanLength'

            if isfield(spatialFeatures, 'chordLengthDistribution')

                value = spatialFeatures.chordLengthDistribution.meanLength;

            end

        case 'chordLengthDistribution.stdLength'

            if isfield(spatialFeatures, 'chordLengthDistribution')

                value = spatialFeatures.chordLengthDistribution.stdLength;

            end

        case 'poreSizeDistribution.meanRadius'

            if isfield(spatialFeatures, 'poreSizeDistribution')

                value = spatialFeatures.poreSizeDistribution.meanRadius;

            end

        case 'poreSizeDistribution.stdRadius'

            if isfield(spatialFeatures, 'poreSizeDistribution')

                value = spatialFeatures.poreSizeDistribution.stdRadius;

            end

        case 'poreSizeDistribution.maxRadius'

            if isfield(spatialFeatures, 'poreSizeDistribution')

                value = spatialFeatures.poreSizeDistribution.maxRadius;

            end

        case 'minkowskiFunctionals.surfaceArea'

            if isfield(spatialFeatures, 'minkowskiFunctionals')

                value = spatialFeatures.minkowskiFunctionals.surfaceArea;

            end

        case 'minkowskiFunctionals.integralMeanCurvature'

            if isfield(spatialFeatures, 'minkowskiFunctionals')

                value = spatialFeatures.minkowskiFunctionals.integralMeanCurvature;

            end

        case 'minkowskiFunctionals.eulerCharacteristic'

            if isfield(spatialFeatures, 'minkowskiFunctionals')

                value = spatialFeatures.minkowskiFunctionals.eulerCharacteristic;

            end

        case 'minkowskiFunctionals.meanBreadth'

            if isfield(spatialFeatures, 'minkowskiFunctionals')

                value = spatialFeatures.minkowskiFunctionals.meanBreadth;

            end

        case 'minkowskiFunctionals.volume'

            if isfield(spatialFeatures, 'minkowskiFunctionals')

                value = spatialFeatures.minkowskiFunctionals.volume;
...
function newFeatures = predictSpatialFeaturesFromGradient(gradientModel, targetPorosity, baseSpatialFeatures)

    if nargin < 3 || isempty(baseSpatialFeatures)

        baseSpatialFeatures = struct();

    end

    if isempty(gradientModel) || gradientModel.numSamples == 0

        newFeatures = baseSpatialFeatures;
    ...
    if ~isfinite(p)
        ...
        p = gradientModel.referencePorosity;

    end

    lower = gradientModel.validPorosityRange(1);

    upper = gradientModel.validPorosityRange(2);

    margin = 0.05;

    p = max(lower - margin, min(upper + margin, p));

    coeffs = gradientModel.coefficients;

    degree = gradientModel.polynomialDegree;

    featureValues = zeros(length(gradientModel.featureNames), 1);

    for i = 1:length(featureValues)

        c = coeffs(i, 1:degree+1);

        featureValues(i) = polyval(c, p);

        stats = gradientModel.featureStats(i, :);

        if stats(2) > stats(1)

            rangeSpan = stats(2) - stats(1);

            minAllow = stats(1) - 0.2 * rangeSpan;

            maxAllow = stats(2) + 0.2 * rangeSpan;

            featureValues(i) = min(max(featureValues(i), minAllow), maxAllow);

        end

        if ~isfinite(featureValues(i))

            featureValues(i) = gradientModel.baseFeatureVector(i);

        end

    end

    newFeatures = reconstructSpatialFeaturesFromVector(gradientModel.featureNames, featureValues, baseSpatialFeatures);

end

function spatialFeatures = reconstructSpatialFeaturesFromVector(featureNames, values, baseSpatialFeatures)

    spatialFeatures = baseSpatialFeatures;
    ...
    for i = 1:length(featureNames)
        ...
        spatialFeatures = setNestedScalarField(spatialFeatures, name, val);

    end

end

function s = setNestedScalarField(s, path, value)
    ...
    s.(head) = setNestedScalarField(s.(head), tail, value);

end

function displaySpatialFeatureSummary(spatialFeatures)

    if isempty(spatialFeatures)
    ...
    summaryList = {
        ...
        'porosityGradient', '孔隙率梯度'; ...

        'spatialAutocorrelation', '空间自相关'; ...

        'surfaceToVolumeRatio', '表面积体积比'; ...

        'tortuosityEstimate', '迂曲度估计'; ...

        'connectivity.largestComponentRatio', '最大连通簇占比'; ...

        'connectivity.eulerNumber', '局部欧拉数'; ...

        'chordLengthDistribution.meanLength', '弦长均值'; ...

        'poreSizeDistribution.meanRadius', '孔隙半径均值'; ...

        'minkowskiFunctionals.integralMeanCurvature', '积分平均曲率'

        };

    for i = 1:size(summaryList, 1)

        val = extractScalarSpatialFeature(spatialFeatures, summaryList{i,1});
...
function mcmcModel = generateComprehensiveInitialModel(dims, targetPorosity, ...
    ...
    % 在阈值化之前自适应匹配两点相关函数，使初始场更接近目标空间特征

    baseField = iterativelyMatchTwoPointCorrelation(baseField, targetTPC, ...

        spatialFeatures.keyDistances, targetPorosity);
    ...
    % 3. 依据目标孔隙率进行阈值化（仅使用用户设定值）

    threshold = quantile(baseField(:), 1 - targetPorosity);
    ...
    % 4b. 使用代表性模板强化初始孔隙簇分布并对簇数进行预平衡

    model = seedClustersFromTemplates(model, densityMap, optParams);

    model = balanceClusterCounts(model, optParams, spatialFeatures);

    model = enforceLocalTwoPointMatch(model, spatialFeatures, targetPorosity);
    ...
    % 6b. 进一步校准初始模型，使多种指标更接近目标

    model = calibrateInitialModelToTargets(model, spatialFeatures, morphologyFeatures, optParams, densityMap);

    model = enforcePorosityClusterTargets(model, optParams, densityMap);
    ...
    model = encourageAdaptiveClusterBalance(model, optParams);
...
function field = iterativelyMatchTwoPointCorrelation(field, targetTPC, keyDistances, targetPorosity)

    % 迭代调整基础场，使其两点相关函数接近目标

    if nargin < 2 || isempty(targetTPC)

        return;

    end

    if nargin < 3 || isempty(keyDistances)

        keyDistances = 1:size(targetTPC, 1);

    end

    if nargin < 4 || isempty(targetPorosity)
    ...
    for iter = 1:maxIter
        ...
        binary = field > quantile(field(:), 1 - targetPorosity);

        currentTPC = computeFastTwoPointCorrelation(binary, keyDistances);

        targetMean = mean(targetTPC(1:size(currentTPC, 1), :), 2);

        currentMean = mean(currentTPC, 2);

        diffVal = targetMean - currentMean;

        if max(abs(diffVal)) < 0.01

            break;

        end

        adjustment = zeros(size(field));

        for k = 1:length(keyDistances)

            lag = keyDistances(k);

            sigma = max(1, lag / 3);

            smoothNoise = imgaussfilt3(randn(size(field)), sigma);

            smoothNoise = smoothNoise / (std(smoothNoise(:)) + eps);

            adjustment = adjustment + diffVal(k) * smoothNoise / length(keyDistances);

        end

        field = field + 0.35 * adjustment;
...
function model = seedClustersFromTemplates(model, densityMap, optParams, nSeeds)

    % 使用代表性簇模板在高密度区域播撒初始孔隙

    if nargin < 2 || isempty(densityMap)

        densityMap = ones(size(model));

    else

        densityMap = double(densityMap);

        densityMap = (densityMap - min(densityMap(:))) / (max(densityMap(:)) - min(densityMap(:)) + eps);

    end

    if nargin < 4 || isempty(nSeeds)

        if isfield(optParams, 'targetClusterCount')

            CC = bwconncomp(model, 26);

            nSeeds = max(0, round(optParams.targetClusterCount - CC.NumObjects));
    ...
    if nSeeds <= 0 || ~isfield(optParams, 'clusterLibrary') || isempty(optParams.clusterLibrary)

        return;

    end

    [~, order] = sort(densityMap(:), 'descend');
    ...
    lib = optParams.clusterLibrary;

    while placed < nSeeds && idx <= numel(order)

        [x, y, z] = ind2sub(volSize, order(idx));

        tmpl = lib{mod(placed, numel(lib)) + 1}.patch;
        ...
        xStart = max(1, x - floor(ps(1) / 2)); xEnd = min(volSize(1), xStart + ps(1) - 1);

        yStart = max(1, y - floor(ps(2) / 2)); yEnd = min(volSize(2), yStart + ps(2) - 1);

        zStart = max(1, z - floor(ps(3) / 2)); zEnd = min(volSize(3), zStart + ps(3) - 1);

        tmplCropped = tmpl(1:(xEnd - xStart + 1), 1:(yEnd - yStart + 1), 1:(zEnd - zStart + 1));

        region = model(xStart:xEnd, yStart:yEnd, zStart:zEnd);

        region = region | tmplCropped;

        model(xStart:xEnd, yStart:yEnd, zStart:zEnd) = region;
    ...
    model = adjustPorosity(model, optParams.targetPorosity, densityMap);

end

function model = balanceClusterCounts(model, optParams, spatialFeatures)
    ...
    current = CC.NumObjects;

    targetCount = optParams.targetClusterCount;
    ...
    if current > targetCount * 1.1
        ...
        toRemove = min(current - round(targetCount), numel(order));
    ...
    elseif current < targetCount * 0.9

        model = seedClustersFromTemplates(model, [], optParams, round(targetCount - current));

    end

    if nargin >= 3 && ~isempty(spatialFeatures)

        keyDistances = spatialFeatures.keyDistances;

        targetTPC = spatialFeatures.twoPointCorr;

        currentTPC = computeFastTwoPointCorrelation(model, keyDistances);

        if mean(abs(currentTPC(:) - targetTPC(:))) > 0.08

            model = enforceLocalTwoPointMatch(model, spatialFeatures, mean(model(:)));
...
function model = enforceLocalTwoPointMatch(model, spatialFeatures, targetPorosity)

    % 通过局部形态调整提升两点相关函数匹配度

    if nargin < 3 || isempty(targetPorosity)

        targetPorosity = mean(model(:));

    end

    if isempty(spatialFeatures) || ~isfield(spatialFeatures, 'twoPointCorr')
    ...
    targetMean = mean(targetTPC, 2);

    currentMean = mean(currentTPC, 2);

    diffMean = targetMean - currentMean;

    if max(abs(diffMean)) < 0.01
    ...
    if mean(diffMean) > 0
        ...
        model = imdilate(model, strel('sphere', 2));

    else

        model = imerode(model, strel('sphere', 1));
...
function model = calibrateInitialModelToTargets(model, targetSpatial, targetMorph, optParams, densityMap)
    ...
    % 通过多次迭代校准初始模型，使其在空间、形态和孔隙率等指标上接近目标
    ...
    for pass = 1:maxPasses
        ...
        if abs(currentPorosity - targetPorosity) > 5e-4
        ...
        if ~isempty(currentMultiScale) && multiScaleMatch < 0.9

            model = applyMultiScaleCorrection(model, currentMultiScale, ...

                optParams.multiScaleSpatialFeatures);

        end

        if isfield(optParams, 'targetMax') || isfield(optParams, 'targetMin')
        ...
        if ~isempty(densityMap) && mod(pass, 2) == 0
        ...
        if spatialMatch >= 0.95 && morphMatch >= 0.95 && ...
                ...
                abs(mean(model(:)) - targetPorosity) < 5e-4 && ...

                (isempty(currentMultiScale) || multiScaleMatch >= 0.92)
    ...
    if isfield(optParams, 'targetMax') || isfield(optParams, 'targetMin')
        ...
        if CC.NumObjects > 0
            ...
            if isfield(optParams, 'targetMax') && ~isempty(optParams.targetMax)
                ...
                targetMax = max(1, double(optParams.targetMax));

                maxTolerance = max(targetMax * 1.1, targetMax + max(8, round(0.03 * targetMax)));

                oversized = find(sizes > maxTolerance);

                for i = 1:numel(oversized)

                    model = reduceOversizedCluster(model, CC.PixelIdxList{oversized(i)}, optParams);

                end

            end

            if isfield(optParams, 'targetMin') && ~isempty(optParams.targetMin)

                targetMin = max(1, double(optParams.targetMin));

                minTolerance = max(3, floor(targetMin * 0.85));

                undersized = find(sizes < minTolerance);

                for i = 1:numel(undersized)

                    if isfield(optParams, 'preserveSmallPores') && optParams.preserveSmallPores

                        grownIdx = growSmallClusterIndices(CC.PixelIdxList{undersized(i)}, ...

                            size(model), max(targetMin, minTolerance));

                        model(CC.PixelIdxList{undersized(i)}) = false;
...
function model = adjustClusterSizeDistribution(model, optParams)
    ...
    if ~isempty(targetMax)
        ...
        maxAllowed = max(targetMax * 1.15, targetMax + max(10, round(0.05 * targetMax)));

    else

        maxAllowed = prctile(double(sizes), 95);

    end

    if ~isempty(targetMin)

        minAllowed = max(5, floor(targetMin * 0.75));

    else

        minAllowed = max(5, round(prctile(double(sizes), 25) * 0.5));
...
function model = reduceOversizedCluster(model, clusterIdx, optParams)
    ...
    if isfield(optParams, 'targetMax') && ~isempty(optParams.targetMax)
        ...
        targetSize = max(1, round(double(optParams.targetMax)));
...
function model = adjustPorosity(model, targetPorosity, densityMap)
    ...
    for iter = 1:maxIterations
        ...
        stepFraction = min(0.02, max(0.001, abs(diff)));
        ...
        if diff > 0
            ...
            scores = neighborCount(candidateIdx);

            scores = normalizeScores(scores);

            if ~isempty(densityMap)

                densityVals = densityMap(candidateIdx);

                densityVals = normalizeScores(densityVals);

                scores = scores + 0.7 * densityVals;
            ...
            if ~isempty(densityMap)
                ...
                scores = scores + 0.7 * (1 - densityVals);
    ...
    if abs(finalDiff) > tolerance && isempty(densityMap)
        ...
        if finalDiff > 0
            ...
            if ~isempty(candidateIdx)
                ...
                selection = candidateIdx(randperm(numel(candidateIdx), min(voxelsToAdjust, numel(candidateIdx))));
...
function E_porosity = calculatePorosityEnergy(features, optParams)
   ...
   targetPorosity = optParams.targetPorosity;
    ...
    if targetPorosity <= 0
        ...
        % 目标为零或负时，直接使用当前孔隙率的绝对值作为能量

        E_porosity = abs(currentPorosity);

        return;

    end

    E_porosity = abs(currentPorosity - targetPorosity) / targetPorosity;
...
function E_cluster = calculateClusterEnergy(features, optParams)
    ...
    if isfield(optParams, 'originalFeatures') && ~isempty(optParams.originalFeatures)

        reference = optParams.originalFeatures;
    ...
    currentSizes = double(features.sizes(:));

    if isempty(currentSizes)
    ...
    if ~isempty(reference) && isfield(reference, 'sizes') && ~isempty(reference.sizes)

        referenceSizes = double(reference.sizes(:));

        refStats = reference.sizeStats;

    else

        referenceSizes = currentSizes;

        refStats = [min(currentSizes), max(currentSizes), mean(currentSizes), std(currentSizes)];

    end

    refMean = max(refStats(3), 1);

    refStd = max(refStats(4), 1);

    curMean = features.sizeStats(3);

    curStd = max(features.sizeStats(4), 1);

    meanPenalty = abs(curMean - refMean) / refMean;

    stdPenalty = abs(curStd - refStd) / refStd;

    % 范围偏离惩罚（仅在明显越界时启用）

    refMin = max(1, refStats(1));

    refMax = max(refMin + 1, refStats(2));

    lowerDeviation = max(0, (refMin - features.sizeStats(1)) / (refMin + eps));

    upperDeviation = max(0, (features.sizeStats(2) - refMax) / (refMax + eps));

    rangePenalty = 0.5 * lowerDeviation + 0.5 * upperDeviation;

    % 极端碎片化或过度聚合的惩罚

    refCount = max(1, reference.numClusters);

    countRatio = features.numClusters / refCount;

    fragmentationPenalty = 0;

    if features.numClusters <= 1

        fragmentationPenalty = fragmentationPenalty + 1.2;

    elseif countRatio > 2.5

        fragmentationPenalty = fragmentationPenalty + min(2.0, (countRatio - 2.5));

    elseif countRatio < 0.4

        fragmentationPenalty = fragmentationPenalty + min(2.0, (0.4 - countRatio));
    ...
    if numel(referenceSizes) > 4

        quantiles = linspace(0, 1, 6);

        edges = unique(quantile(referenceSizes, quantiles));

        if numel(edges) < 3

            edges = linspace(min(referenceSizes), max(referenceSizes), 4);

        end

        edges(1) = floor(edges(1));

        edges(end) = ceil(edges(end));

        edges = unique(edges);

        if numel(edges) > 2

            refHist = histcounts(referenceSizes, edges, 'Normalization', 'probability');

            curHist = histcounts(currentSizes, edges, 'Normalization', 'probability');

            minLen = min(numel(refHist), numel(curHist));

            if minLen > 0

                refHist = refHist(1:minLen);

                curHist = curHist(1:minLen);

                distributionPenalty = mean(abs(refHist - curHist));
    ...
    similarityReward = max(0, 0.3 - distributionPenalty);

    E_cluster = 0.7 * meanPenalty + 0.7 * stdPenalty + 0.6 * rangePenalty + ...

        fragmentationPenalty + 0.8 * distributionPenalty - 0.4 * similarityReward;

    E_cluster = max(E_cluster, 0);
...
function mcmcState = enforceComprehensiveConstraints(mcmcState, optParams, phase, parallelHint)
    ...
    % 放宽的簇约束：仅在极端情况下进行自适应调节

    mcmcState.model = enforceClusterSizeConstraints(mcmcState.model, optParams);

    mcmcState.model = encourageAdaptiveClusterBalance(mcmcState.model, optParams);
...
function model = enforceClusterSizeConstraints(model, optParams)
    ...
    sizes = double(cellfun(@numel, CC.PixelIdxList));
    ...
    if isfield(optParams, 'originalFeatures') && ~isempty(optParams.originalFeatures)
        ...
        refStats = optParams.originalFeatures.sizeStats;

    else

        refStats = [min(sizes), max(sizes), mean(sizes), std(sizes)];

    end

    meanSize = max(refStats(3), 1);

    stdSize = max(refStats(4), 1);

    refMax = max(refStats(2), meanSize + 2 * stdSize);

    largeThreshold = max(refMax * 1.1, meanSize + 3.0 * stdSize);

    growthThreshold = max(3, round(meanSize * 0.25));

    tinyThreshold = max(1, round(meanSize * 0.08));

    modified = false;

    % 仅对明显超大的簇进行缩减

    largeClusters = find(sizes > largeThreshold);

    maxLargeAdjust = min(numel(largeClusters), 3);

    for i = 1:maxLargeAdjust
    ...
    tinyClusters = find(sizes <= tinyThreshold);

    for i = 1:min(numel(tinyClusters), 5)

        idx = tinyClusters(i);

        if isfield(optParams, 'preserveSmallPores') && optParams.preserveSmallPores

            model = growSmallCluster(model, CC.PixelIdxList{idx});
    ...
    smallClusters = find(sizes > tinyThreshold & sizes < growthThreshold);

    if numel(smallClusters) >= 2
        ...
        for i = 1:length(smallClusters)
            ...
            for j = i+1:length(smallClusters)
                ...
                [dist, ~, ~] = findClosestPoints(model, CC.PixelIdxList{smallClusters(i)}, CC.PixelIdxList{smallClusters(j)});

                if dist < 4

                    model = connectClusters(model, CC.PixelIdxList{smallClusters(i)}, CC.PixelIdxList{smallClusters(j)});
    ...
    elseif numel(smallClusters) == 1

        model = growSmallCluster(model, CC.PixelIdxList{smallClusters});
    ...
    if modified
        ...
        model = smoothBinaryModel(model, 1);

    end

end

function model = encourageAdaptiveClusterBalance(model, optParams)

    % 自适应地平衡簇数量，避免极端碎片化或过度聚合
    ...
    if ~isfield(optParams, 'originalFeatures') || isempty(optParams.originalFeatures)
        ...
        referenceCount = optParams.originalFeatures.numClusters;
    ...
    if isempty(referenceCount) || referenceCount <= 0

        referenceCount = CC.NumObjects;

    end

    lowerBound = max(1, round(0.45 * referenceCount));

    upperBound = max(lowerBound + 1, round(2.5 * referenceCount));

    adjusted = false;

    if CC.NumObjects < lowerBound

        deficit = lowerBound - CC.NumObjects;

        maxAdjust = min(deficit, 3);
    ...
    elseif CC.NumObjects > upperBound

        excess = CC.NumObjects - upperBound;

        maxAdjust = min(excess, 3);
    ...
    if adjusted && isfield(optParams, 'targetPorosity') && ~isempty(optParams.targetPorosity)
...
function model = smoothBinaryModel(model, radius)
    ...
    try
        ...
        se = strel('sphere', max(1, radius));

        model = imopen(imclose(model, se), se);

    catch

        % 如果不支持球形结构元素，则使用立方体近似

        se = true(max(1, radius * 2 + 1));
    ...
    switch moveType
        ...
        case {'morphology_preserving', 'local_shape', 'boundary_smooth', ...
            ...
            if morphDeficit > 0
                ...
                deltaE = baseDE - phaseBoost * (0.25 + 3.2 * morphDeficit);
...
function displayComprehensiveFinalResults(originalModel, finalModel, ...
    ...
    fprintf('  各向异性: 原始=%.4f | 目标=%.4f | 最终=%.4f\n', ...

        originalSpatial.anisotropy, optParams.spatialFeatures.anisotropy, finalSpatial.anisotropy);

    if isfield(optParams.spatialFeatures, 'connectivity') && ...

            isfield(optParams.spatialFeatures.connectivity, 'largestComponentRatio')

        targetConnectivity = optParams.spatialFeatures.connectivity.largestComponentRatio;

    else

        targetConnectivity = NaN;

    end

    fprintf('  连通性(最大簇占比): 原始=%.4f | 目标=%.4f | 最终=%.4f\n', ...

        originalSpatial.connectivity.largestComponentRatio, targetConnectivity, ...

        finalSpatial.connectivity.largestComponentRatio);

    if isfield(optParams.spatialFeatures, 'chordLengthDistribution') && ...

            isfield(optParams.spatialFeatures.chordLengthDistribution, 'meanLength') && ...

            isfield(finalSpatial, 'chordLengthDistribution')

        fprintf('  弦长均值: 原始=%.3f | 目标=%.3f | 最终=%.3f\n', ...

            originalSpatial.chordLengthDistribution.meanLength, ...

            optParams.spatialFeatures.chordLengthDistribution.meanLength, ...

            finalSpatial.chordLengthDistribution.meanLength);

    end

    if isfield(optParams.spatialFeatures, 'poreSizeDistribution') && ...

            isfield(finalSpatial, 'poreSizeDistribution')

        fprintf('  孔隙半径均值: 原始=%.3f | 目标=%.3f | 最终=%.3f\n', ...

            originalSpatial.poreSizeDistribution.meanRadius, ...

            optParams.spatialFeatures.poreSizeDistribution.meanRadius, ...

            finalSpatial.poreSizeDistribution.meanRadius);

    end

    if isfield(optParams.spatialFeatures, 'minkowskiFunctionals') && ...

            isfield(finalSpatial, 'minkowskiFunctionals')

        fprintf('  积分平均曲率: 原始=%.3f | 目标=%.3f | 最终=%.3f\n', ...

            originalSpatial.minkowskiFunctionals.integralMeanCurvature, ...

            optParams.spatialFeatures.minkowskiFunctionals.integralMeanCurvature, ...
    ...
    if isfield(optParams, 'spatialGradientModel') && ...

            isfield(optParams.spatialGradientModel, 'numSamples')

        gradientSampleCount = optParams.spatialGradientModel.numSamples;

    end

    fprintf('\n  梯度模型推断的目标特征摘要（样本数: %d）：\n', gradientSampleCount);

    displaySpatialFeatureSummary(optParams.spatialFeatures);

    % 4. 优化性能

    fprintf('\n4. 簇分布自适应结果：\n');

    fprintf('  最终簇最小/最大尺寸: %d / %d\n', finalFeatures.sizeStats(1), finalFeatures.sizeStats(2));

    fprintf('  平均尺寸: %.1f | 标准差: %.1f\n', finalFeatures.sizeStats(3), finalFeatures.sizeStats(4));

    if isfield(optParams.originalFeatures, 'sizes') && ~isempty(optParams.originalFeatures.sizes)

        origSizes = double(optParams.originalFeatures.sizes);

        finalSizes = double(finalFeatures.sizes);

        edges = linspace(min(origSizes), max(origSizes), 6);

        if numel(unique(edges)) > 2 && numel(finalSizes) > 0

            origHist = histcounts(origSizes, edges, 'Normalization', 'probability');

            finalHist = histcounts(finalSizes, edges, 'Normalization', 'probability');

            histDiff = mean(abs(origHist - finalHist));

            fprintf('  与原始簇尺寸分布差异: %.4f\n', histDiff);
    ...
    morphMatch = calculateMorphologyMatch(finalMorph, optParams.morphologyFeatures);

    spatialMatch = calculateSpatialMatch(finalSpatial, optParams.spatialFeatures);

    originSpatialMatch = calculateSpatialMatch(finalSpatial, originalSpatial);
    ...
    fprintf('  与新空间目标匹配度: %.3f\n', spatialMatch);

    fprintf('  与原始空间特征匹配度: %.3f\n', originSpatialMatch);
