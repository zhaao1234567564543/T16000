...
targetRatios = struct( ...
    ...
    'connectivity',      0.12, ...
    'multiScale',        0.05, ...
    'shapePreservation', 0.05, ...
    'structureCoherence',0.05, ...
    'island',            0.13, ...
    'directOverlap',     0.02); % 加强簇/连通性/岛屿惩罚，弱化直接重叠
...
mcmcState.bestModel = enforcePorosityHardConstraint(mcmcState.bestModel, target_porosity, 1000);
mcmcState.bestEnergy = min(mcmcState.bestEnergy, computeUnifiedEnergySnapshot(mcmcState.bestModel, optParams, weights));
fprintf('>>> 使用小孔隙保留模式（统一简化后处理） <<<\n');
finalModel = comprehensivePostProcessWithSmallPores(mcmcState.bestModel, optParams);
% 后处理后再次精确校准孔隙率
finalModel = enforcePorosityHardConstraint(finalModel, target_porosity, 1000);
% 检查模型质量（统一参考模型）
...
function mcmcModel = generateComprehensiveInitialModel(dims, targetPorosity, ...
    ...
    % 生成与目标孔隙率和簇统计接近的初始模型，减少后续 MCMC 负担
    fprintf('  生成综合优化的初始模型...');

    targetCount = optParams.targetClusterCount;
    ref = optParams.clusterReference;
    maxAllowed = [];
    if isfield(ref, 'maxSize') && ~isempty(ref.maxSize)
        maxAllowed = 1.2 * ref.maxSize;
    end

    sizes = sampleInitialClusterSizes(targetCount, ref);
    targetVolume = round(targetPorosity * prod(dims));
    scale = targetVolume / max(sum(sizes), 1);
    sizes = max(1, round(sizes * scale));
    if ~isempty(maxAllowed)
        sizes = min(sizes, round(maxAllowed));
    end

    anisotropyProfile = estimateAnisotropyFromFeatures(spatialFeatures, morphologyFeatures);
    avgRadius = nthroot((3 * mean(sizes)) / (4 * pi), 3);
    minSeparation = max(1, round(2 * avgRadius));
    radii = nthroot((3 * sizes) / (4 * pi), 3);
    centers = placeClusterCentersPoisson(dims, radii, minSeparation, 600);
    ...
    model = trimToInterior(model);
    model = lightSmooth(model);

    summary = extractEfficientClusterFeatures(model);
    if summary.numClusters > 1.2 * targetCount
        excess = summary.numClusters - round(1.1 * targetCount);
        model = removeSmallestClusters(model, excess);
    elseif summary.numClusters < 0.8 * targetCount
        deficit = round(0.8 * targetCount) - summary.numClusters;
        model = addSmallClusters(model, ref, deficit, minSeparation, anisotropyProfile);
    end
    if ~isempty(maxAllowed)
        model = capLargestClusters(model, maxAllowed);
    end

    model = enforcePorosityHardConstraint(model, targetPorosity, 200);

    clusterSummary = extractEfficientClusterFeatures(model);
    ...
    fprintf('\n    初始簇数: %d (目标 %d)，最大簇: %d / %.0f，孔隙率: %.4f\n', ...
        clusterSummary.numClusters, targetCount, ...
        max(clusterSummary.sizes), maxAllowed, mean(model(:)));
...
function sizes = sampleInitialClusterSizes(targetCount, clusterReference)
    if isempty(targetCount) || targetCount <= 0
        targetCount = max(1, round(clusterReference.numClusters));
    end
    mu = max(clusterReference.meanSize, 1);
    sigma = max(clusterReference.stdSize, 1);
    maxSize = [];
    if isfield(clusterReference, 'maxSize')
        maxSize = clusterReference.maxSize;
    elseif isfield(clusterReference, 'maxSizeTarget')
        maxSize = clusterReference.maxSizeTarget;
    end
    sizes = zeros(targetCount, 1);
    for i = 1:targetCount
        val = round(truncatedNormal(mu, sigma, 1, inf));
        sizes(i) = max(1, val);
    end
    if ~isempty(maxSize)
        sizes = min(sizes, round(1.2 * maxSize));
    end
end
function val = truncatedNormal(mu, sigma, minVal, maxVal)
...
function centers = placeClusterCentersPoisson(dims, radii, minSeparation, maxAttempts)
    if nargin < 4
        maxAttempts = 400;
    end
    ...
    for i = 1:n
        ...
        while ~placed && attempts < maxAttempts
            ...
            required = minSeparation + radii(i) + radii(1:i-1);
...
function model = trimToInterior(model)
    % 防止生成阶段簇直接穿越边界
    mask = true(size(model));
    mask([1 end], :, :) = false;
    mask(:, [1 end], :) = false;
    mask(:, :, [1 end]) = false;
    model = model & mask;
end
function model = lightSmooth(model)
    % 轻微平滑，避免破坏局部形态
    se = strel('sphere', 1);
    model = imopen(imclose(model, se), se);
end
function model = addSmallClusters(model, clusterRef, count, minSeparation, anisotropyProfile)
    if count <= 0
        return;
    end
    if nargin < 5 || isempty(anisotropyProfile)
        anisotropyProfile = [1, 1, 1];
    end
    sizes = clusterRef.sizeArray(:);
    ...
    centers = placeClusterCentersPoisson(dims, radii, minSeparation, 200);
    for i = 1:count
        axes = sampleAnisotropicAxes(radii(i), anisotropyProfile);
...
function model = enforcePorosityHardConstraint(model, targetPorosity, maxIters)
    ...
    while iter < maxIters
        ...
        currentPorosity = currentVoxels / totalVoxels;
        if abs(currentPorosity - targetPorosity) < 1e-4
            break;
        end
        ...
        stepCount = min(max(1, round(0.5 * totalVoxels * abs(targetPorosity - currentPorosity))), min(5, numel(candidates)));
        chosen = selectBoundaryCandidates(model, candidates, stepCount, toPore);
        model(chosen) = toPore;
        iter = iter + 1;
    end
end
function chosen = selectBoundaryCandidates(model, candidates, count, toPore)
    if isempty(candidates)
        chosen = [];
        return;
    end
    ...
    chosen = candidates(order(1:min(count, numel(order))));
end
...
function weights = getDefaultEnergyWeights()
    ...
    weights.connectivity = 6.0;
    weights.multiScale = 4.0;
    weights.shapePreservation = 6.0;
    weights.structureCoherence = 5.0;
    weights.island = 10.0;
    weights.directOverlap = 0.8; % 与原始模型直接重叠项
end
...
function mcmcState = enforceComprehensiveConstraints(mcmcState, optParams, phase, parallelHint)
    % 轻量级约束：仅对严重偏离的簇统计做局部纠偏
    if nargin < 4
        parallelHint = [];
    end
    modified = false;
    stats = extractEfficientClusterFeatures(mcmcState.model);

    if isfield(optParams, 'targetMax') && ~isempty(optParams.targetMax)
        if max(stats.sizes) > 2 * optParams.targetMax
            mcmcState.model = locallyTrimLargestCluster(mcmcState.model, optParams.targetMax);
            modified = true;
        end
    end

    if isfield(optParams, 'targetClusterCount') && ~isempty(optParams.targetClusterCount)
        tgt = optParams.targetClusterCount;
        if stats.numClusters < 0.7 * tgt
            deficit = max(1, tgt - stats.numClusters);
            mcmcState.model = carveBoundaryVoids(mcmcState.model, deficit);
            modified = true;
        elseif stats.numClusters > 1.3 * tgt
            removeCount = stats.numClusters - round(1.1 * tgt);
            mcmcState.model = removeSmallestClusters(mcmcState.model, removeCount);
            modified = true;
        end
    end
...
function model = locallyTrimLargestCluster(model, maxTarget)
    CC = bwconncomp(model, 26);
    if CC.NumObjects == 0
        return;
    end
    sizes = cellfun(@numel, CC.PixelIdxList);
    [largestSize, idx] = max(sizes);
    if largestSize <= maxTarget
        return;
    end
    clusterMask = false(size(model));
    clusterMask(CC.PixelIdxList{idx}) = true;
    boundaryMask = bwperim(clusterMask, 26);
    candidates = find(boundaryMask);
    removeCount = min(max(1, largestSize - round(maxTarget)), min(5, numel(candidates)));
    if removeCount > 0
        chosen = candidates(randperm(numel(candidates), removeCount));
        model(chosen) = false;
    end
end
function model = carveBoundaryVoids(model, count)
    context = computeBoundaryMasks(model);
    candidates = context.solidIdx;
    if isempty(candidates) || count <= 0
        return;
    end
    changeCount = min(max(1, round(count)), min(5, numel(candidates)));
    chosen = candidates(randperm(numel(candidates), changeCount));
    model(chosen) = true;
end
...
function mcmcState = refreshBoundaryContext(mcmcState)
    ...
    kernel(2, 2, 2) = 0;
    neighborPore = convn(double(model), kernel, 'same');
    neighborSolid = convn(double(~model), kernel, 'same');
    poreBoundary = model & neighborSolid > 0;
    solidBoundary = ~model & neighborPore > 0;
    mcmcState.boundaryPores = find(poreBoundary);
    mcmcState.boundarySolids = find(solidBoundary);
...
function [moves, moveTypes] = generateComprehensiveBatchMoves(mcmcState, batchSize, optParams, lookupTables, parallelEnabled, strategy)
    %#ok<INUSD,INUSA>
    % 仅在孔隙-基质边界附近生成最多 5 个体素的小步移动
    if nargin < 6 || isempty(strategy)
        strategy = 'boundary_local';
    end
    model = mcmcState.model;
    moves = cell(batchSize, 1);
    moveTypes = cell(batchSize, 1);
    poreBoundary = mcmcState.boundaryPores;
    solidBoundary = mcmcState.boundarySolids;
    for i = 1:batchSize
        if rand() < 0.5 && ~isempty(solidBoundary)
            idx = sampleBoundaryVoxels(solidBoundary, 1);
            moves{i} = buildFlipMove(model, idx, true);
            moveTypes{i} = 'solid_to_pore';
        elseif ~isempty(poreBoundary)
            idx = sampleBoundaryVoxels(poreBoundary, 1);
            moves{i} = buildFlipMove(model, idx, false);
            moveTypes{i} = 'pore_to_solid';
        else
            moves{i} = buildFlipMove(model, [], true);
            moveTypes{i} = 'noop';
        end
        if rand() < 0.2 && ~isempty(moves{i}.linearIdx)
            moves{i} = expandToLocalNeighborhood(model, moves{i}, 5);
            moveTypes{i} = [moveTypes{i}, '_local'];
        end
    end
end
function idx = sampleBoundaryVoxels(boundaryIdx, count)
    if isempty(boundaryIdx)
        idx = [];
        return;
    end
    count = min(count, numel(boundaryIdx));
    idx = boundaryIdx(randperm(numel(boundaryIdx), count));
end
function move = buildFlipMove(model, idx, newValue)
    ...
    move.newValues = repmat(logical(newValue), numel(idx), 1);
end
function move = expandToLocalNeighborhood(model, move, maxVoxels)
    if isempty(move.linearIdx)
        return;
    end
    dims = size(model);
    [x, y, z] = ind2sub(dims, move.linearIdx(1));
    xr = max(1, x-1):min(dims(1), x+1);
    yr = max(1, y-1):min(dims(2), y+1);
    zr = max(1, z-1):min(dims(3), z+1);
    [xx, yy, zz] = ndgrid(xr, yr, zr);
    localIdx = sub2ind(dims, xx(:), yy(:), zz(:));
    localIdx = setdiff(localIdx, move.linearIdx);
    extraCount = min(maxVoxels - numel(move.linearIdx), numel(localIdx));
    if extraCount <= 0
        return;
    end
    chosen = localIdx(randperm(numel(localIdx), extraCount));
    move.linearIdx = [move.linearIdx; chosen];
    move.oldValues = [move.oldValues; model(chosen)];
    move.newValues = [move.newValues; repmat(move.newValues(1), extraCount, 1)];
end
%% ========== 评估和应用函数 ==========
...
function finalModel = comprehensivePostProcessWithSmallPores(model, optParams)
    fprintf('>>> 使用小孔隙保留模式（简化后处理） <<<\n');
    finalModel = model;
    CC = bwconncomp(finalModel, 26);
    sizes = cellfun(@numel, CC.PixelIdxList);
    tinyIdx = find(sizes <= 2);
    for i = 1:numel(tinyIdx)
        finalModel(CC.PixelIdxList{tinyIdx(i)}) = false;
    end
    se = strel('sphere', 1);
    smoothed = imclose(finalModel, se);
    smoothed = imopen(smoothed, se);
    finalModel = smoothed;
end
