...
optParams.porosityTolerance = 0.002; % 近硬约束的孔隙率容差
...
targetRatios = struct( ...
 ...
 'connectivity', 0.10, ...
 'multiScale', 0.05, ...
 'shapePreservation', 0.08, ...
 'structureCoherence',0.07, ...
 'directOverlap', 0.08); % 提高簇与连通性权重，避免拓扑偏差被忽视
...
for iter = 1:maxIterations
 ...
 morphHistory = [morphHistory, currentMorphMatch];
 spatialHistory = [spatialHistory, currentSpatialMatch];
 if numel(morphHistory) > phaseWindow
 morphHistory = morphHistory(end-phaseWindow+1:end);
 end
 if numel(spatialHistory) > phaseWindow
 spatialHistory = spatialHistory(end-phaseWindow+1:end);
 end

 switch optimization_phase
 case 'morphology'
 stagnated = numel(morphHistory) >= phaseWindow && ...
 (max(morphHistory) - min(morphHistory)) < 0.01;
 if currentMorphMatch > 0.75 || stagnated
 fprintf('\n切换到空间优化阶段...\n');
 optimization_phase = 'spatial';
 end
 case 'spatial'
 stagnated = numel(spatialHistory) >= phaseWindow && ...
 (max(spatialHistory) - min(spatialHistory)) < 0.01;
 if currentSpatialMatch > 0.80 || stagnated
 fprintf('\n切换到平衡优化阶段...\n');
...
function morphologyFeatures = computeDetailedMorphologyFeatures(binaryModel)
 ...
 [elong, spher, convx, solid, shapeType] = computeClusterMorphologyMetrics(pixelIdx, modelSize);
elongation(i) = elong;
sphericity(i) = spher;
convexity(i) = convx;
solidity(i) = solid;
...
morphologyFeatures.shapeTypes = shapeTypes;
morphologyFeatures.shapeTypeProportions = calculateShapeTypeProportions(shapeTypes);
...
function [elongation, sphericity, convexity, solidity, shapeType] = ...
 ...
 eigenvalues = eig(C);
 eigenvalues = sort(max(eigenvalues, 1e-2), 'descend');
 ...
 lambdaMin = max(eigenvalues(3), 1e-2);
 elongation = sqrt(eigenvalues(1) / lambdaMin);
 elongation = min(elongation, 20);
 volume = nVoxels;
...
solidity = convexity;
r12 = eigenvalues(1) / max(eigenvalues(2), 1e-2);
r23 = eigenvalues(2) / max(lambdaMin, 1e-2);
if r12 < 1.3 && r23 < 1.3
shapeType = 1; % 等轴
elseif r12 >= r23
shapeType = 2; % 纺锤/棒状
...
function proportions = calculateShapeTypeProportions(shapeTypes)
 % 统计不同形态类型的占比
 if nargin < 1 || isempty(shapeTypes)
 proportions = [0, 0, 0];
 return;
 end
 counts = [sum(shapeTypes == 1), sum(shapeTypes == 2), sum(shapeTypes == 3)];
...
function [model, summary] = finalizeInitialCandidateModel(baseField, densityMap, targetPorosity, ...
 ...
 sortedField = sort(baseField(:), 'ascend');
 % 以分位数阈值直接匹配目标孔隙率，避免二次“自动阈值搜索”
 cutoffIdx = max(1, min(numel(sortedField), round((1 - targetPorosity) * numel(sortedField))));
 threshold = sortedField(cutoffIdx);
...
function energy = calculatePorosityEnergy(clusterFeatures, optParams)
 ...
 targetPorosity = optParams.targetPorosity;
 deltaPhi = 0.002;
 if isfield(optParams, 'porosityTolerance') && ~isempty(optParams.porosityTolerance)
 deltaPhi = optParams.porosityTolerance;
 end
 if isempty(targetPorosity) || ~isfinite(targetPorosity)
 energy = 0;
 return;
 ...
 diff = abs(currentPorosity - targetPorosity);
 energy = min(1, diff / max(deltaPhi, eps));
end
function E_cluster = calculateClusterEnergy(features, optParams)
 % 归一化的簇能量：同时约束簇数与簇大小分布
 ...
 N = features.numClusters;
 N_t = max(1, clusterRef.numClusters);
 diffN = min(1, abs(N - N_t) / (N_t + 1));

 mu = mean(sizes);
 mu_t = max(clusterRef.meanSize, eps);
 diffMu = min(1, abs(mu - mu_t) / (mu_t + 1));

 sigma = std(sizes);
 sigma_t = max(clusterRef.stdSize, eps);
 diffSigma = min(1, abs(sigma - sigma_t) / (sigma_t + 1));

 E_cluster = (diffN + diffMu + diffSigma) / 3;
end
...
function E_direct = calculateDirectOverlapEnergy(model, optParams)
 ...
 mismatch = mean(xor(binModel(:), refModel(:)));
 % 以重叠提升、差异惩罚的形式构造能量
 E_direct = (1 - overlap) + mismatch;
end
...
function weights = initializeNormalizedEnergyWeights(components, targetRatios)
 ...
 weights = struct();
 for i = 1:numel(fn)
 weights.(fn{i}) = ratios(i);
 end

 % 强化孔隙率和核心匹配项，保持总和为1
 coreFields = {'porosity','morphology','spatial','cluster'};
 boost = 1.2;
 for i = 1:numel(coreFields)
 if isfield(weights, coreFields{i})
 weights.(coreFields{i}) = weights.(coreFields{i}) * boost;
 end
 end
 vals = structfun(@(v) v, weights);
 total = sum(vals(:));
 weights = structfun(@(v) v / total, weights, 'UniformOutput', false);
...
function components = evaluateEnergyComponents(model, clusterFeatures, spatialFeatures, ...
 ...
 morphMatch = calculateMorphologyMatch(morphologyFeatures, optParams.morphologyFeatures);
 components.morphology = 1 - morphMatch;

 spatialMatch = calculateSpatialMatch(spatialFeatures, optParams.spatialFeatures);
 components.spatial = 1 - spatialMatch;

 components.cluster = calculateClusterEnergy(clusterFeatures, optParams);
 components.connectivity = min(1, max(0, calculateConnectivityEnergy(spatialFeatures, optParams)));
 components.multiScale = min(1, max(0, calculateMultiScaleEnergy(multiScaleFeatures, optParams)));
 components.shapePreservation = min(1, max(0, calculateShapePreservationEnergy(clusterFeatures, ...
 morphologyFeatures, optParams)));
 components.structureCoherence = min(1, max(0, calculateStructureCoherenceEnergy(struct('model', model))));
 components.directOverlap = min(1, max(0, calculateDirectOverlapEnergy(model, optParams)));
end
%% ========== 特征更新和约束函数 ==========
...
function mcmcState = enforceComprehensiveConstraints(mcmcState, optParams, phase, parallelHint)
 ...
 % 当簇数量严重偏离时优先触发结构修正
 if isfield(optParams, 'clusterReference') && isfield(mcmcState, 'features')
 targetN = optParams.clusterReference.numClusters;
 curN = mcmcState.features.numClusters;
 deltaN = abs(curN - targetN) / max(targetN, 1);
 if deltaN > 0.4
 fprintf('簇数量偏差 %.2f，触发强制簇修正...\n', deltaN);
 for k = 1:3
 if curN < targetN
 move = generateSplitLargeClusterMove(mcmcState.model, optParams);
 else
 move = generateMergeSmallClustersMove(mcmcState.model, optParams);
 end
 if ~isempty(move.linearIdx)
 mcmcState.model(move.linearIdx) = move.newValues;
 end
 end
 mcmcState = updateAllFeatures(mcmcState, parallelHint);
 end
...
function strategy = selectComprehensiveAdaptiveMoveStrategy(iter, maxIter, mcmcState, phase)
 ...
 strategies = {'morphology_preserving', 'cluster_shape', 'local_shape', 'fine_morphology', 'boundary_smooth'};
 ...
 % 根据簇数量偏差加入拆分/合并策略
 if isfield(mcmcState, 'features') && isfield(mcmcState.features, 'numClusters') && ...
 isfield(mcmcState.optParams, 'clusterReference')
 targetN = mcmcState.optParams.clusterReference.numClusters;
 curN = mcmcState.features.numClusters;
 deltaN = abs(curN - targetN) / max(targetN, 1);
 if deltaN > 0.4
 strategies = [strategies, {'cluster_split', 'cluster_merge'}];
 weights = [weights * 0.7, 0.2, 0.1];
 elseif deltaN > 0.2
 strategies = [strategies, {'cluster_split', 'cluster_merge'}];
 weights = [weights * 0.8, 0.12, 0.08];
 end
 end

 % 始终提供孔隙率保持的交换移动
 strategies = [strategies, {'porosity_swap'}];
 weights = [weights, 0.1];
...
function [move, moveType] = dispatchComprehensiveMove(model, strategy, optParams, lookupTables)
 ...
 moveType = 'cluster';
 case 'cluster_split'
 move = generateSplitLargeClusterMove(model, optParams);
 moveType = 'cluster_split';
 case 'cluster_merge'
 move = generateMergeSmallClustersMove(model, optParams);
 moveType = 'cluster_merge';
 case 'boundary'
 move = generateBoundaryMove(model);
 moveType = 'boundary';
 case 'porosity_swap'
 move = generatePorositySwapMove(model, optParams);
 moveType = 'porosity_swap';
 otherwise
...
function move = generatePorositySwapMove(model, optParams)
 % 孔隙-基质交换，保持全局孔隙率
 if nargin < 2
 optParams = struct();
 end
 move = struct('linearIdx', [], 'oldValues', [], 'newValues', []);
 poreIdx = find(model);
 matrixIdx = find(~model);
 if isempty(poreIdx) || isempty(matrixIdx)
 move = generateLocalMove(model, 3);
 return;
 end
 maxSwap = min([numel(poreIdx), numel(matrixIdx), 50]);
 nSwap = max(1, round(0.001 * numel(model)));
 nSwap = min(nSwap, maxSwap);
 swapPore = poreIdx(randperm(numel(poreIdx), nSwap));
 swapMatrix = matrixIdx(randperm(numel(matrixIdx), nSwap));
 move.linearIdx = [swapPore; swapMatrix];
 move.oldValues = [true(nSwap,1); false(nSwap,1)];
 move.newValues = [false(nSwap,1); true(nSwap,1)];
end
function move = generateSplitLargeClusterMove(model, optParams)
 % 拆分过大的簇，同时保持总体孔隙率
 move = struct('linearIdx', [], 'oldValues', [], 'newValues', []);
 CC = bwconncomp(model, 26);
 if CC.NumObjects == 0
 ...
 targetN = max(1, optParams.clusterReference.numClusters);
 if CC.NumObjects >= targetN
 move = generateLocalMove(model, 3);
 return;
 end
 sizes = cellfun(@numel, CC.PixelIdxList);
 ...
 neckMask = distMap <= prctile(distMap(clusterMask), 30);
 removable = find(clusterMask & neckMask);
 if isempty(removable)
 move = generateLocalMove(model, 3);
 return;
 end
 nRemove = min(20, numel(removable));
 removeIdx = removable(randperm(numel(removable), nRemove));
 growCandidates = find(imdilate(clusterMask, ones(3,3,3)) & ~model);
 if isempty(growCandidates)
 move.linearIdx = removeIdx;
 move.oldValues = true(numel(removeIdx),1);
 move.newValues = false(numel(removeIdx),1);
 return;
 end
 nGrow = min(nRemove, numel(growCandidates));
 growIdx = growCandidates(randperm(numel(growCandidates), nGrow));
 move.linearIdx = [removeIdx; growIdx];
 move.oldValues = [true(numel(removeIdx),1); false(numel(growIdx),1)];
 move.newValues = [false(numel(removeIdx),1); true(numel(growIdx),1)];
end
function move = generateMergeSmallClustersMove(model, optParams)
 % 将过小簇连接到邻近大簇，避免簇数量过多
 move = struct('linearIdx', [], 'oldValues', [], 'newValues', []);
 CC = bwconncomp(model, 26);
 if CC.NumObjects < 2
 ...
 sizes = cellfun(@numel, CC.PixelIdxList);
 [~, bigIdx] = max(sizes);
 smallIdx = find(sizes < prctile(double(sizes), 25));
 if isempty(smallIdx)
 move = generateLocalMove(model, 3);
 return;
 end
 smallIdx = smallIdx(1);
 [sx, sy, sz] = ind2sub(size(model), CC.PixelIdxList{smallIdx});
 [bx, by, bz] = ind2sub(size(model), CC.PixelIdxList{bigIdx});
 startPt = round([mean(sx), mean(sy), mean(sz)]);
 endPt = round([mean(bx), mean(by), mean(bz)]);
 nSteps = max(3, ceil(norm(startPt - endPt)));
 xs = round(linspace(startPt(1), endPt(1), nSteps));
 ys = round(linspace(startPt(2), endPt(2), nSteps));
 zs = round(linspace(startPt(3), endPt(3), nSteps));
 pathIdx = sub2ind(size(model), xs, ys, zs);
 pathIdx = unique(pathIdx(~model(pathIdx)));
 if isempty(pathIdx)
 move = generateLocalMove(model, 3);
 return;
 end
 bigMask = false(size(model));
 bigMask(CC.PixelIdxList{bigIdx}) = true;
 boundaryBig = find(bwperim(bigMask, 26));
 if isempty(boundaryBig)
 boundaryBig = CC.PixelIdxList{bigIdx};
 end
 nSwap = min(numel(pathIdx), numel(boundaryBig));
 addIdx = pathIdx(1:nSwap);
 removeIdx = boundaryBig(randperm(numel(boundaryBig), nSwap));
 move.linearIdx = [removeIdx(:); addIdx(:)];
 move.oldValues = [true(numel(removeIdx),1); false(numel(addIdx),1)];
 move.newValues = [false(numel(removeIdx),1); true(numel(addIdx),1)];
end
%% ========== 评估和应用函数 ==========
...
function deltaE = evaluateComprehensiveMoveDelta(mcmcState, move, moveType, ...
 ...
 % 孔隙率近硬约束检查
 deltaPhi = 0.002;
 if isfield(mcmcState.optParams, 'porosityTolerance') && ~isempty(mcmcState.optParams.porosityTolerance)
 deltaPhi = mcmcState.optParams.porosityTolerance;
 end
 targetPhi = mcmcState.optParams.targetPorosity;
 if ~isempty(move.linearIdx)
 nFlipped = sum(move.newValues) - sum(move.oldValues);
 newPhi = mean(mcmcState.model(:)) + nFlipped / numel(mcmcState.model);
 if abs(newPhi - targetPhi) > 2 * deltaPhi
 deltaE = inf;
 return;
 end
 end
...
function deltaE = evaluateBaseMoveDelta(mcmcState, move, localStats)
 ...
 deltaE_porosity = (min(1, newError / max(deltaPhi, eps)) - min(1, currentError / max(deltaPhi, eps))) * ...
 mcmcState.weights.porosity;

 % 显式惩罚越界趋势
 if newError > 2 * deltaPhi
 deltaE_porosity = deltaE_porosity + mcmcState.weights.porosity * 5;
 end
 % 利用预计算的局部统计量估计表面积变化
...
function match = calculateMorphologyMatch(currentMorph, targetMorph)
 ...
 curLog = log1p(currentMorph.elongation(:));
 tgtLog = log1p(targetMorph.elongation(:));
 medCurElong = median(curLog);
 medTgtElong = median(tgtLog);
 scale = iqr(tgtLog);
 if scale <= 0
 scale = max(medTgtElong, 1e-3);
 end
 elongDiff = abs(medCurElong - medTgtElong) / scale;
 ...
 scale = max(iqr(targetMorph.sphericity(:)), 0.1);
 spherDiff = abs(medCurSpher - medTgtSpher) / scale;
 ...
 % 形态类型比例匹配
 typeMatch = 1;
 if isfield(currentMorph, 'shapeTypeProportions') && isfield(targetMorph, 'shapeTypeProportions')
 curProp = currentMorph.shapeTypeProportions;
 tgtProp = targetMorph.shapeTypeProportions;
 if numel(curProp) >= 3 && numel(tgtProp) >= 3
 typeDiff = sum(abs(curProp(1:3) - tgtProp(1:3))) / 2; % L1/2 范数归一
 typeDiff = min(1, typeDiff);
 typeMatch = 1 - typeDiff;
 end
 end
 ...
 % 综合匹配度，重视形状与类型一致性
 match = 0.35 * elongMatch + 0.25 * spherMatch + 0.2 * typeMatch + 0.2 * densityMatch;
 match = max(0, min(1, match));
end
