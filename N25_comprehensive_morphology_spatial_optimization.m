...
% 依据目标孔隙率与原始模型的统计特征，自适应推断初始簇规模与数量
[initialTargets, clusterAdaptation] = initializeAdaptiveClusterTargets( ...
    originalFeatures, target_porosity, raw_porosity);

fprintf('\n>>> 自适应簇目标初始化完成：\n');
fprintf('    目标簇数量（初值）：%d\n', initialTargets.count);
fprintf('    目标簇尺寸范围（初值）：[%d, %d]\n', ...
    initialTargets.minSize, initialTargets.maxSize);
% 询问是否保留小孔隙特征
fprintf('\n小孔隙保留选项：\n');
...
optParams.targetMax = initialTargets.maxSize;
optParams.targetMin = initialTargets.minSize;
optParams.targetClusterCount = initialTargets.count;
optParams.clusterAdaptation = clusterAdaptation;
...
performanceMonitor.targetMinHistory(1) = optParams.targetMin;
performanceMonitor.targetMaxHistory(1) = optParams.targetMax;
performanceMonitor.targetCountHistory(1) = optParams.targetClusterCount;
performanceMonitor.acceptanceHistory(1) = mcmcState.acceptanceRate;
%% 5. 综合MCMC主循环
...
for iter = 1:maxIterations
    ...
    % 自适应调整目标簇统计
    [mcmcState, performanceMonitor] = updateAdaptiveClusterTargetsDuringRun( ...
        mcmcState, performanceMonitor, iter);
...
%% ========== 自适应簇目标管理 ==========
function [targets, adaptationState] = initializeAdaptiveClusterTargets(originalFeatures, targetPorosity, rawPorosity)
    % 基于原始特征与目标孔隙率初始化簇目标
    ...
    % 统计量与安全处理
    baselineMin = max(1, round(prctile(sizes, 20)));
    baselineMax = max(baselineMin + 1, round(prctile(sizes, 80)));
    baselineMean = mean(sizes);
    baselineCount = max(1, double(originalFeatures.numClusters));

    porosityRatio = targetPorosity / max(rawPorosity, eps);
    porosityRatio = max(0.3, min(3.0, porosityRatio));
    sizeScale = nthroot(porosityRatio, 3);

    targets = struct();
    targets.minSize = max(1, round(baselineMin * sizeScale));
    targets.maxSize = max(targets.minSize + 1, round(baselineMax * sizeScale));
    targets.count = max(1, round(baselineCount * porosityRatio));

    adaptationState = struct();
    adaptationState.enabled = true;
    adaptationState.baseline = struct('minSize', baselineMin, 'maxSize', baselineMax, ...
        'meanSize', baselineMean, 'count', baselineCount);
    adaptationState.sizeScale = sizeScale;
    adaptationState.porosityRatio = porosityRatio;
    adaptationState.baseSmoothing = 0.25;
    adaptationState.correctionStrength = 0.35;
    adaptationState.explorationStrength = 0.05;
    adaptationState.updateInterval = 20;
    adaptationState.history = [];
    adaptationState.lastUpdate = 0;
    adaptationState.current = struct('minSize', targets.minSize, ...
        'maxSize', targets.maxSize, 'count', targets.count);

    % 允许范围，避免过度波动
    adaptationState.minBounds = [max(1, round(baselineMin * max(0.3, sizeScale * 0.5))), ...
        max(2, round(baselineMax * max(0.4, sizeScale * 0.5))), ...
        max(1, round(baselineCount * max(0.3, porosityRatio * 0.5)))];
    adaptationState.maxBounds = [round(baselineMin * max(2.0, sizeScale * 2.5)), ...
        round(baselineMax * max(2.5, sizeScale * 3.0)), ...
        round(baselineCount * max(1.8, porosityRatio * 2.2))];
end
function [mcmcState, performanceMonitor] = updateAdaptiveClusterTargetsDuringRun( ...
    mcmcState, performanceMonitor, iter)
    % 根据当前模型状态动态调整簇目标
    config = mcmcState.clusterAdaptation;
    if ~isstruct(config) || ~isfield(config, 'enabled') || ~config.enabled
        if isfield(performanceMonitor, 'acceptanceHistory')
            performanceMonitor.acceptanceHistory(iter) = mcmcState.acceptanceRate;
        end
        if isfield(performanceMonitor, 'targetMinHistory')
            performanceMonitor.targetMinHistory(iter) = mcmcState.optParams.targetMin;
            performanceMonitor.targetMaxHistory(iter) = mcmcState.optParams.targetMax;
            performanceMonitor.targetCountHistory(iter) = mcmcState.optParams.targetClusterCount;
        end
        return;
    end

    if isempty(config.lastUpdate)
        config.lastUpdate = 0;
    end

    baseline = config.baseline;
    desiredCount = max(1, round(baseline.count * config.porosityRatio));
    desiredMin = max(1, round(baseline.minSize * config.sizeScale));
    desiredMax = max(desiredMin + 1, round(baseline.maxSize * config.sizeScale));

    if isfield(config, 'updateInterval') && config.updateInterval > 1
        shouldUpdate = (iter == 1) || ((iter - config.lastUpdate) >= config.updateInterval);
    else
        shouldUpdate = true;
    end
    ...
    if ~isempty(mcmcState.features.sizes)
        sizes = double(mcmcState.features.sizes);
        currentMin = max(1, prctile(sizes, 20));
        currentMax = max(currentMin + 1, prctile(sizes, 80));
        currentMean = mean(sizes);
    else
        currentMin = double(mcmcState.optParams.targetMin);
        currentMax = double(mcmcState.optParams.targetMax);
        currentMean = (currentMin + currentMax) / 2;
    end
    currentCount = max(1, double(mcmcState.features.numClusters));

    morphMatch = max(0, min(1, mcmcState.currentMorphMatch));
    spatialMatch = max(0, min(1, mcmcState.currentSpatialMatch));
    mismatch = max(0, 1 - min(morphMatch, spatialMatch));

    smoothing = min(0.8, config.baseSmoothing + 0.3 * mismatch);
    correction = config.correctionStrength * (1 + mismatch);

    if mismatch > 0.25
        shouldUpdate = true;
    end

    sizeErrorMin = desiredMin - currentMin;
    sizeErrorMax = desiredMax - currentMax;
    countError = desiredCount - currentCount;

    exploration = (rand(1,3) - 0.5) * 2 * config.explorationStrength * max(0.2, mismatch + 0.1);

    if shouldUpdate
        targetMin = (1 - smoothing) * double(mcmcState.optParams.targetMin) + ...
            smoothing * (desiredMin + correction * sizeErrorMin) + exploration(1) * desiredMin;
        targetMax = (1 - smoothing) * double(mcmcState.optParams.targetMax) + ...
            smoothing * (desiredMax + correction * sizeErrorMax) + exploration(2) * desiredMax;
        targetCount = (1 - smoothing) * double(mcmcState.optParams.targetClusterCount) + ...
            smoothing * (desiredCount + correction * countError) + exploration(3) * desiredCount;

        % 边界限制
        targetMin = max(config.minBounds(1), min(config.maxBounds(1), round(targetMin)));
        targetMax = max(targetMin + 1, min(config.maxBounds(2), round(targetMax)));
        targetCount = max(config.minBounds(3), min(config.maxBounds(3), round(targetCount)));

        % 更新状态
        mcmcState.optParams.targetMin = targetMin;
        mcmcState.optParams.targetMax = targetMax;
        mcmcState.optParams.targetClusterCount = targetCount;

        config.current = struct('minSize', targetMin, 'maxSize', targetMax, ...
            'count', targetCount, 'meanSize', currentMean);
        config.lastUpdate = iter;
        config.history = [config.history; iter, targetMin, targetMax, targetCount, ...
            morphMatch, spatialMatch, currentMean]; %#ok<AGROW>
    else
        targetMin = mcmcState.optParams.targetMin;
        targetMax = mcmcState.optParams.targetMax;
        targetCount = mcmcState.optParams.targetClusterCount;
    end
    mcmcState.clusterAdaptation = config;

    if isfield(performanceMonitor, 'targetMinHistory')
        performanceMonitor.targetMinHistory(iter) = targetMin;
        performanceMonitor.targetMaxHistory(iter) = targetMax;
        performanceMonitor.targetCountHistory(iter) = targetCount;
    end
    if isfield(performanceMonitor, 'acceptanceHistory')
        performanceMonitor.acceptanceHistory(iter) = mcmcState.acceptanceRate;
    end
    ...
    % 同步optParams
    if isfield(mcmcState, 'optParams')
        mcmcState.optParams.clusterAdaptation = config;
    end
end
%% ========== 新增的模型调整函数 ==========
...
function mcmcState = initializeComprehensiveMCMCState(model, optParams, weights, parallelHint)
    ...
    % 自适应簇目标跟踪
    if isfield(optParams, 'clusterAdaptation')
        mcmcState.clusterAdaptation = optParams.clusterAdaptation;
        mcmcState.clusterAdaptation.enabled = true;
    else
        mcmcState.clusterAdaptation = struct('enabled', false);
    end
...
function performanceMonitor = initializeComprehensivePerformanceMonitor(maxIterations)
    ...
    performanceMonitor.acceptanceHistory = nan(1, maxIterations);
    performanceMonitor.targetMinHistory = nan(1, maxIterations);
    performanceMonitor.targetMaxHistory = nan(1, maxIterations);
    performanceMonitor.targetCountHistory = nan(1, maxIterations);
...
function displayComprehensiveFinalResults(originalModel, finalModel, ...
    ...
    if isfield(optParams, 'targetMin') && isfield(optParams, 'targetMax')
        fprintf('  自适应目标簇尺寸范围: [%d, %d]\n', ...
            round(optParams.targetMin), round(optParams.targetMax));
    end
    ...
    avgAcceptance = [];
    if isfield(performanceMonitor, 'acceptanceHistory')
        acceptance = performanceMonitor.acceptanceHistory;
        validIdx = ~isnan(acceptance);
        if any(validIdx)
            avgAcceptance = mean(acceptance(validIdx));
        end
    end
    if isempty(avgAcceptance)
        ratio = performanceMonitor.acceptanceRatio;
        ratio = ratio(ratio > 0);
        if ~isempty(ratio)
            avgAcceptance = mean(ratio);
        else
            avgAcceptance = 0;
        end
    end
    ...
    % 可视化最终对比
    plotAcceptanceEvolution(performanceMonitor);
    visualizeFinalComparison(originalModel, finalModel, performanceMonitor);
end
function plotAcceptanceEvolution(performanceMonitor)
    % 绘制接受率演化曲线（含自适应目标统计）
    if ~isfield(performanceMonitor, 'acceptanceHistory')
        return;
    end
    ...
    try
        ...
        yyaxis left;
        h = plot(find(validIdx), acceptance(validIdx), 'LineWidth', 1.5);
        handles(end+1) = h; %#ok<AGROW>
        labels{end+1} = '接受率'; %#ok<AGROW>
        ylabel('接受率');
        ylim([0, 1]);

        yyaxis right;
        ylabel('目标簇数量');
        if isfield(performanceMonitor, 'targetCountHistory')
            targetCount = performanceMonitor.targetCountHistory;
            countIdx = ~isnan(targetCount);
            if any(countIdx)
                hCount = plot(find(countIdx), targetCount(countIdx), '--', ...
                    'Color', [0.85, 0.33, 0.10], 'LineWidth', 1.2);
                handles(end+1) = hCount; %#ok<AGROW>
                labels{end+1} = '目标簇数量'; %#ok<AGROW>
            end
        end
        ...
        if isfield(performanceMonitor, 'targetMinHistory') && ...
                isfield(performanceMonitor, 'targetMaxHistory')
            minHist = performanceMonitor.targetMinHistory;
            maxHist = performanceMonitor.targetMaxHistory;
            idxMin = ~isnan(minHist);
            idxMax = ~isnan(maxHist);
            if any(idxMin)
                scaledMin = minHist(idxMin);
                if numel(scaledMin) > 1
                    scaledMin = (scaledMin - min(scaledMin)) ./ max(1e-6, (max(scaledMin) - min(scaledMin)));
                else
                    scaledMin = zeros(size(scaledMin));
                end
                hMin = plot(find(idxMin), scaledMin, ':', ...
                    'Color', [0.30, 0.75, 0.93], 'LineWidth', 1.0);
                handles(end+1) = hMin; %#ok<AGROW>
                labels{end+1} = '目标最小簇尺度(归一化)'; %#ok<AGROW>
            end
            if any(idxMax)
                scaledMax = maxHist(idxMax);
                if numel(scaledMax) > 1
                    scaledMax = (scaledMax - min(scaledMax)) ./ max(1e-6, (max(scaledMax) - min(scaledMax)));
                else
                    scaledMax = ones(size(scaledMax));
                end
                hMax = plot(find(idxMax), scaledMax, ':', ...
                    'Color', [0.13, 0.54, 0.13], 'LineWidth', 1.0);
                handles(end+1) = hMax; %#ok<AGROW>
                labels{end+1} = '目标最大簇尺度(归一化)'; %#ok<AGROW>
            end
        end
