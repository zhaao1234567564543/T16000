...
config = struct( ...
    ...
    'diagnosticMatchRaw', true, ...                % 诊断模式：直接对原始模型特征做匹配
...
pore_threshold = 120; % 仍然保留固定阈值以便对比
% 计算原始模型统计信息
raw_porosity = mean(rawModel(:) <= pore_threshold);
fprintf('原始灰度模型孔隙率：%.4f\n', raw_porosity);

% 统一使用同一个参考二值模型
referenceBinaryModel = adaptiveBinarizeModel(rawModel, raw_porosity);
...
% 诊断模式：可直接使用原始模型特征作为目标，验证算法极限能力
if config.diagnosticMatchRaw
    fprintf('\n已启用诊断模式：目标孔隙率与空间/形态特征直接对齐原始模型。\n');
    target_porosity = referencePorosity; %#ok<NASGU> % 记录意图
    new_target_spatial_features = spatialFeatures;
    morphologyFeatures = computeDetailedMorphologyFeatures(referenceBinaryModel); %#ok<NASGU> % 强化对原始形态的匹配
end
% 询问是否保留小孔隙特征
fprintf('\n小孔隙保留选项：\n');
...
function currentWeights = adjustWeightsForPhase(baseWeights, phase)
    ...
    switch phase
        case 'morphology'
            % 形态优化阶段
            currentWeights.morphology = baseWeights.morphology * 1.8;
            currentWeights.shapePreservation = baseWeights.shapePreservation * 1.6;
            currentWeights.directOverlap = baseWeights.directOverlap * 1.4; % 在早期直接向原始形态靠拢
            currentWeights.spatial = baseWeights.spatial * 0.65;
            currentWeights.multiScale = baseWeights.multiScale * 0.65;
