...
if isempty(target_porosity) || ~isfinite(target_porosity) || target_porosity <= 0
    warning('未提供有效的孔隙率目标，使用原始模型孔隙率。');
    target_porosity = raw_porosity;
end
target_porosity = max(1e-4, min(0.95, double(target_porosity)));
fprintf('\n>>> 启动自适应簇控制：仅使用目标孔隙率，簇大小与数量将在优化过程中自动探索。\n');
% 询问是否保留小孔隙特征
fprintf('\n小孔隙保留选项：\n');
...
optParams.modelVolume = prod(double(dims));
...
optParams.adaptiveTargets = initializeAdaptiveClusterTargets(optParams, originalFeatures, target_porosity);
optParams.targetMin = round(max(1, optParams.adaptiveTargets.currentMin));
optParams.targetMax = round(max(optParams.targetMin + 1, optParams.adaptiveTargets.currentMax));
optParams.targetClusterCount = round(max(1, optParams.adaptiveTargets.currentCount));
fprintf('初始自适应簇目标：最小=%d, 最大=%d, 数量=%d\n', ...
    optParams.targetMin, optParams.targetMax, optParams.targetClusterCount);
% 生成快速查找表
fprintf('正在生成快速查找表...\n');
...
for iter = 1:maxIterations
    ...
    % 自适应更新簇目标
    [mcmcState, targetsUpdated] = updateAdaptiveClusterTargetsForState( ...
        mcmcState, iter, maxIterations);
    optParams = mcmcState.optParams;
    if targetsUpdated && mod(iter, 100) == 0
        fprintf('  ⟳ 自适应簇目标: min=%d, max=%d, count=%d\n', ...
            optParams.targetMin, ...
            optParams.targetMax, ...
...
function adaptiveTargets = initializeAdaptiveClusterTargets(optParams, originalFeatures, targetPorosity)
    % 初始化自适应簇目标，基于原始特征和目标孔隙率动态估计
    adaptiveTargets = struct();

    if nargin < 2 || isempty(originalFeatures) || originalFeatures.numClusters == 0
        % 退化情况：构造基础默认值
        modelVolume = optParams.modelVolume;
        baseCount = max(8, round(modelVolume * targetPorosity / 500));
        adaptiveTargets.baseMin = 5;
        adaptiveTargets.baseMax = 80;
        adaptiveTargets.baseMean = 40;
        adaptiveTargets.baseCount = baseCount;
    else
        sizes = originalFeatures.sizes;
        adaptiveTargets.baseMin = max(1, round(prctile(sizes, 10)));
        adaptiveTargets.baseMax = max(adaptiveTargets.baseMin + 1, round(prctile(sizes, 95)));
        adaptiveTargets.baseMean = max(1, mean(sizes));
        adaptiveTargets.baseCount = max(1, double(originalFeatures.numClusters));
    end

    adaptiveTargets.minBounds = [max(1, floor(adaptiveTargets.baseMin * 0.5)), ...
        ceil(adaptiveTargets.baseMin * 1.6)];
    adaptiveTargets.maxBounds = [max(adaptiveTargets.minBounds(1) + 1, floor(adaptiveTargets.baseMax * 0.5)), ...
        ceil(adaptiveTargets.baseMax * 1.6)];
    adaptiveTargets.countBounds = [max(1, floor(adaptiveTargets.baseCount * 0.5)), ...
        ceil(max(adaptiveTargets.baseCount * 1.8, adaptiveTargets.baseCount + 5))];

    porosityScale = max(0.3, min(3.0, targetPorosity / max(optParams.originalPorosity, 1e-6)));
    % 初始估计
    adaptiveTargets.currentMin = min(adaptiveTargets.minBounds(2), ...
        max(adaptiveTargets.minBounds(1), adaptiveTargets.baseMin * porosityScale^0.55));
    adaptiveTargets.currentMax = min(adaptiveTargets.maxBounds(2), ...
        max(adaptiveTargets.maxBounds(1), adaptiveTargets.baseMax * porosityScale^0.9));

    baseTotalVolume = adaptiveTargets.baseCount * adaptiveTargets.baseMean;
    targetVolume = targetPorosity * optParams.modelVolume;
    if baseTotalVolume <= 0
        scaleCount = max(0.5, min(2.0, porosityScale));
    else
        scaleCount = max(0.4, min(2.5, (targetVolume / baseTotalVolume)^(0.5)));
    end
    adaptiveTargets.currentCount = min(adaptiveTargets.countBounds(2), ...
        max(adaptiveTargets.countBounds(1), adaptiveTargets.baseCount * scaleCount));

    adaptiveTargets.currentMean = max(1, (adaptiveTargets.currentMin + adaptiveTargets.currentMax) / 2);
    adaptiveTargets.learningRate = 0.18;
    adaptiveTargets.explorationFactor = 0.12;
    adaptiveTargets.history = struct('iteration', [], 'targetMin', [], 'targetMax', [], ...
        'targetCount', [], 'porosity', []);
end
function [mcmcState, updated] = updateAdaptiveClusterTargetsForState(mcmcState, iteration, maxIterations)
    % 在优化过程中根据当前模型特征自适应调整簇大小和数量目标
    updated = false;

    if ~isfield(mcmcState, 'optParams') || ...
            ~isfield(mcmcState.optParams, 'adaptiveTargets') || ...
            isempty(mcmcState.optParams.adaptiveTargets)
        return;
    end
    ...
    optParams = mcmcState.optParams;
    adaptiveTargets = optParams.adaptiveTargets;

    modelVolume = optParams.modelVolume;
    currentVolume = sum(features.sizes);
    currentPorosity = currentVolume / max(modelVolume, 1);
    targetPorosity = optParams.targetPorosity;
    porosityError = targetPorosity - currentPorosity;

    porosityScale = max(0.3, min(3.0, targetPorosity / max(optParams.originalPorosity, 1e-6)));

    desiredMean = adaptiveTargets.baseMean * porosityScale^0.82;
    desiredMin = adaptiveTargets.baseMin * porosityScale^0.6;
    desiredMax = adaptiveTargets.baseMax * porosityScale^0.95;

    currentStats = features.sizeStats;
    currentMean = currentStats(3);
    currentMin = currentStats(1);
    currentMax = currentStats(2);
    currentCount = features.numClusters;

    countFromVolume = targetPorosity * modelVolume / max(currentMean, 1);
    desiredCount = (adaptiveTargets.baseCount * porosityScale^0.45 + countFromVolume) / 2;

    progress = min(0.98, iteration / max(maxIterations, 1));
    dynamicRate = adaptiveTargets.learningRate * (1 - progress)^0.7;
    dynamicRate = max(0.02, min(0.3, dynamicRate));

    exploration = adaptiveTargets.explorationFactor * (0.5 - rand()) * (1 - progress);

    newMin = adaptiveTargets.currentMin + dynamicRate * ((desiredMin - currentMin) + 0.3 * (desiredMean - currentMean));
    newMax = adaptiveTargets.currentMax + dynamicRate * ((desiredMax - currentMax) + 0.7 * (desiredMean - currentMean));
    newCount = adaptiveTargets.currentCount + dynamicRate * ((desiredCount - currentCount) + ...
        0.4 * (porosityError / max(targetPorosity, 1e-6)) * adaptiveTargets.currentCount);

    newMin = newMin + exploration * adaptiveTargets.currentMin;
    newMax = newMax - exploration * adaptiveTargets.currentMax;
    newCount = newCount + exploration * adaptiveTargets.currentCount;

    newMin = min(adaptiveTargets.minBounds(2), max(adaptiveTargets.minBounds(1), newMin));
    newMax = min(adaptiveTargets.maxBounds(2), max(adaptiveTargets.maxBounds(1), newMax));
    newCount = min(adaptiveTargets.countBounds(2), max(adaptiveTargets.countBounds(1), newCount));

    if newMax <= newMin
        midpoint = (newMin + newMax) / 2;
        newMin = max(adaptiveTargets.minBounds(1), midpoint - 1);
        newMax = min(adaptiveTargets.maxBounds(2), midpoint + 1);
    end

    if abs(newMin - adaptiveTargets.currentMin) > 0.5 || ...
            abs(newMax - adaptiveTargets.currentMax) > 0.5 || ...
            abs(newCount - adaptiveTargets.currentCount) > 0.5
        updated = true;
    end

    adaptiveTargets.currentMin = newMin;
    adaptiveTargets.currentMax = newMax;
    adaptiveTargets.currentCount = newCount;
    adaptiveTargets.currentMean = max(1, (newMin + newMax) / 2);
    adaptiveTargets.history.iteration(end+1) = iteration; %#ok<AGROW>
    adaptiveTargets.history.targetMin(end+1) = newMin; %#ok<AGROW>
    adaptiveTargets.history.targetMax(end+1) = newMax; %#ok<AGROW>
    adaptiveTargets.history.targetCount(end+1) = newCount; %#ok<AGROW>
    adaptiveTargets.history.porosity(end+1) = currentPorosity; %#ok<AGROW>

    optParams.adaptiveTargets = adaptiveTargets;
    optParams.targetMin = round(max(1, newMin));
    optParams.targetMax = round(max(optParams.targetMin + 1, newMax));
    optParams.targetClusterCount = round(max(1, newCount));
...
function displayComprehensiveFinalResults(originalModel, finalModel, ...
    ...
    if isfield(optParams, 'adaptiveTargets') && ~isempty(optParams.adaptiveTargets)
        at = optParams.adaptiveTargets;
        fprintf('  自适应簇范围: min≈%.1f | max≈%.1f | mean≈%.1f\n', ...
            at.currentMin, at.currentMax, at.currentMean);
        fprintf('  探索记录次数: %d\n', numel(at.history.iteration));
    end
