...
config = struct( ...
    ...
    'constraintInterval', 90, ...                  % 强制约束与全特征重算间隔（由 40 改为 90）
    ...
    'diagnosticMatchRaw', true, ...                % 默认对齐原始空间/形态特征，孔隙率由用户设定
...
pore_threshold = input('请输入 CT 分割阈值（灰度，如 120）： ');
if isempty(pore_threshold) || ~isfinite(pore_threshold)
    warning('未提供有效阈值，使用默认阈值 120。');
    pore_threshold = 120;
end
% 基于用户阈值直接二值化，得到参考模型与实际样品孔隙率
referenceBinaryModel = rawModel <= pore_threshold;
referencePorosity = mean(referenceBinaryModel(:));
raw_porosity = referencePorosity; % 避免出现多个“原始孔隙率”定义
fprintf('样品参考二值模型孔隙率（实际孔隙率）：%.4f\n', referencePorosity);
...
if config.diagnosticMatchRaw
    optParams.spatialFeatures = spatialFeatures; % 匹配原始空间特征
    optParams.morphologyFeatures = baseMorphology; % 匹配原始形态特征
else
    optParams.spatialFeatures = new_target_spatial_features; % 使用梯度预测特征
...
initial_porosity = mean(mcmcModel(:));
%% 4. MCMC 迭代参数设置
...
targetRatios = struct('porosity', 0.30, 'morphology', 0.25, 'spatial', 0.25, ...
    'cluster', 0.04, 'connectivity', 0.04, 'multiScale', 0.04, ...
    'shapePreservation', 0.04, 'structureCoherence', 0.04, ...
    'directOverlap', 0.10); % 增加直接重叠匹配的占比
...
function binVol = adaptiveBinarizeModel(vol, raw_porosity)
    % 自适应二值化，使输出孔隙率逼近原始孔隙率
    vol_double = double(vol);
    % 使用二分搜索找到合适的阈值，评估时包含形态学清理对孔隙率的影响
    low = min(vol_double(:));
    high = max(vol_double(:));
    tol = 1e-3;
    maxIter = 80;
    ...
    for iter = 1:maxIter
        threshold = (low + high) / 2;
        candidate = vol_double <= threshold;
        % 在搜索过程中即刻应用形态学清理，确保孔隙率测量一致
        cleanedCandidate = imopen(candidate, strel('cube', 2));
        curr_por = mean(cleanedCandidate(:));
        if abs(curr_por - raw_porosity) <= tol
            binVol = cleanedCandidate;
            finalPorosity = curr_por;
            break;
        end
    ...
    % 如仍有偏差，进行微调保证单调逼近
    adjustStep = (high - low) / 20;
    while abs(finalPorosity - raw_porosity) > tol && adjustStep > eps
        if finalPorosity > raw_porosity
            threshold = threshold - adjustStep;
        else
            threshold = threshold + adjustStep;
        end
        candidate = imopen(vol_double <= threshold, strel('cube', 2));
        newPor = mean(candidate(:));
        if abs(newPor - raw_porosity) < abs(finalPorosity - raw_porosity)
            binVol = candidate;
            finalPorosity = newPor;
        else
            adjustStep = adjustStep / 2;
        end
    end
    fprintf('二值化完成，阈值: %.2f，孔隙率: %.4f\n', threshold, finalPorosity);
end
...
function finalModel = comprehensivePostProcessWithSmallPores(model, optParams)
    ...
    % 记录当前孔隙率并确定目标孔隙率（必须由用户提供）
    targetPorosity = optParams.targetPorosity;
    if isempty(targetPorosity) || ~isfinite(targetPorosity)
        error('targetPorosity 未定义：请在主函数中正确设置 optParams.targetPorosity。');
    end
    targetPorosity = max(0, min(1, targetPorosity));
...
function targetMorph = buildMorphologyTargetForPorosity(baseMorph, refPorosity, targetPorosity)
    ...
    if nargin < 3 || isempty(targetPorosity) || ~isfinite(targetPorosity)
        error('targetPorosity 未定义：请在主函数中正确设置目标孔隙率。');
    end
