...
function mcmcModel = generateComprehensiveInitialModel(dims, targetPorosity, ...
    ...
    model = harmonizeClusterPopulation(model, optParams, morphologyFeatures, spatialFeatures);
    % 5. 匹配孔隙尺寸分布
...
function model = harmonizeClusterPopulation(model, optParams, morphologyFeatures, spatialFeatures)
    % 在初始模型阶段进一步约束簇数量、形态和空间分布
    if nargin < 3 || isempty(morphologyFeatures)
        morphologyFeatures = struct();
    end
    if nargin < 4
        spatialFeatures = struct();
    ...
    tolerance = max(1, round(0.03 * targetCount));
    maxRounds = 5;
    for r = 1:maxRounds
        CC = bwconncomp(model, 26);
        currentCount = CC.NumObjects;
        ...
        stats = regionprops3(CC, 'Volume', 'Solidity');
        if abs(currentCount - targetCount) <= tolerance
            break;
        end
        if currentCount > targetCount
            % 过多簇：优先移除低致密度且体积小的簇
            [~, order] = sortrows([stats.Solidity, stats.Volume], [1, 2]);
            nRemove = min(currentCount - targetCount, max(1, round(0.25 * (currentCount - targetCount))));
            for i = 1:nRemove
                model(CC.PixelIdxList{order(i)}) = false;
            end
        else
            % 簇数量不足：对大簇执行分裂以提升簇数量和形态多样性
            model = splitLargeClustersToMatchCount(model, CC, stats, targetCount);
        end
        model = smoothClusterBoundaries(model);
        model = enforceLocalSpacingAgainstReference(model, optParams, spatialFeatures);
    end
    % 最后再执行一次空间匹配以避免局部拥挤
    model = enforceLocalSpacingAgainstReference(model, optParams, spatialFeatures);
end
function model = splitLargeClustersToMatchCount(model, CC, stats, targetCount)
    % 使用距离变换+分水岭分解大簇，提升簇数量并靠近目标统计
    deficit = targetCount - CC.NumObjects;
    if deficit <= 0 || isempty(stats)
        return;
    end
    [~, order] = sort(stats.Volume, 'descend');
    for i = 1:min(numel(order), max(1, deficit))
        idx = order(i);
        ...
        segSizes = zeros(numel(labels), 1);
        for k = 1:numel(labels)
            segSizes(k) = nnz(L == labels(k));
        end
        [~, segOrder] = sort(segSizes, 'descend');
        keep = segOrder(1:min(numel(segOrder), deficit + 1));
        newMask = false(size(mask));
        for k = 1:numel(keep)
            newMask = newMask | (L == labels(keep(k)));
        end
        model(mask) = false;
        model = model | newMask;
        CC = bwconncomp(model, 26);
...
function model = smoothClusterBoundaries(model)
    % 轻量化的簇边界平滑，避免过度细碎或锯齿
    se = strel('sphere', 1);
    model = imclose(model, se);
    model = imopen(model, se);
end
function model = enforceLocalSpacingAgainstReference(model, optParams, spatialFeatures)
    % 结合参考密度图与各向异性，抑制拥挤区域并填补稀疏区域
    if nargin < 3
        spatialFeatures = struct();
    end
    if ~isfield(optParams, 'referenceDensityMap') || isempty(optParams.referenceDensityMap)
    ...
    targetMap = optParams.referenceDensityMap;
    if ~isequal(size(targetMap), size(model))
        try
            targetMap = imresize3(targetMap, size(model), 'linear');
        catch
            targetMap = imresize(targetMap, [size(model, 1), size(model, 2)]);
            if size(model, 3) > 1
                targetMap = repmat(targetMap, [1, 1, size(model, 3)]);
            end
        end
    end
    targetMap = imgaussfilt3(targetMap, 1);
    kernel = ones(5, 5, 5) / 125;
    localDensity = convn(double(model), kernel, 'same');
    crowded = localDensity > targetMap + 0.05;
    sparse = localDensity < targetMap - 0.05;
    if isfield(spatialFeatures, 'anisotropy') && ~isempty(spatialFeatures.anisotropy)
        aniso = spatialFeatures.anisotropy;
        dirWeights = normalizeAnisotropyWeights(aniso);
    else
        dirWeights = [1, 1, 1];
    end
    % 从拥挤区域按方向权重移除体素
    if any(crowded(:))
        removeMask = crowded & model;
        randField = rand(size(model));
        weightedField = randField .* buildDirectionalWeightField(size(model), dirWeights);
        threshold = quantile(weightedField(removeMask), 0.4);
        model(removeMask & weightedField <= threshold) = false;
    end
    % 在稀疏且接近现有簇的区域补充体素
    if any(sparse(:))
        growRegion = imdilate(model, strel('sphere', 1)) & sparse;
        candidates = growRegion & ~model;
        if any(candidates(:))
            addField = buildDirectionalWeightField(size(model), dirWeights) .* rand(size(model));
            threshold = quantile(addField(candidates), 0.6);
            model(candidates & addField >= threshold) = true;
        end
    end
    model = smoothClusterBoundaries(model);
end
function weights = normalizeAnisotropyWeights(anisotropy)
    % 将各向异性分量转化为概率权重
    if isempty(anisotropy) || numel(anisotropy) < 3
        weights = [1, 1, 1];
        return;
    end
    vals = max(eps, anisotropy(:)');
    weights = vals / sum(vals);
end
function field = buildDirectionalWeightField(sz, weights)
    % 构建方向性偏置场，引导增删体素优先方向
    if numel(weights) < 3
        weights = [1, 1, 1];
    end
    [x, y, z] = ndgrid(linspace(-1, 1, sz(1)), linspace(-1, 1, sz(2)), linspace(-1, 1, sz(3)));
    field = abs(weights(1) * x) + abs(weights(2) * y) + abs(weights(3) * z);
    field = field / (max(field(:)) + eps);
end
