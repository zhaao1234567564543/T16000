...
fprintf('\n孔隙簇参数设置：\n');
fprintf('1. 手动设定孔隙簇大小和数量\n');
fprintf('2. 跳过，自动根据原始模型的最优空间特征生成\n');
cluster_setting_choice = input('请选择 (1 或 2): ');
if isempty(cluster_setting_choice) || ~ismember(cluster_setting_choice, [1, 2])
    cluster_setting_choice = 1;
end
auto_cluster_config = (cluster_setting_choice == 2);
if auto_cluster_config
    fprintf('\n已选择自动孔隙簇配置，将在初始模型生成过程中依据原始模型的簇空间特征随机生成目标。\n');
    autoClusterTargets = initializeAutoClusterConfig(originalFeatures, spatialFeatures, morphologyFeatures);
    target_max = autoClusterTargets.currentTargets.maxSize;
    target_min = autoClusterTargets.currentTargets.minSize;
    target_cluster_count = autoClusterTargets.currentTargets.count;
else
    target_max = input('请输入目标最大孔隙簇: ');
    if isempty(target_max) || ~isfinite(target_max)
        warning('未提供有效的最大孔隙簇尺寸，默认使用原始模型统计值。');
        target_max = originalFeatures.sizeStats(2);
    end
    target_min = input('请输入目标最小孔隙簇: ');
    if isempty(target_min) || ~isfinite(target_min)
        warning('未提供有效的最小孔隙簇尺寸，默认使用原始模型统计值。');
        target_min = max(1, originalFeatures.sizeStats(1));
    end
...
optParams.autoClusterConfig = auto_cluster_config;
if auto_cluster_config
    optParams.autoClusterTargets = autoClusterTargets;
else
    optParams.autoClusterTargets = [];
end
...
[mcmcModel, optParams] = generateComprehensiveInitialModel(dims, target_porosity, originalFeatures, ...
    spatialFeatures, morphologyFeatures, optParams);
optParams = refreshAutoClusterTargets(optParams, 'initial');
%% 4. MCMC 迭代参数设置
...
if optParams.autoClusterConfig
    weights.cluster = 3.5;
    weights.morphology = weights.morphology * 1.2;
    weights.spatial = weights.spatial * 1.2;
    weights.structureCoherence = weights.structureCoherence * 1.1;
end
optParams = refreshAutoClusterTargets(optParams, 'mcmc');
% 初始化MCMC状态
...
for iter = 1:maxIterations
    iterStart = tic;
    if optParams.autoClusterConfig && mod(iter, 150) == 1
        optParams = refreshAutoClusterTargets(optParams, 'mcmc');
...
%% ========== 自动簇目标配置函数 ==========
function autoConfig = initializeAutoClusterConfig(originalFeatures, spatialFeatures, morphologyFeatures)
    % 根据原始模型特征构建自动孔隙簇目标配置
    if nargin < 1 || isempty(originalFeatures)
        originalFeatures = struct();
    end
    autoConfig = struct();
    if isfield(originalFeatures, 'sizes') && ~isempty(originalFeatures.sizes)
        ...
        sizes = linspace(6, 24, 12)';
    end
    if isempty(sizes)
        sizes = linspace(4, 18, 8)';
    end
    baseMin = max(1, round(quantile(sizes, 0.2)));
    baseMax = max(baseMin + 1, round(quantile(sizes, 0.8)));
    minRange = [max(1, round(quantile(sizes, 0.1))), max(1, round(quantile(sizes, 0.35)))];
    if diff(minRange) <= 0
        minRange(2) = minRange(1) + max(2, round(baseMin * 0.25));
    end
    maxRange = [max(baseMin + 1, round(quantile(sizes, 0.65))), ...
        max(baseMin + 2, round(quantile(sizes, 0.95)))];
    if diff(maxRange) <= 0
        maxRange(2) = maxRange(1) + max(3, round(baseMax * 0.25));
    end
    if isfield(originalFeatures, 'numClusters') && ~isempty(originalFeatures.numClusters)
        baseCount = double(originalFeatures.numClusters);
    else
        baseCount = max(numel(sizes), 1);
    end
    countRange = [max(1, round(baseCount * 0.85)), max(1, round(baseCount * 1.15))];
    if diff(countRange) <= 0
        countRange(2) = countRange(1) + max(1, round(baseCount * 0.15));
    end
    minSize = min(sizes);
    maxSize = max(sizes);
    if minSize == maxSize
    ...
    nBins = min(12, max(6, round(sqrt(numel(sizes)))));
    edges = linspace(minSize, maxSize, nBins + 1);
    edges = unique(edges);
    if numel(edges) < 2
        edges = [minSize, maxSize + 1];
    end
    histVals = computeAutoClusterHistogram(sizes, edges);
    autoConfig.minRange = minRange;
    autoConfig.maxRange = maxRange;
    autoConfig.countRange = countRange;
    autoConfig.meanSize = mean(sizes);
    autoConfig.stdSize = std(sizes);
    if autoConfig.stdSize == 0
        autoConfig.stdSize = max(1, autoConfig.meanSize * 0.1);
    end
    autoConfig.sizeHistogramEdges = edges;
    autoConfig.sizeDistribution = histVals;
    if isfield(originalFeatures, 'spatialUniformity') && ~isempty(originalFeatures.spatialUniformity)
        autoConfig.spatialUniformity = max(1e-3, originalFeatures.spatialUniformity);
    else
        autoConfig.spatialUniformity = 0.5;
    end
    if isfield(originalFeatures, 'compactness') && ~isempty(originalFeatures.compactness)
        validCompactness = originalFeatures.compactness(~isnan(originalFeatures.compactness));
        if ~isempty(validCompactness)
            autoConfig.compactness = mean(validCompactness);
        else
            autoConfig.compactness = 0.6;
        end
    else
        autoConfig.compactness = 0.6;
    end
    autoConfig.morphologySignature = struct('elongation', [], 'sphericity', [], 'solidity', []);
    if nargin >= 3 && ~isempty(morphologyFeatures)
        if isfield(morphologyFeatures, 'elongation') && ~isempty(morphologyFeatures.elongation)
            autoConfig.morphologySignature.elongation = mean(morphologyFeatures.elongation);
        end
        if isfield(morphologyFeatures, 'sphericity') && ~isempty(morphologyFeatures.sphericity)
            autoConfig.morphologySignature.sphericity = mean(morphologyFeatures.sphericity);
        end
        if isfield(morphologyFeatures, 'solidity') && ~isempty(morphologyFeatures.solidity)
            autoConfig.morphologySignature.solidity = mean(morphologyFeatures.solidity);
        end
    end
    if nargin >= 2 && ~isempty(spatialFeatures) && isfield(spatialFeatures, 'anisotropy')
        autoConfig.referenceAnisotropy = spatialFeatures.anisotropy;
    else
...
function optParams = refreshAutoClusterTargets(optParams, context)
    ...
    if ~isfield(optParams, 'autoClusterTargets') || isempty(optParams.autoClusterTargets)
        optParams.autoClusterTargets = initializeAutoClusterConfig(optParams.originalFeatures, ...
            optParams.spatialFeatures, optParams.morphologyFeatures);
    end
    optParams.autoClusterTargets = sampleAutoClusterTargets(optParams.autoClusterTargets, context);
...
function autoConfig = sampleAutoClusterTargets(autoConfig, context)
    ...
    if ~isfield(autoConfig, 'minRange') || isempty(autoConfig.minRange)
        autoConfig.minRange = [5, 15];
    end
    if ~isfield(autoConfig, 'maxRange') || isempty(autoConfig.maxRange)
        autoConfig.maxRange = [20, 60];
    end
    if ~isfield(autoConfig, 'countRange') || isempty(autoConfig.countRange)
        autoConfig.countRange = [10, 50];
    end
    bias = 0.5;
    ...
    jitter = @(range, baseBias) range(1) + min(max(baseBias + 0.25 * randn(), 0), 1) * (range(2) - range(1));
    minSize = max(1, round(jitter(autoConfig.minRange, bias)));
    maxBias = min(max(bias + 0.1, 0), 1);
    maxSize = round(jitter(autoConfig.maxRange, maxBias));
    if maxSize <= minSize
        maxSize = minSize + max(2, round(minSize * 0.25));
    end
    countBias = min(max(bias + 0.05 * randn(), 0), 1);
    count = max(1, round(jitter(autoConfig.countRange, countBias)));
    autoConfig.currentTargets = struct();
    autoConfig.currentTargets.minSize = minSize;
...
function histVals = computeAutoClusterHistogram(sizes, edges)
    ...
    if nargin < 2 || isempty(edges)
        edges = linspace(min(sizes), max(sizes) + 1, 7);
    end
    edges = unique(edges);
    if numel(edges) < 2
        edges = [edges(1), edges(1) + 1];
    end
    histVals = histcounts(sizes, edges, 'Normalization', 'probability');
    if isempty(histVals)
        histVals = ones(length(edges) - 1, 1) / max(length(edges) - 1, 1);
    end
    histVals = histVals(:)';
    if all(histVals == 0)
...
function [mcmcModel, optParams] = generateComprehensiveInitialModel(dims, targetPorosity, ...
    originalFeatures, spatialFeatures, morphologyFeatures, optParams)
    % 生成综合优化的初始模型
    fprintf('  生成综合优化的初始模型...');
    if nargin >= 6 && isfield(optParams, 'autoClusterConfig') && optParams.autoClusterConfig
        optParams = refreshAutoClusterTargets(optParams, 'initial');
    end
    % 1. 基于空间相关性与各向异性生成基础场
    ...
    if isfield(optParams, 'targetMax') || isfield(optParams, 'targetMin')
        ...
        if CC.NumObjects > 0
            ...
            if isfield(optParams, 'autoClusterConfig') && optParams.autoClusterConfig && ...
                    ...
                    isfield(optParams.autoClusterTargets, 'currentTargets')
                sizeTolerance = optParams.autoClusterTargets.currentTargets.sizeTolerance;
            end
            if isfield(optParams, 'targetMax') && ~isempty(optParams.targetMax)
                targetMax = max(1, double(optParams.targetMax));
                if isempty(sizeTolerance)
                    maxTolerance = max(targetMax * 1.1, targetMax + max(8, round(0.03 * targetMax)));
                else
                    maxTolerance = targetMax + sizeTolerance;
                end
            ...
            if isfield(optParams, 'targetMin') && ~isempty(optParams.targetMin)
                ...
                if isempty(sizeTolerance)
                    ...
                    minTolerance = max(3, targetMin - sizeTolerance);
                end
...
function E_cluster = calculateClusterEnergy(features, optParams)
    ...
    if isfield(optParams, 'autoClusterConfig') && optParams.autoClusterConfig
        autoCfg = struct();
        if isfield(optParams, 'autoClusterTargets') && ~isempty(optParams.autoClusterTargets)
            autoCfg = optParams.autoClusterTargets;
        else
            autoCfg = initializeAutoClusterConfig(optParams.originalFeatures, ...
                optParams.spatialFeatures, optParams.morphologyFeatures);
        end
        if ~isfield(autoCfg, 'currentTargets') || isempty(autoCfg.currentTargets)
            autoCfg = sampleAutoClusterTargets(autoCfg, 'mcmc');
        end
        currentTargets = autoCfg.currentTargets;
        if isfield(autoCfg, 'sizeHistogramEdges') && ~isempty(autoCfg.sizeHistogramEdges)
            sizeEdges = autoCfg.sizeHistogramEdges;
        else
            sizeEdges = linspace(min(features.sizes), max(features.sizes) + 1, 7);
        end
        referenceHist = autoCfg.sizeDistribution;
        currentHist = computeAutoClusterHistogram(features.sizes, sizeEdges);
        if numel(referenceHist) ~= numel(currentHist)
            referenceHist = interp1(linspace(0, 1, numel(referenceHist)), referenceHist, ...
                linspace(0, 1, numel(currentHist)), 'linear', 'extrap');
            referenceHist(~isfinite(referenceHist)) = 0;
            referenceHist = referenceHist / (sum(referenceHist) + eps);
        end
        histDiff = mean(abs(currentHist - referenceHist));
        meanSize = mean(features.sizes);
        meanDiff = abs(meanSize - autoCfg.meanSize) / (autoCfg.meanSize + eps);
        stdSize = std(double(features.sizes));
        stdDiff = abs(stdSize - autoCfg.stdSize) / (autoCfg.stdSize + eps);
        uniformityTarget = autoCfg.spatialUniformity;
        uniformityDiff = abs(features.spatialUniformity - uniformityTarget) / (uniformityTarget + 1e-3);
        compactnessDiff = 0;
        if isfield(features, 'compactness') && ~isempty(features.compactness)
            currentCompactness = mean(features.compactness);
            compactnessDiff = abs(currentCompactness - currentTargets.compactness) / ...
        ...
        if isfield(currentTargets, 'minSize') && isfield(currentTargets, 'maxSize')
            if features.sizeStats(1) < currentTargets.minSize * 0.8
                sizeRangePenalty = sizeRangePenalty + ...
                    (currentTargets.minSize * 0.8 - features.sizeStats(1)) / (currentTargets.minSize + eps);
            end
            if features.sizeStats(2) > currentTargets.maxSize * 1.2
                sizeRangePenalty = sizeRangePenalty + ...
                    (features.sizeStats(2) - currentTargets.maxSize * 1.2) / (currentTargets.maxSize + eps);
            end
        end
...
function model = enforceClusterSizeConstraints(model, optParams)
    ...
    if isfield(optParams, 'autoClusterConfig') && optParams.autoClusterConfig && ...
            ...
            isfield(optParams.autoClusterTargets, 'currentTargets')
        currentTargets = optParams.autoClusterTargets.currentTargets;
        if isfield(currentTargets, 'sizeTolerance') && ~isempty(currentTargets.sizeTolerance)
            sizeTolerance = currentTargets.sizeTolerance;
        end
        if isempty(targetMax) && isfield(currentTargets, 'maxSize')
            targetMax = max(1, double(currentTargets.maxSize));
        end
        if isempty(targetMin) && isfield(currentTargets, 'minSize')
            targetMin = max(1, double(currentTargets.minSize));
        end
    end
    if ~isempty(targetMax)
        if isempty(sizeTolerance)
            ...
            largeThreshold = targetMax + sizeTolerance;
        end
    else
    ...
    if ~isempty(targetMin)
        if isempty(sizeTolerance)
            ...
            removeThreshold = max(1, targetMin - sizeTolerance);
            growThreshold = max(removeThreshold + 1, targetMin + floor(sizeTolerance / 2));
        end
    else
...
function model = enforceTargetClusterCount(model, optParams)
    ...
    if isfield(optParams, 'autoClusterConfig') && optParams.autoClusterConfig && ...
        ...
        acceptableRange = optParams.autoClusterTargets.countRange;
        acceptableRange = max(1, round(acceptableRange));
