...
function mcmcModel = generateComprehensiveInitialModel(dims, targetPorosity, ...
    ...
    % 生成综合优化的初始模型（基于簇对象的目标导向建模）
    fprintf('  生成综合优化的初始模型...');
    % 1. 构造多尺度密度引导场
    densityMap = constructMultiScaleDensityMap(optParams, dims);
    referenceBinary = [];
    if isfield(optParams, 'originalBinaryModel') && ~isempty(optParams.originalBinaryModel)
        referenceBinary = optParams.originalBinaryModel;
    end
    % 2. 基于参考簇统计直接生成初始簇模型，避免超大连通簇
    [clusterModel, clusterSummary] = generateClusterBasedInitialModel(dims, targetPorosity, ...
        referenceBinary, densityMap, optParams);
    % 3. 仅为补充微观纹理引入小尺度方向性场，不再主导大尺度簇结构
    targetTPC = spatialFeatures.twoPointCorr;
    targetAnisotropy = spatialFeatures.anisotropy;
    correlationLength = max(2, round(min(dims) / 30));
    if ~isempty(targetTPC)
        correlationLength = max(correlationLength, estimateCorrelationLength(targetTPC));
    end
    microField = synthesizeDirectionalGaussianField(dims, correlationLength, ...
        targetAnisotropy, spatialFeatures, optParams);
    blendedModel = blendReferenceDirectionalPatterns(clusterModel, microField);
    [model, sequentialSummary] = finalizeInitialCandidateModel(blendedModel, densityMap, ...
        targetPorosity, originalFeatures, spatialFeatures, morphologyFeatures, optParams, true);
    fprintf('\n    簇生成候选 -> 簇数: %d, 空间: %.3f, 形态: %.3f, 孔隙率偏差: %.3f\n', ...
        clusterSummary.clusterCount, ...
        sequentialSummary.spatialMatch, ...
        sequentialSummary.morphologyMatch, ...
        abs(sequentialSummary.porosity - targetPorosity));
    % 6. 多指标确定性优化（保留，但依赖新簇模型的稳定拓扑）
    nOptSteps = 40;
...
function [model, summary] = generateClusterBasedInitialModel(dims, targetPorosity, referenceBinaryModel, densityMap, optParams)
    % 基于簇对象直接构建初始模型，避免产生超级大簇
    if nargin < 3 || isempty(referenceBinaryModel)
        warning('缺少参考二值模型，使用均匀随机场作为参考。');
        referenceBinaryModel = rand(dims) < targetPorosity;
    end
    stats = collectReferenceClusterStats(referenceBinaryModel);
    targetCount = round(stats.numClusters * (targetPorosity / max(stats.porosityRef, eps)));
    targetCount = max(1, targetCount);
    totalVolume = targetPorosity * prod(dims);
    [volumes, muV, sigmaV] = sampleTargetClusterVolumes(stats, targetCount, totalVolume);
    clusterSpecs = buildClusterSpecs(volumes, stats);
    centers = placeClusterCenters(clusterSpecs, dims, densityMap, stats.hardcoreRadius);
    for i = 1:numel(clusterSpecs)
        clusterSpecs(i).center = centers(i, :);
    end
    model = renderEllipsoidClusters(clusterSpecs, dims, optParams);
    phiInit = mean(model(:));
    if abs(phiInit - targetPorosity) > 0.002
        globalScale = nthroot(max(eps, targetPorosity) / max(eps, phiInit), 3);
        globalScale = min(max(globalScale, 0.85), 1.15);
        for i = 1:numel(clusterSpecs)
            clusterSpecs(i).axes = clusterSpecs(i).axes * globalScale;
        end
        model = renderEllipsoidClusters(clusterSpecs, dims, optParams);
    end
    summary = struct();
    summary.clusterCount = numel(clusterSpecs);
    summary.porosity = mean(model(:));
    summary.muVolume = muV;
    summary.sigmaVolume = sigmaV;
end

function stats = collectReferenceClusterStats(binaryModel)
    % 提取参考模型的簇级统计量
    CC = bwconncomp(binaryModel, 26);
    stats.numClusters = CC.NumObjects;
    stats.porosityRef = mean(binaryModel(:));
    if stats.numClusters == 0
        stats.volumes = 1;
        stats.axisRatios = ones(1, 3);
        stats.orientations = eye(3);
        stats.hardcoreRadius = 1.5;
        stats.muVolume = log(1);
        stats.sigmaVolume = 0.25;
        return;
    end
    volumes = cellfun(@numel, CC.PixelIdxList);
    stats.volumes = volumes;
    centers = zeros(stats.numClusters, 3);
    axisRatios = zeros(stats.numClusters, 3);
    orientations = cell(stats.numClusters, 1);
    for i = 1:stats.numClusters
        idx = CC.PixelIdxList{i};
        [x, y, z] = ind2sub(size(binaryModel), idx);
        coords = [x, y, z];
        centers(i, :) = mean(coords, 1);
        covMat = cov(double(coords));
        covMat = covMat + eye(3) * eps;
        [Q, D] = eig(covMat);
        lambda = max(diag(D), eps);
        axisLen = sqrt(lambda(:)');
        axisLen = axisLen / (prod(axisLen)^(1/3) + eps);
        axisRatios(i, :) = axisLen;
        orientations{i} = Q;
    end
    stats.centers = centers;
    stats.axisRatios = axisRatios;
    stats.orientations = orientations;
    nnDist = inf(stats.numClusters, 1);
    if stats.numClusters > 1
        try
            ...
            for i = 1:stats.numClusters
                ...
                d = sqrt(sum(deltas.^2, 2));
                d(i) = inf;
                nnDist(i) = min(d);
            end
        end
    end
    stats.hardcoreRadius = max(1.0, prctile(nnDist, 10));
    stats.muVolume = mean(log(max(volumes, 1)));
    stats.sigmaVolume = max(std(log(max(volumes, 1))), 0.1);
end

function [volumes, muV, sigmaV] = sampleTargetClusterVolumes(stats, targetCount, totalVolume)
    % 从对数正态分布采样簇体积并缩放至目标总量
    muV = stats.muVolume;
    sigmaV = stats.sigmaVolume;
    volumes = lognrnd(muV, sigmaV, targetCount, 1);
    scaleFactor = totalVolume / max(sum(volumes), eps);
    volumes = volumes * scaleFactor;
end

function clusterSpecs = buildClusterSpecs(volumes, stats)
    % 根据参考形态构造目标簇的椭球参数
    n = numel(volumes);
    clusterSpecs = repmat(struct('axes', [], 'rotation', [], 'center', []), n, 1);
    for i = 1:n
        templateIdx = 1 + mod(i-1, size(stats.axisRatios, 1));
        axisRatio = stats.axisRatios(templateIdx, :);
        rotation = stats.orientations{templateIdx};
        volume = volumes(i);
        baseScale = nthroot(volume / ((4/3) * pi * prod(axisRatio)), 3);
        axes = baseScale * axisRatio;
        clusterSpecs(i).axes = axes;
        clusterSpecs(i).rotation = rotation;
        clusterSpecs(i).center = [NaN, NaN, NaN];
    end
end

function centers = placeClusterCenters(clusterSpecs, dims, densityMap, hardcoreRadius)
    % 顺序放置簇中心，利用密度引导并加入硬核距离约束
    n = numel(clusterSpecs);
    centers = zeros(n, 3);
    if isempty(densityMap)
        weights = ones(dims);
    else
        weights = max(densityMap, 0);
    end
    ...
    for i = 1:n
        ...
        while ~placed && attempts < maxAttempts
            ...
            distances = sqrt(sum((centers(1:i-1, :) - candidate).^2, 2));
            minGap = hardcoreRadius + eta * mean(clusterSpecs(i).axes);
            if all(distances >= minGap)
                centers(i, :) = candidate;
                placed = true;
            end
        end
        if ~placed
            centers(i, :) = min(max(candidate, 1), dims);
        end
    end
end

function model = renderEllipsoidClusters(clusterSpecs, dims, optParams)
    % 将椭球簇对象栅格化到三维网格，并控制重叠比例
    model = false(dims);
    overlapLimit = 0.28;
    if nargin >= 3 && isfield(optParams, 'maxInitialOverlap') && ~isempty(optParams.maxInitialOverlap)
        overlapLimit = optParams.maxInitialOverlap;
    end
    for i = 1:numel(clusterSpecs)
        spec = clusterSpecs(i);
        center = spec.center;
        axes = max(spec.axes, 1);
        R = spec.rotation;
        bboxMin = max(floor(center - axes - 2), 1);
        bboxMax = min(ceil(center + axes + 2), dims);
        [X, Y, Z] = ndgrid(bboxMin(1):bboxMax(1), bboxMin(2):bboxMax(2), bboxMin(3):bboxMax(3));
        coords = [X(:) - center(1), Y(:) - center(2), Z(:) - center(3)] * R;
        ellipsoidVals = (coords(:, 1) ./ axes(1)).^2 + ...
            (coords(:, 2) ./ axes(2)).^2 + ...
            (coords(:, 3) ./ axes(3)).^2;
        maskLocal = reshape(ellipsoidVals <= 1, size(X));
        subModel = false(dims);
        subModel(bboxMin(1):bboxMax(1), bboxMin(2):bboxMax(2), bboxMin(3):bboxMax(3)) = maskLocal;
        overlapRatio = nnz(subModel & model) / max(1, nnz(subModel));
        attempt = 0;
        while overlapRatio > overlapLimit && attempt < 8
            jitter = (rand(1, 3) - 0.5) .* max(1, axes) * 0.5;
            center = min(max(center + jitter, 1), dims);
            bboxMin = max(floor(center - axes - 2), 1);
            bboxMax = min(ceil(center + axes + 2), dims);
            [X, Y, Z] = ndgrid(bboxMin(1):bboxMax(1), bboxMin(2):bboxMax(2), bboxMin(3):bboxMax(3));
            coords = [X(:) - center(1), Y(:) - center(2), Z(:) - center(3)] * R;
...
function E_cluster = calculateClusterEnergy(features, optParams)
    ...
    if isempty(clusterRef) || ~isfield(clusterRef, 'meanSize') || clusterRef.meanSize <= 0
        ...
        clusterRef.cdfEdges = clusterRef.histEdges;
        clusterRef.sizeCDF = cumsum(histcounts(log10(sizes+1), clusterRef.cdfEdges, 'Normalization', 'cdf'));
    end
    logSizes = log10(sizes + 1);
    refEdges = clusterRef.cdfEdges;
    currentCDF = cumsum(histcounts(logSizes, refEdges, 'Normalization', 'cdf'));
    if numel(currentCDF) ~= numel(clusterRef.sizeCDF)
        currentCDF = interp1(linspace(0, 1, numel(currentCDF)), currentCDF, ...
            linspace(0, 1, numel(clusterRef.sizeCDF)), 'linear', 'extrap');
        currentCDF(isnan(currentCDF)) = 0;
    end
    % 基于Kolmogorov-Smirnov的分布差异
    E_CDF = max(abs(currentCDF - clusterRef.sizeCDF));
    % 簇数量偏差
    N = features.numClusters;
    N0 = max(1, clusterRef.numClusters);
    E_N = ((N - N0) / N0) ^ 2;
    wN = 0.5; wCDF = 0.5;
    E_cluster = wN * E_N + wCDF * E_CDF;
end
