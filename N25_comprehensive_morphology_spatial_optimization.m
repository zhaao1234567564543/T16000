...
function mcmcModel = generateComprehensiveInitialModel(dims, targetPorosity, ...
    originalFeatures, spatialFeatures, morphologyFeatures, optParams) %#ok<INUSD>
    % 生成初始模型：以目标孔隙率为唯一约束，按簇统计采样
    fprintf('  生成综合优化的初始模型...
');
    vol = prod(dims);
    targetCount = max(1, round(optParams.targetClusterCount));
    ref = optParams.clusterReference;
    meanSize = max(1, ref.meanSize);
    stdSize = max(1, ref.stdSize);
    maxSize = max(1, ref.maxSize * 1.2);
    minSize = max(1, ref.minSize);
    targetVolume = round(targetPorosity * vol);

    sizes = [];
    while sum(sizes) < targetVolume && numel(sizes) < targetCount * 2
        candidate = round(meanSize + stdSize * randn());
        candidate = min(maxSize, max(minSize, candidate));
        sizes(end+1) = candidate; %#ok<AGROW>
    end
    if numel(sizes) > round(1.1 * targetCount)
        sizes = sizes(1:round(1.1 * targetCount));
    elseif numel(sizes) < round(0.9 * targetCount)
        sizes = [sizes, repmat(round(mean(sizes)), 1, round(0.9 * targetCount) - numel(sizes))];
    end

    radii = nthroot((3 * sizes) / (4 * pi), 3);
    avgRadius = nthroot((3 * meanSize) / (4 * pi), 3);
    minSpacing = max(1, avgRadius * 2);

    centers = zeros(numel(sizes), 3);
    placed = 0;
    attempts = 0;
    while placed < numel(sizes) && attempts < 5000
        attempts = attempts + 1;
        ...
        dists = sqrt(sum((centers(1:placed, :) - candidate).^2, 2));
        if all(dists >= minSpacing)
            placed = placed + 1;
            centers(placed, :) = candidate;
        end
    end
    if placed < numel(sizes)
        centers = centers(1:placed, :);
        sizes = sizes(1:placed);
    ...
    for i = 1:numel(sizes)
        r = radii(i);
        [xg, yg, zg] = ndgrid(max(1, round(centers(i,1)-r)):min(dims(1), round(centers(i,1)+r)), ...
            max(1, round(centers(i,2)-r)):min(dims(2), round(centers(i,2)+r)), ...
            max(1, round(centers(i,3)-r)):min(dims(3), round(centers(i,3)+r)));
        mask = (xg - centers(i,1)).^2 + (yg - centers(i,2)).^2 + (zg - centers(i,3)).^2 <= r^2;
        linIdx = sub2ind(dims, xg(mask), yg(mask), zg(mask));
        if numel(linIdx) > sizes(i)
            linIdx = linIdx(randperm(numel(linIdx), sizes(i)));
        end
        model(linIdx) = true;
    end

    summary = extractEfficientClusterFeatures(model);
    if max(summary.sizes) > ref.maxSize * 1.2
        model = shrinkLargestCluster(model, ref.maxSize * 1.2);
    end
    model = enforcePorosityHardConstraint(model, targetPorosity, 500);
    mcmcModel = model;
    fprintf('    初始簇数: %d (目标 %d)，孔隙率: %.4f
', summary.numClusters, targetCount, mean(model(:)));
end
...
function model = applyMultiScaleCorrection(model, varargin)
    % 禁用多尺度差异纠正，保持当前模型
end
...
function model = enforcePorosityHardConstraint(model, targetPorosity, maxIters)
    ...
    while iter < maxIters
        ...
        if abs(diff) / numel(model) < 1e-4
            break;
        end
        boundary = computeBoundaryMasks(model);
        if diff > 0
            candidates = boundary.solidIdx;
            flipCount = min(abs(diff), numel(candidates));
            if flipCount == 0
                break;
            end
            chosen = selectLeastDisruptiveVoxels(model, candidates, flipCount, true);
            model(chosen) = true;
        else
            candidates = boundary.poreIdx;
            flipCount = min(abs(diff), numel(candidates));
            if flipCount == 0
                break;
            end
            chosen = selectLeastDisruptiveVoxels(model, candidates, flipCount, false);
            model(chosen) = false;
        end
        iter = iter + 1;
    end
...
function model = enhanceConnectivity(model)
    % 已禁用的全局连通性增强，保持模型当前拓扑
end
function model = createConnectionPath(model, varargin)
    % 已禁用的连接生成逻辑
end
function model = adjustModelAnisotropy(model, varargin)
    % 已禁用的各向异性重写，直接返回输入模型
end
function model = comprehensiveModelAdjustment(model, varargin)
    % 已禁用的综合调整，直接返回当前模型
end
function localStructure = createTargetStructure(sz, varargin)
    % 占位符：返回空结构以避免大规模扰动
    localStructure = false(sz, sz, max(1, round(sz/2)));
end
...
function mcmcState = refreshBoundaryContext(mcmcState)
    % 计算26邻域边界体素集合，周期性刷新以控制开销
    if ~isfield(mcmcState, 'boundaryRefreshCounter')
        mcmcState.boundaryRefreshCounter = 0;
    end
    mcmcState.boundaryRefreshCounter = mcmcState.boundaryRefreshCounter + 1;
    if mcmcState.boundaryRefreshCounter >= 25 || ~isfield(mcmcState, 'boundaryContext')
        pores = mcmcState.model;
        solids = ~pores;
        kernel = true(3,3,3);
        kernel(2,2,2) = false;
        neighborPore = convn(double(pores), double(kernel), 'same');
        neighborSolid = convn(double(solids), double(kernel), 'same');
        poreBoundary = pores & (neighborSolid > 0);
        solidBoundary = solids & (neighborPore > 0);
        mcmcState.boundaryContext = struct( ...
            'poreIdx', find(poreBoundary), ...
            'solidIdx', find(solidBoundary), ...
            'poreBoundaryMask', poreBoundary, ...
            'solidBoundaryMask', solidBoundary);
        mcmcState.boundaryRefreshCounter = 0;
    end
end
%% ========== 移动策略和生成函数 ==========
...
function [moves, moveTypes] = generateComprehensiveBatchMoves( ...
    mcmcState, batchSize, optParams, lookupTables, parallelEnabled, moveStrategy)
    %#ok<INUSD>
    if nargin < 6 || isempty(moveStrategy)
        moveStrategy = 'boundary';
    end
    ...
    boundary = mcmcState.boundaryContext;
    poreIdx = boundary.poreIdx;
    solidIdx = boundary.solidIdx;
    if isempty(poreIdx)
        poreIdx = [];
    end
    if isempty(solidIdx)
    ...
    for i = 1:batchSize
        if isempty(poreIdx) && isempty(solidIdx)
            moves{i} = struct('linearIdx', [], 'oldValues', [], 'newValues', []);
            moveTypes{i} = 'none';
            continue;
        end
        r = rand();
        if r < 0.45 && ~isempty(solidIdx)
            center = solidIdx(randi(numel(solidIdx)));
            moves{i} = createSingleFlip(center, false, true);
            moveTypes{i} = 'flip_to_pore';
        elseif r < 0.9 && ~isempty(poreIdx)
            center = poreIdx(randi(numel(poreIdx)));
            moves{i} = createSingleFlip(center, true, false);
            moveTypes{i} = 'flip_to_solid';
        else
            allBoundary = [poreIdx; solidIdx];
            center = allBoundary(randi(numel(allBoundary)));
            moves{i} = createLocalPatchMove(center, mcmcState.model);
            moveTypes{i} = 'local_patch';
        end
    end
end
function move = createSingleFlip(centerIdx, oldVal, newVal)
    move.linearIdx = centerIdx(:);
    move.oldValues = oldVal;
    move.newValues = newVal;
end
function move = createLocalPatchMove(centerIdx, model)
    dims = size(model);
    ...
    for dx = offsets
        for dy = offsets
            for dz = offsets
                ...
                if x >= 1 && x <= dims(1) && y >= 1 && y <= dims(2) && z >= 1 && z <= dims(3)
                    voxels(end+1) = sub2ind(dims, x, y, z); %#ok<AGROW>
                end
            end
        end
    end
    voxels = unique([centerIdx; voxels(:)]);
    voxels = voxels(randperm(numel(voxels), min(5, numel(voxels))));
    oldValues = model(voxels);
    newValues = ~oldValues;
    move.linearIdx = voxels(:);
    move.oldValues = oldValues(:);
...
function deltaEnergies = evaluateComprehensiveBatchMoves(mcmcState, moves, ...
    moveTypes, lookupTables, phase, parallelEnabled) %#ok<INUSD>
    nMoves = numel(moves);
    deltaEnergies = inf(nMoves, 1);
    if nMoves == 0
        return;
    end
    weights = getEnergyWeights(mcmcState.optParams, mcmcState.weights);
    baseEnergy = mcmcState.currentEnergy;
    for i = 1:nMoves
        move = moves{i};
        if isempty(move) || ~isfield(move, 'linearIdx') || isempty(move.linearIdx)
            continue;
        end
        candidate = mcmcState.model;
        candidate(move.linearIdx) = move.newValues;
        [candEnergy, ~, ~] = computeUnifiedEnergySnapshot(candidate, mcmcState.optParams, weights);
        deltaEnergies(i) = candEnergy - baseEnergy;
    end
end
...
function model = morphologyGuidedPerturbation(model, varargin)
    % 已禁用形态扰动，返回原模型
end
function model = largeSpatialPerturbation(model, varargin)
    % 已禁用大规模空间扰动
end
function model = balancedPerturbation(model, varargin)
    % 已禁用平衡扰动
end
...
function finalModel = comprehensivePostProcessWithSmallPores(model, optParams)
    % 极简后处理：仅移除极小孔隙、轻微平滑，并交由最终孔隙率硬约束处理
...
function model = referenceBasedMorphologyRestoration(model, varargin)
    % 禁用基于参考的大范围形态恢复
end
...
function model = structureOptimization(model, varargin)
    % 禁用结构优化，保留当前形态
end
...
function model = ensureConnectivity(model, varargin)
    % 禁用后处理阶段的强制连通性修改
end
