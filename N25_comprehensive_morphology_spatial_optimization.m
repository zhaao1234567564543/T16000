...
function mcmcModel = generateComprehensiveInitialModel(dims, targetPorosity, ...
    ...
    model = adjustClusterSizeDistribution(model, optParams, densityMap);
    ...
    model = fineTuneClusterStatistics(model, optParams, densityMap);
...
function model = adjustClusterSizeDistribution(model, optParams, densityMap)
    % 调整簇大小分布，使其更贴合目标统计量
    if nargin < 3
        densityMap = [];
    end

    targetMax = max(1, round(optParams.targetMax));
    targetMin = max(1, round(optParams.targetMin));
    targetMax = max(targetMax, targetMin);

    maxAllowed = max(targetMax, ceil(targetMax * 1.1));
    minAllowed = max(5, floor(targetMin * 0.9));
    ...
    while iter < maxIterations
        ...
        oversized = find(sizes > maxAllowed, 1, 'first');
        ...
        undersized = find(sizes < minAllowed, 1, 'first');
        if ~isempty(undersized)
...
function model = fineTuneClusterStatistics(model, optParams, densityMap)
    % 多轮微调簇数量与尺寸，使其与目标统计量高度一致
    if nargin < 3
        densityMap = [];
    end
    if ~isfield(optParams, 'targetClusterCount') || isempty(optParams.targetClusterCount)
    ...
    targetMin = max(1, round(optParams.targetMin));
    targetMax = max(targetMin, round(optParams.targetMax));
    ...
    for pass = 1:maxPasses
        ...
        % 调整过大的簇
        if stats.maxSize > targetMax
            CC = bwconncomp(model, 26);
            sizes = cellfun(@numel, CC.PixelIdxList);
            [~, order] = sort(sizes, 'descend');
            for idx = order'
                if sizes(idx) <= targetMax
                    continue;
                end
        ...
        % 调整过小的簇
        if stats.minSize < targetMin
            CC = bwconncomp(model, 26);
            sizes = cellfun(@numel, CC.PixelIdxList);
            [~, order] = sort(sizes, 'ascend');
            for idx = order'
                if sizes(idx) >= targetMin
                    break;
                end
                if isfield(optParams, 'preserveSmallPores') && optParams.preserveSmallPores
        ...
        if stats.count > 0
            if stats.maxSize > targetMax
                CC = bwconncomp(model, 26);
                sizes = cellfun(@numel, CC.PixelIdxList);
                oversized = find(sizes > targetMax);
                for idx = oversized'
                    refinedIdx = reduceOversizedClusterIndices(CC.PixelIdxList{idx}, size(model), targetMax, densityMap);
                    if ~isempty(refinedIdx)
                        model(CC.PixelIdxList{idx}) = false;
            ...
            if stats.minSize < targetMin
                CC = bwconncomp(model, 26);
                sizes = cellfun(@numel, CC.PixelIdxList);
                undersized = find(sizes < targetMin);
                for idx = undersized'
                    if isfield(optParams, 'preserveSmallPores') && optParams.preserveSmallPores
                        grownIdx = growSmallClusterIndices(CC.PixelIdxList{idx}, size(model), targetMin, densityMap);
                        if ~isempty(grownIdx)
        ...
        stats = computeClusterStatistics(model);
        if clusterStatsWithinTolerance(stats, targetMin, targetMax, targetCount)
            break;
        end
    end
end

function stats = computeClusterStatistics(model)
    % 计算当前模型的簇统计信息
    CC = bwconncomp(model, 26);
    stats = struct('count', CC.NumObjects, 'sizes', [], 'minSize', 0, 'maxSize', 0, 'meanSize', 0);
    if CC.NumObjects == 0
        return;
    end
    stats.sizes = cellfun(@numel, CC.PixelIdxList);
    stats.minSize = min(stats.sizes);
    stats.maxSize = max(stats.sizes);
    stats.meanSize = mean(stats.sizes);
end

function satisfied = clusterStatsWithinTolerance(stats, targetMin, targetMax, targetCount)
    % 判断簇统计是否在容差范围内
    if stats.count ~= targetCount
        satisfied = false;
        return;
    end
    if stats.count == 0
    ...
    minTol = max(1, floor(targetMin * 0.95));
...
function refinedIdx = reduceOversizedClusterIndices(clusterIdx, volumeSize, targetSize, varargin)
    ...
    while nnz(mask) > targetSize && iter < maxIterations
        ...
        removeCount = min(nnz(mask) - targetSize, max(1, round(0.02 * nnz(mask))));
        removeCount = min(removeCount, numel(order));
...
function model = reduceOversizedCluster(model, clusterIdx, optParams)
    ...
    densityMap = [];
    if isfield(optParams, 'referenceDensityMap')
...
function model = growSmallCluster(model, clusterIdx, optParams)
    ...
    if nargin >= 3 && ~isempty(optParams)
        if isfield(optParams, 'targetMin')
            targetSize = optParams.targetMin;
        end
        if isfield(optParams, 'referenceDensityMap')
            densityMap = optParams.referenceDensityMap;
        end
...
function model = enforceClusterSizeConstraints(model, optParams)
    ...
    if ~isempty(tinyClusters)
        for i = 1:length(tinyClusters)
            if isfield(optParams, 'preserveSmallPores') && optParams.preserveSmallPores
                model = growSmallCluster(model, CC.PixelIdxList{tinyClusters(i)}, optParams);
            else
...
function model = enforceTargetClusterCount(model, optParams)
    ...
    if CC.NumObjects ~= targetCount
        ...
        if CC.NumObjects > targetCount && tolerance > 0
            % 移除偏离目标尺寸最严重的簇以精确匹配
            sizes = cellfun(@numel, CC.PixelIdxList);
            penalties = abs(sizes - mean([optParams.targetMin, optParams.targetMax]));
            penalties = penalties + max(0, sizes - optParams.targetMax) + max(0, optParams.targetMin - sizes);
