...
function model = intelligentSplitCluster(model, clusterIdx, targetSize)
    ...
    if clusterSize == 0 || clusterSize <= max(targetSize, 1)
        return;
    end

    nParts = max(2, ceil(clusterSize / (max(targetSize, 1) * 1.2)));

    clusterMask = false(size(model));
    clusterMask(clusterIdx) = true;

    [model, success, nSegments] = graphCutSplitAndUpdateModel(model, clusterMask, nParts);

    if success
        fprintf('      图割成功：将簇划分为 %d 个部分\n', nSegments);
    else
        fprintf('      图割分裂失败，使用备选方法...\n');
        if clusterSize > 100
...
function [model, success, nSegments] = graphCutSplitAndUpdateModel(model, clusterMask, nParts)
    % 基于图割的簇分裂并更新原始模型
    success = false;
    nSegments = 1;

    if nnz(clusterMask) < 2
        return;
    end

    dims = size(model);
    [x, y, z] = ind2sub(dims, find(clusterMask));
    xmin = max(1, min(x) - 1); xmax = min(dims(1), max(x) + 1);
    ymin = max(1, min(y) - 1); ymax = min(dims(2), max(y) + 1);
    zmin = max(1, min(z) - 1); zmax = min(dims(3), max(z) + 1);

    localCluster = clusterMask(xmin:xmax, ymin:ymax, zmin:zmax);

    [labelsLocal, boundaryLocal, success] = graphCutPartitionCluster(localCluster, nParts);
    if ~success
        return;
    end

    segmentedLocal = localCluster;
    segmentedLocal(boundaryLocal) = false;

    localRegion = model(xmin:xmax, ymin:ymax, zmin:zmax);
    localRegion(localCluster) = false;
    localRegion(segmentedLocal) = true;
    model(xmin:xmax, ymin:ymax, zmin:zmax) = localRegion;

    labelVals = labelsLocal(labelsLocal > 0);
    if ~isempty(labelVals)
        nSegments = numel(unique(labelVals));
    end
end

function [labelsVolume, boundaryMask, success] = graphCutPartitionCluster(clusterMask, nParts)
    % 对给定簇执行归一化图割分割
    success = false;
    labelsVolume = zeros(size(clusterMask), 'uint16');
    boundaryMask = false(size(clusterMask));

    voxelIdx = find(clusterMask);
    nVox = numel(voxelIdx);
    if nVox < 2
        return;
    end

    dims = size(clusterMask);
    nParts = min(max(2, nParts), min(8, nVox));

    [vx, vy, vz] = ind2sub(dims, voxelIdx);
    indexMap = zeros(dims, 'int32');
    indexMap(voxelIdx) = int32(1:nVox);

    maxEdges = max(1, nVox * 13);
    rows = zeros(maxEdges, 1);
    cols = zeros(maxEdges, 1);
    vals = zeros(maxEdges, 1);
    edgeCount = 0;

    distField = bwdist(~clusterMask);

    for n = 1:nVox
        cx = vx(n); cy = vy(n); cz = vz(n);
        baseIndex = indexMap(cx, cy, cz);
        centerThickness = distField(cx, cy, cz) + 1;
        ...
        for dx = -1:1
            for dy = -1:1
                for dz = -1:1
                    ...
                    neighborIndex = indexMap(nx, ny, nz);
                    if neighborIndex == 0 || neighborIndex <= baseIndex
                        continue;
                    end

                    edgeCount = edgeCount + 1;
                    if edgeCount > numel(rows)
                        rows = [rows; zeros(maxEdges, 1)];
                        cols = [cols; zeros(maxEdges, 1)];
                        vals = [vals; zeros(maxEdges, 1)];
                    end

                    rows(edgeCount) = baseIndex;
                    cols(edgeCount) = neighborIndex;
                    neighborThickness = distField(nx, ny, nz) + 1;
                    spatialDistance = sqrt(double(dx^2 + dy^2 + dz^2));
                    vals(edgeCount) = (centerThickness + neighborThickness) / (spatialDistance * 2);
                end
            end
        end
    end
    ...
    rows = rows(1:edgeCount);
    cols = cols(1:edgeCount);
    vals = vals(1:edgeCount);

    W = sparse([rows; cols], [cols; rows], [vals; vals], nVox, nVox);
    degreeVals = full(sum(W, 2));
    D = spdiags(degreeVals, 0, nVox, nVox);
    L = D - W;

    numVecs = min(max(2, nParts), max(2, min(nVox - 1, 8)));

    try
        opts = struct('tol', 1e-5, 'maxit', 400);
        [vecs, diagVals] = eigs(L, D, numVecs, 'smallestabs', opts);
    catch
        try
            opts = struct('tol', 1e-5, 'maxit', 400);
            [vecs, diagVals] = eigs(L + 1e-6 * speye(nVox), D, numVecs, 'smallestabs', opts);
        catch
            if nVox <= 400
                [vecs, diagVals] = eig(full(L), full(D));
            else
                return;
            end
        end
    ...
    vecs = real(vecs);
    diagVals = real(diag(diagVals));
    [diagVals, order] = sort(diagVals);
    vecs = vecs(:, order);
    ...
    vecs = vecs(:, 2:min(numVecs, size(vecs, 2)));
    if isempty(vecs)
        return;
    end

    rowNorm = sqrt(sum(vecs.^2, 2));
    rowNorm(rowNorm == 0) = 1;
    spectralFeatures = vecs ./ rowNorm;

    numClusters = min(max(2, nParts), size(spectralFeatures, 2) + 1);

    if numClusters == 2
        fiedlerVec = spectralFeatures(:, 1);
        if std(fiedlerVec) < eps
            return;
        end
        threshold = median(fiedlerVec);
        labels = ones(nVox, 1);
        labels(fiedlerVec > threshold) = 2;
    else
        try
            labels = kmeans(spectralFeatures, numClusters, ...
                'Replicates', 5, 'MaxIter', 200, 'Display', 'off', 'Start', 'plus');
        catch
            return;
        end
    end
    ...
    labelsVolume(voxelIdx) = uint16(labels);

    boundaryNodes = false(nVox, 1);
    for e = 1:edgeCount
        a = rows(e);
        b = cols(e);
        if labels(a) ~= labels(b)
            boundaryNodes(a) = true;
            boundaryNodes(b) = true;
        end
    end

    boundaryMask(voxelIdx(boundaryNodes)) = true;
    success = true;
end
...
function model = splitClusterForTarget(model, CC, optParams)
    ...
    model = intelligentSplitCluster(model, CC.PixelIdxList{largestIdx}, optParams.targetMax);
...
function model = splitOversizedPores(model, optParams)
    ...
    sizes = cellfun(@numel, CC.PixelIdxList);
    sizeThreshold = max(optParams.targetMax, 1) * 2;
    oversizedIdx = find(sizes > sizeThreshold);
    ...
    for i = 1:length(oversizedIdx)
        ...
        targetParts = max(2, ceil(sizes(idx) / max(optParams.targetMax, 1)));
        [model, success, nSegments] = graphCutSplitAndUpdateModel(model, clusterMask, targetParts);

        if success
            fprintf('    图割后处理：簇被细分为 %d 个部分\n', nSegments);
        else
            fprintf('    图割后处理失败，回退到形态学分割...\n');
            model = watershedFallbackSplit(model, clusterMask);
        end
    end
end

function model = watershedFallbackSplit(model, clusterMask)
    % 图割失败时的保底分水岭分割
    clusterIdx = find(clusterMask);
    if isempty(clusterIdx)
        return;
    end

    dims = size(model);
    [x, y, z] = ind2sub(dims, clusterIdx);
    xmin = max(1, min(x) - 1); xmax = min(dims(1), max(x) + 1);
    ymin = max(1, min(y) - 1); ymax = min(dims(2), max(y) + 1);
    zmin = max(1, min(z) - 1); zmax = min(dims(3), max(z) + 1);
    ...
    D = -bwdist(~localCluster);
    markerMask = imextendedmin(D, 2);
    segmented = watershed(imimposemin(D, markerMask));

    refinedLocal = localCluster;
    refinedLocal(segmented == 0) = false;

    localRegion = model(xmin:xmax, ymin:ymax, zmin:zmax);
    localRegion(localCluster) = false;
    localRegion(refinedLocal) = true;
    model(xmin:xmax, ymin:ymax, zmin:zmax) = localRegion;
end
